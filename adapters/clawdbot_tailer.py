#!/usr/bin/env python3
"""Clawdbot adapter: tail session JSONL -> kaitd /ingest

This makes Kait model/provider-agnostic and (mostly) runtime-agnostic.
It reads what Clawdbot already writes (session transcripts) and emits normalized
KaitEventV1 events.

Usage:
  python3 adapters/clawdbot_tailer.py --kaitd http://127.0.0.1:<kaitd-port> --agent main

Notes:
- This is intentionally simple: it tails the latest session file for an agent.
- It de-dupes using a simple line offset persisted in ~/.kait/adapters/*.json.
"""
    ...
    while True:
        try:
            ...
            for line in batch:
                ...
                _post_json(args.kaitd.rstrip("/") + "/ingest", evt, token=token)
                sent += 1
