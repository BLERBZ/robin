#!/usr/bin/env python3
"""OpenClaw adapter: tail session JSONL -> kaitd /ingest

Reads OpenClaw session transcripts (~/.openclaw/agents/<agent>/sessions/) and
emits normalized KaitEventV1 events to kaitd.

Usage:
  python3 adapters/openclaw_tailer.py --kaitd http://127.0.0.1:8787 --agent main
...
- De-dupes using per-file line offsets persisted in ~/.kait/adapters/.
...
STATE_DIR = Path.home() / ".kait" / "adapters"

MAX_TOOL_RESULT_CHARS = 4000

DEFAULT_REPORT_DIR = Path.home() / ".openclaw" / "workspace" / "kait_reports"
DEFAULT_HOOK_EVENTS_FILE = Path(
    os.environ.get("KAIT_OPENCLAW_HOOK_EVENTS_FILE")
    or (Path.home() / ".kait" / "openclaw_hook_events.jsonl")
)

# Optional integration heartbeat (off by default)
HEARTBEAT_ENABLED = os.environ.get("KAIT_OPENCLAW_HEARTBEAT", "").strip().lower() not in ("", "0", "false", "no")
HEARTBEAT_EVERY_SECONDS = int(float(os.environ.get("KAIT_OPENCLAW_HEARTBEAT_MINUTES", "15")) * 60)
HEARTBEAT_PATH = Path(
    os.environ.get("KAIT_OPENCLAW_HEARTBEAT_PATH")
    or (Path.home() / ".kait" / "logs" / "openclaw_tailer_heartbeat.jsonl")
)
...
def parse_openclaw_line(obj: dict, session_key: str) -> list:
    """Parse one JSONL line into zero or more KaitEventV1 events."""
    events = []
...
def _scan_reports(report_dir: Path, kaitd_url: str, token: str = None, verbose: bool = False):
    ...
    for f in sorted(report_dir.glob("*.json")):
        ...
        try:
            _post_json(kaitd_url.rstrip("/") + "/ingest", evt, token=token)
        except Exception as e:
            if verbose:
...
def _parse_hook_event_row(row: dict):
    """Map hook spool row -> KaitEventV1 (or None when unsupported)."""
...
def main():
    ap = argparse.ArgumentParser(description="OpenClaw adapter: tail session JSONL -> kaitd /ingest")
    ...
    ap.add_argument("--token", default=None, help="kaitd auth token (or set KAITD_TOKEN env, or use ~/.kait/kaitd.token)")
        ...
        try:
            ...
            for session_key, session_file in sessions:
                ...
                if state.is_new_file(file_key):
                    ...
                    try:
                        _post_json(kaitd_url.rstrip("/") + "/ingest", boundary_evt, token=token)
                        if args.verbose:
            ...
            # --- Scan self-reports ---
            try:
                _scan_reports(report_dir, kaitd_url, token=token, verbose=args.verbose)
            except Exception as e:
                if args.verbose:
