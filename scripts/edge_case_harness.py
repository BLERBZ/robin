...
DEFAULT_CASES = [
    ...
    ("unicode_context", "Edit", {"file_path": str(Path.home() / ".kait" / "notes" / "edge_unicode.md")}, "append unicode test"),
    ("json_blob", "Write", {"file_path": str(Path.home() / ".kait" / "notes" / "edge_json.md")}, '{"action":"update","target":"auth","flags":["safe","dry_run"]}'),
    ("long_context", "Write", {"file_path": str(Path.home() / ".kait" / "notes" / "edge_long_context.md")}, "long context " * 120),
    ("code_snippet", "Edit", {"file_path": str(Path.home() / ".kait" / "notes" / "edge_code.md")}, "def handler(req):\n    return 200\n"),
]
