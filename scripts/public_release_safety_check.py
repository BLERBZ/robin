...
def _dep_audit_timeout_seconds() -> int:
    raw = os.environ.get("KAIT_RELEASE_DEP_AUDIT_TIMEOUT_SEC", "").strip()
    if not raw:
        return 180
    try:
...
def _dependency_audit_is_strict() -> bool:
    configured = _parse_env_bool("KAIT_RELEASE_REQUIRE_DEP_AUDIT")
...
def _run_dependency_audit() -> list[str]:
    ...
    env_skip = (os.environ.get("KAIT_RELEASE_SKIP_DEP_AUDIT", "").strip().lower() in {"1", "true", "yes", "on"})
    if env_skip:
        return findings
...
def _sast_scan_is_strict() -> bool:
    configured = _parse_env_bool("KAIT_RELEASE_REQUIRE_SAST")
    if configured is not None:
        return configured
    return True
...
def _sast_refresh_requested() -> bool:
    return _parse_env_bool("KAIT_RELEASE_REFRESH_SAST_BASELINE") is True


def _sast_baseline_allowed() -> bool:
    """Baseline allowlist is opt-in; default gate is strict/no-baseline."""
    return _parse_env_bool("KAIT_RELEASE_ALLOW_SAST_BASELINE") is True
...
def _run_sast_ruff_gate() -> list[str]:
    ...
    skip_sast = _parse_env_bool("KAIT_RELEASE_SKIP_SAST") is True
    if skip_sast:
        return findings
    ...
    if not baseline:
        if strict:
            return [
                ...
                "sast_ruff: run with KAIT_RELEASE_REFRESH_SAST_BASELINE=1 to create baseline intentionally",
            ]
        return findings
