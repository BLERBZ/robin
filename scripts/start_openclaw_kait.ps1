# start_openclaw_kait.ps1 - Start OpenClaw bridge-layer services without owning core Kait by default.
param(
    [switch]$WithCore
)
...
$pidFile = Join-Path $RepoRoot "scripts\.kait_openclaw_pids.json"
...
Set-DefaultEnv "KAIT_CHIP_REQUIRE_LEARNING_SCHEMA" "1"
Set-DefaultEnv "KAIT_CHIP_OBSERVER_ONLY" "1"
Set-DefaultEnv "KAIT_CHIP_MIN_LEARNING_EVIDENCE" "2"
Set-DefaultEnv "KAIT_CHIP_MIN_CONFIDENCE" "0.65"
Set-DefaultEnv "KAIT_CHIP_MIN_SCORE" "0.25"
Set-DefaultEnv "KAIT_CHIP_MERGE_MIN_CONFIDENCE" "0.65"
Set-DefaultEnv "KAIT_CHIP_MERGE_MIN_QUALITY" "0.62"

Write-Host "=== Kait x OpenClaw - Starting ===" -ForegroundColor Cyan
...
if ($WithCore) {
    # 1) kaitd (only if not already running)
    $kaitdProc = Get-ProcessByPattern "kaitd.py|-m kaitd"
    if ($kaitdProc) {
        Write-Host "`n[1/3] kaitd already running (PID $($kaitdProc.ProcessId))" -ForegroundColor DarkGray
    } else {
        Write-Host "`n[1/3] Starting kaitd..." -ForegroundColor Yellow
        $kaitd = Start-Process -FilePath python -ArgumentList "$RepoRoot\kaitd.py" `
            -WorkingDirectory $RepoRoot -WindowStyle Hidden -PassThru
        $ownedKaitd = $true
        Write-Host "  PID: $($kaitd.Id)"
...
$hookSpool = Join-Path $env:USERPROFILE ".kait\openclaw_hook_events.jsonl"
if ($existingTailer) {
...
# Save OpenClaw-managed PIDs in an isolated file (never shared with kait.ps1).
@{
    owner        = "openclaw_bridge_layer"
    kaitd       = if ($kaitd) { $kaitd.Id } else { $null }
    bridge       = if ($bridge) { $bridge.Id } else { $null }
    tailer       = $tailerPid
    owns_kaitd  = $ownedKaitd
