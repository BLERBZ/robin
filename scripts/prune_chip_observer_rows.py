...
Reads observer policy from ~/.kait/chip_observer_policy.json and removes rows
...
def _load_registry_active(project_path: str) -> List[str]:
    registry = Path.home() / ".kait" / "chip_registry.json"
...
def main() -> int:
    ...
    ap.add_argument("--policy-file", default=str(Path.home() / ".kait" / "chip_observer_policy.json"))
    ap.add_argument("--chip-dir", default=str(Path.home() / ".kait" / "chip_insights"))
    ap.add_argument("--active-only", action="store_true", help="Only process active chips for a project")
    ap.add_argument(
        "--project-path",
        default=".",
        help="Project path key used in ~/.kait/chip_registry.json when --active-only is set",
    )
    ap.add_argument(
    ...
    if bool(args.archive):
        ...
        archive_dir = Path.home() / ".kait" / "archive" / "chip_insights_prune" / stamp
