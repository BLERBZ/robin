...
$kaitRoot = Join-Path $homeDir ".kait"
$queuePath = Join-Path $kaitRoot "queue\events.jsonl"
$bridgeHeartbeatPath = Join-Path $kaitRoot "bridge_worker_heartbeat.json"
$schedulerHeartbeatPath = Join-Path $kaitRoot "scheduler_heartbeat.json"
...
$hookSpoolPath = Join-Path $kaitRoot "openclaw_hook_events.jsonl"
if ($openclawCfg) {
    $allow = @()
    if ($openclawCfg.plugins -and $openclawCfg.plugins.allow) {
        $allow = @($openclawCfg.plugins.allow)
    }
    if ($allow -contains "kait-telemetry-hooks") {
        Add-Check "plugin.allowlist" "PASS" "kait-telemetry-hooks included."
    }
    else {
        Add-Check "plugin.allowlist" "FAIL" "kait-telemetry-hooks missing from plugins.allow."
    }

    $entry = $null
    if ($openclawCfg.plugins -and $openclawCfg.plugins.entries) {
        $entry = $openclawCfg.plugins.entries."kait-telemetry-hooks"
    }
    if ($entry -and $entry.enabled -ne $false) {
        Add-Check "plugin.entry_enabled" "PASS" "plugins.entries.kait-telemetry-hooks enabled."
    }
    else {
        Add-Check "plugin.entry_enabled" "FAIL" "plugins.entries.kait-telemetry-hooks not enabled."
    }
...
try {
    $svcRaw = & python -c "from lib.service_control import service_status; import json; s=service_status(); print(json.dumps({k:s.get(k,{}) for k in ['kaitd','bridge_worker','scheduler','watchdog']}))"
    $svc = $svcRaw | ConvertFrom-Json
    foreach ($name in @("kaitd", "bridge_worker", "scheduler", "watchdog")) {
        $row = $svc.$name
...
if (-not (Test-Path $cronJobsPath)) {
    ...
    try {
        ...
        $jobs = @($cron.jobs)
        $targetNames = @("kait-health-alert-watch", "kait-context-refresh")
