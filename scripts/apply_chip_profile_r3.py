...
Writes chip_merge quality limits into ~/.kait/tuneables.json so merge/distillation
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Apply R3 chip profile to ~/.kait/tuneables.json")
    ap.add_argument(
        "--path",
        default=str(Path.home() / ".kait" / "tuneables.json"),
        help="Tuneables file path (default: ~/.kait/tuneables.json)",
    )
    args = ap.parse_args()
