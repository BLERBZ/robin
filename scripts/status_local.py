#!/usr/bin/env python3
"""Show local Kait service status (cross-platform)."""
...
def main() -> None:
    ...
    try:
        ...
        print(
            f"[kait] queue: {queue_count} events (pattern backlog {pattern_backlog}, validation backlog {validation_backlog})"
        )
    except Exception as e:
        print(f"[kait] queue: error ({e})")
    ...
    try:
        ...
        if last_ts:
            ...
            print(
                f"[kait] validation: last {age_s}s (+{stats.get('validated', 0)} / -{stats.get('contradicted', 0)})"
            )
        else:
            print("[kait] validation: never run")
    except Exception as e:
        print(f"[kait] validation: error ({e})")
