...
def check_tuneables() -> dict:
    path = Path.home() / ".kait" / "tuneables.json"
    if not path.exists():
...
def check_index(require_nonempty: bool = True) -> int:
    db = Path.home() / ".kait" / "semantic" / "insights_vec.sqlite"
    if not db.exists():
    ...
    if require_nonempty and count == 0:
        raise SystemExit("[ERROR] Semantic index is empty (run python -m kait.index_embeddings --all)")
    return count
...
def main() -> int:
    run_cmd("python -m kait.cli health", "Health Check")
    run_cmd("python -m kait.cli services", "Services Check")
    check_env_vars()
    check_tuneables()
    ...
    logs = Path.home() / ".kait" / "logs" / "semantic_retrieval.jsonl"
    tail_jsonl(logs, "Semantic Retrieval Log (last 2)", max_lines=2)
    metrics = Path.home() / ".kait" / "advisor" / "metrics.json"
