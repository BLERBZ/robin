#!/usr/bin/env python3
"""Generate a redacted Kait x OpenClaw integration audit report."""
...
def _has_llm_hooks(cfg: Dict[str, Any]) -> bool:
    ...
    if not isinstance(entries, dict):
        return False
    telemetry_raw = entries.get("kait-telemetry-hooks")
...
def build_report(
    *,
    ...
    health_job = None
    for row in job_rows:
        if str(row.get("name") or "").strip() == "kait-health-alert-watch":
            health_job = row
            break
...
def render_markdown(report: Dict[str, Any]) -> str:
    ...
    health = report.get("kait_health_job") or {}
    ...
    lines = [
        ...
        f"| kait-health-alert-watch present | `{_status(bool(health.get('present')))}` |",
...
def main() -> int:
    home = Path.home()
    ap = argparse.ArgumentParser(description="Audit Kait x OpenClaw integration state.")
    ap.add_argument(
        "--openclaw-config",
        type=Path,
