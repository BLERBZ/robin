#!/usr/bin/env python3
"""Run a real-time OpenClaw x Kait end-to-end benchmark."""
...
HOOK_SPOOL = KAIT_DIR / "openclaw_hook_events.jsonl"
QUEUE_FILE = KAIT_DIR / "queue" / "events.jsonl"
REQUESTS_FILE = KAIT_DIR / "advice_feedback_requests.jsonl"
FEEDBACK_FILE = KAIT_DIR / "advice_feedback.jsonl"
OUTCOME_FILE = KAIT_DIR / "meta_ralph" / "outcome_tracking.json"
ENGINE_FILE = KAIT_DIR / "advisory_engine.jsonl"
FALLBACK_ADVISORY = KAIT_DIR / "llm_advisory.md"
...
def _canary_suite(agent: str, timeout_s: int) -> Dict[str, Any]:
    prompts = [
        (
            "Kait realtime benchmark canary A. Run one Bash tool call to execute "
            "`echo KAIT_REALTIME_CANARY_A`, then provide one actionable recommendation."
        ),
        (
            "Kait realtime benchmark canary B. Read AGENTS.md from the workspace, then "
...
def _workspace_metrics(now_ts: float) -> Dict[str, Any]:
    ...
    for ws in discover_openclaw_workspaces(include_nonexistent=True):
        context = ws / "KAIT_CONTEXT.md"
        advisory = ws / "KAIT_ADVISORY.md"
        notif = ws / "KAIT_NOTIFICATIONS.md"
        row = {
            "workspace": str(ws),
            "exists": ws.exists(),
...
def run_benchmark(
    *,
    ...
    core_ok = all(
        ...
        for name in ("kaitd", "bridge_worker", "scheduler", "watchdog")
    )
    checks = [
        _check(core_ok, "core_services_running", {k: (svc.get(k) or {}).get("running") for k in ("kaitd", "bridge_worker", "scheduler", "watchdog")}),
        _check(
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Realtime OpenClaw x Kait E2E benchmark")
    ...
    ap.add_argument("--canary-agent", default="kait-speed", help="OpenClaw agent id used for canary turns")
