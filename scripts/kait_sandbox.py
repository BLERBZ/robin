#!/usr/bin/env python3
"""
Kait Sandbox Harness

Runs an isolated end-to-end Kait learning loop (capture -> process -> validate ->
...
def _set_env(paths: SandboxPaths) -> None:
    # Force all Kait Path.home() calls into the sandbox.
...
def _patch_repo_relative_paths(sandbox_home: Path) -> None:
    # Some modules write to repo-local paths; repoint to sandbox.
    from lib import aha_tracker as _aha
    from lib import kait_voice as _voice
    from lib import growth_tracker as _growth

    sandbox_kait = sandbox_home / ".kait"

    _aha.KAIT_DIR = sandbox_kait
    _aha.AHA_FILE = sandbox_kait / "aha_moments.json"

    _voice.KAIT_DIR = sandbox_kait
    _voice.KAIT_DIR = sandbox_kait  # back-compat alias
    _voice.VOICE_FILE = sandbox_kait / "voice.json"

    _growth.GrowthTracker.GROWTH_FILE = sandbox_kait / "growth.json"
...
def _seed_project_profile(project_dir: Path) -> None:
    ...
    record_answer(profile, "gen_goal", "Build a Kait sandbox that exercises the full learning loop.")
    ...
    _add_entry("goals", "Automate regression checks for Kait learning quality.")
    _add_entry("milestones", "Sandbox generates report + baseline diff.")
    _add_entry("references", "Kait sandbox harness format")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Kait sandbox harness")
    ap.add_argument("--sandbox-dir", default=str(REPO_ROOT / "sandbox" / "kait_sandbox"))
    ap.add_argument("--project-dir", default=None)
    ap.add_argument("--workspace-dir", default=None)
    ap.add_argument("--skills-dir", default=None)
    ap.add_argument("--report", default=None)
    ap.add_argument("--baseline", default=str(REPO_ROOT / "sandbox" / "kait_sandbox_baseline.json"))
    ap.add_argument("--diff", default=str(REPO_ROOT / "sandbox" / "kait_sandbox_diff.json"))
