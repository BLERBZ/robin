#!/usr/bin/env python3
"""Stress benchmark local Ollama models across Kait-style intelligence tasks."""
...
def _prompt_for_live_event(tool_name: str, ctx: str) -> str:
    return (
        "Live pipeline replay from Kait events.\n"
        f"Tool: {tool_name}\n"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Stress-test local Ollama models across Kait workloads.")
    ap.add_argument(
        "--models",
    ...
    ap.add_argument(
        "--live-log",
        default=str(Path.home() / ".kait" / "queue" / "events.jsonl"),
        help="Path to live Kait events log used for dynamic scenarios",
    )
    ap.add_argument(
        "--live-scenarios",
        type=int,
