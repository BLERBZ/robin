#!/usr/bin/env python3
"""Verify [KAIT] advisory emissions are working correctly.
...
def _scenarios():
    ...
    def i01():
        ...
        assert text.startswith("[KAIT]"), f"NOTE should have [KAIT] prefix, got: {text[:20]}"
        assert len(text) < 500, f"Should be concise, got {len(text)} chars"
        return f"formatted correctly ({len(text)} chars)"
    add("I01", "NOTE format has [KAIT] prefix", i01)

    def i02():
        from lib.advisory_emitter import format_advisory
        text = format_advisory("[Caution] Never store passwords in plaintext.", "warning")
        assert "[KAIT ADVISORY]" in text, f"WARNING should have [KAIT ADVISORY], got: {text[:30]}"
...
def main():
    ...
    print()
    print("[KAIT ADVISORY VERIFICATION]")
    print("=" * 60)
