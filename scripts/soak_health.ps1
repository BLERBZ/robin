param(
  [int]$Minutes = 10,
  [string]$KaitdBaseUrl = "http://127.0.0.1:8787",
...
while ((Get-Date) -lt $deadline) {
  ...
  try {
    ...
    $health = (Invoke-RestMethod -Uri ($KaitdBaseUrl + "/health") -Method Get -TimeoutSec 5).ToString().Trim()
    $status = Invoke-RestMethod -Uri ($KaitdBaseUrl + "/status") -Method Get -TimeoutSec 5
