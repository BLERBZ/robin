# stop_openclaw_kait.ps1 - Stop OpenClaw bridge-layer services safely.
param(
    [switch]$ForceCore
)
...
Write-Host "=== Kait x OpenClaw - Stopping ===" -ForegroundColor Cyan
if ($ForceCore) {
...
if (Test-Path $pidFile) {
    ...
    if ($ForceCore) {
        ...
        if ([bool]$pids.owns_kaitd) {
            Stop-IfRunning -Pid ([int]$pids.kaitd) -Name "kaitd"
        } elseif ($pids.kaitd) {
            Write-Host "  kaitd PID present but not owned by OpenClaw layer; skipped" -ForegroundColor DarkGray
        }
    }
