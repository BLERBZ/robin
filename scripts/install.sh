...
echo "========================================"
echo "  KAIT - Self-Evolving Intelligence"
echo "========================================"
echo ""
...
if [ -f "$KAIT_DIR/pyproject.toml" ]; then
  code=0
  pip_install_editable "$KAIT_DIR"
  code=$?
  if [ "$code" -ne 0 ]; then
    if [ "$code" -ne 10 ]; then
      exit "$code"
    ...
    ensure_venv
    pip_install_editable "$KAIT_DIR" || exit $?
  fi
...
# Create Kait config directory
echo ""
echo "Setting up Kait config..."
mkdir -p "$HOME/.kait"
echo "Config directory: ~/.kait"
...
if [ -d "$CLAUDE_CONFIG_DIR" ]; then
  ...
  if [[ ! "$hook_reply" =~ ^[Nn]$ ]]; then
    cat > "$CLAUDE_CONFIG_DIR/kait-hooks.json" <<'HOOKS'
{
  "hooks": {
    "PostToolUse": [{
      "matcher": "",
      "hooks": [{
        "type": "command",
        "command": "python3 $KAIT_DIR/hooks/observe.py"
      }]
    }],
    "PostToolUseFailure": [{
      "matcher": "",
...
cd "$KAIT_DIR"
if "$PYTHON_BIN" -m kait.cli health > /dev/null 2>&1; then
  echo "Kait is working!"
else
...
echo "========================================"
echo ""
echo "Kait directory: $KAIT_DIR"
echo ""
echo "Quick start:"
echo "  cd $KAIT_DIR"
echo "  $PYTHON_BIN -m kait.cli status    # Check status"
echo "  $PYTHON_BIN -m kait.cli health    # Health check"
echo "  $PYTHON_BIN -m kait.cli learnings # View learnings"
echo ""
...
echo "  $PYTHON_BIN -m kait.cli sync   # Sync learnings"
echo ""
echo "Documentation: $KAIT_DIR/README.md"
echo ""
