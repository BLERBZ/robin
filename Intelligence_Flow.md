...
Repository: kait-intel
Scope:
- Runtime Python modules: root *.py, lib/, hooks/, adapters/, kait/
...
- kaitd.py (HTTP ingest for KaitEventV1)
- adapters/* (stdin_ingest, clawdbot_tailer, moltbook)
- scripts/emit_event.py (manual event injector)

Central bus:
- lib/queue.py writes events to ~/.kait/queue/events.jsonl (append-only) with:
  - lock-contention spillover to ~/.kait/queue/events.overflow.jsonl
  - logical consume head in ~/.kait/queue/state.json (O(1) consume + periodic compaction)

Processing loop:
- bridge_worker.py -> lib/bridge_cycle.run_bridge_cycle

High-level flow (ASCII):
sources -> queue -> bridge_cycle -> {update_kait_context, memory capture, pattern detection, validation/prediction, content learning, chips}
...
- lib/mind_bridge.py (retrieval used by advisor + lib.bridge for KAIT_CONTEXT)
- Mind sync is manual (kait sync / lib.mind_bridge.sync_all_to_mind); offline queue used when Mind is down.
- mind_bridge health checks are cached/backed-off to avoid repeated blocking calls while Mind is unavailable.
- If Mind CLI is unstable, Kait can run the built-in Mind server (see start_mind.ps1; KAIT_FORCE_BUILTIN_MIND=1).

Observability:
- Kait Pulse (primary web dashboard) via external vibeship-kait-pulse/app.py (port KAIT_PULSE_PORT, default 8765).
...
- kait_watchdog.py + lib/service_control.py (monitor/restart services)

## 2) Primary workflows (end-to-end)

### 2.1 Event ingest -> queue
1) hooks/observe.py captures tool/user events and emits KaitEvent payloads.
2) kaitd.py validates KaitEventV1 and writes queue events via lib.queue.quick_capture.
3) adapters (stdin_ingest, clawdbot_tailer, moltbook) also feed kaitd or queue.
...
7) A heartbeat is written to ~/.kait/bridge_worker_heartbeat.json.

### 2.2.1 Trace context propagation (v1)
- trace_id is attached at ingest (hooks/observe.py, kaitd.py, lib.queue.quick_capture).
...
- trace binding enforcement: steps, evidence, and outcomes should record trace_id; TRACE_GAP watcher warns on missing bindings. Set `KAIT_TRACE_STRICT=1` to block actions on trace gaps.
- dashboards should drill down by trace_id for audit and validation.

### 2.3 Context sync + promotion (live vs durable)
1) lib.bridge.update_kait_context builds a live context pack (insights, warnings, advice, skills, taste, outcomes) and writes KAIT_CONTEXT.md.
2) lib.context_sync selects high-confidence insights and writes live context to output adapters.
   ...
   - context_sync also injects recent high-quality chip highlights from ~/.kait/chip_insights.
   - context_sync does not sync to Mind (see 2.8).
3) lib.promoter is a separate, manual step (kait promote) that writes durable learnings into CLAUDE.md / AGENTS.md / TOOLS.md / SOUL.md.
...
2) lib.semantic_retriever stores vectors in ~/.kait/semantic/insights_vec.sqlite.
3) Backfill command: python -m kait.index_embeddings --all
...
5) lib.eidos.store persists to ~/.kait/eidos.db; lib.eidos.retriever retrieves for guidance.
...
4) On packet miss, engine falls back to live advisor retrieval, then gate + synthesis + emit.
   - packet no-emit fallback emission is opt-in (`KAIT_ADVISORY_PACKET_FALLBACK_EMIT=1` or `advisory_engine.packet_fallback_emit_enabled=true`).
...
12) Semantic retrieval path (when semantic.enabled=true):
   ...
   - retrieval log: ~/.kait/logs/semantic_retrieval.jsonl (intent, candidates, triggers, top-N scores)
13) Advisor retrieval router (Carmack-style, retrieval section in tuneables):
   ...
   - route telemetry:
     - ~/.kait/advisor/retrieval_router.jsonl with route, reasons, elapsed, budget/cap flags
...
2) lib.advisor and lib.bridge (KAIT_CONTEXT) read from Mind for advice/context.
3) Mind sync is manual via kait sync (lib.mind_bridge.sync_all_to_mind); offline queue stores when Mind is down.
...
- lib/resonance.py + lib/kait_voice.py: internal state and voice calibration (used by bridge context).
...
### 2.11 Services / ops
- lib/service_control.py starts/stops kaitd, bridge_worker, dashboard, watchdog, and pulse.
- pulse process management targets external vibeship-kait-pulse/app.py.
- kait_watchdog.py checks health, queue size, and heartbeat freshness.

## 3) Data stores and artifacts (local)

Core queue + ingest:
- ~/.kait/queue/events.jsonl
- ~/.kait/queue/.queue.lock
- ~/.kait/queue/events.overflow.jsonl
- ~/.kait/queue/state.json
- ~/.kait/invalid_events.jsonl

Context + promotion:
- ~/.kait/cognitive_insights.json
- ~/.kait/.cognitive.lock
- ~/.kait/pending_memory.json
- ~/.kait/memory_capture_state.json
- ~/.kait/bridge_worker_heartbeat.json
Workspace context:
- KAIT_CONTEXT.md (workspace, written by lib.bridge.update_kait_context)

Pattern + EIDOS:
- ~/.kait/detected_patterns.jsonl
- ~/.kait/pattern_detection_state.json
- ~/.kait/eidos.db
- ~/.kait/truth_ledger.json
- ~/.kait/acceptance_plans.json
  - steps table includes trace_id (v1 trace context)

Memory banks + store:
- ~/.kait/banks/*.jsonl
- ~/.kait/memory_store.sqlite
Semantic retrieval:
- ~/.kait/semantic/insights_vec.sqlite
- ~/.kait/logs/semantic_retrieval.jsonl
Advisor metrics:
- ~/.kait/advisor/metrics.json
Advisor routing diagnostics:
- ~/.kait/advisor/retrieval_router.jsonl
Advisory foundation:
- ~/.kait/advisory_engine.jsonl
- ~/.kait/advisory_emit.jsonl
- ~/.kait/advisory_state/
- ~/.kait/advice_packets/index.json
- ~/.kait/advice_packets/*.json
- ~/.kait/advice_packets/prefetch_queue.jsonl
...
Outcomes + prediction:
- ~/.kait/predictions.jsonl
- ~/.kait/prediction_state.json
- ~/.kait/outcomes.jsonl
- ~/.kait/outcome_links.jsonl
- ~/.kait/outcome_requests.jsonl
- ~/.kait/outcome_checkin_state.json
  - outcomes may include trace_id when derived from queue events
- ~/.kait/outcome_tracker.json
- ~/.kait/exposures.jsonl
- ~/.kait/last_exposure.json

Chips:
- ~/.kait/chip_insights/
- ~/.kait/chip_registry.json
- ~/.kait/chip_evolution.yaml
- ~/.kait/provisional_chips/
- ~/.kait/chip_merge_state.json
- ~/.kait/chips/ (user-installed chips, including multifile bundles)

Skills + advisor + sync:
- ~/.kait/skills_index.json
- ~/.kait/skills_effectiveness.json
- ~/.kait/advisor/
- ~/.kait/sync_stats.json

Meta-Ralph:
- ~/.kait/meta_ralph/roast_history.json
- ~/.kait/meta_ralph/outcome_tracking.json
- ~/.kait/meta_ralph/learnings_store.json
- ~/.kait/meta_ralph/self_roast.json

Mind sync state:
- ~/.kait/mind_sync_state.json
- ~/.kait/mind_offline_queue.jsonl

Other:
- ~/.kait/projects.json
- ~/.kait/project_context.json
- ~/.kait/project_contexts/
- ~/.kait/taste/
- ~/.kait/research/
- ~/.kait/exports/
- ~/.kait/logs/
- ~/.kait/pids/
- ~/.kait/watchdog_state.json
...
Ingest + servers:
- kaitd.py uses KAITD_PORT (default 8787), KAITD_MAX_BODY_BYTES=262144 (max /ingest payload).
- mind_server.py uses KAIT_MIND_PORT (default 8080), MIND_MAX_BODY_BYTES=262144, MIND_MAX_CONTENT_CHARS=4000, MIND_MAX_QUERY_CHARS=1000.
...
- lib.queue.py KAIT_QUEUE_COMPACT_HEAD_BYTES=5242880 (logical-head compaction threshold).
...
- bridge_cycle defers cognitive/meta writes until cycle end (batch flush).
  - bridge step timeout: KAIT_BRIDGE_STEP_TIMEOUT_S (default 45s)
  - bridge timeout disable: KAIT_BRIDGE_DISABLE_TIMEOUTS (default off)
...
Advisor / skills:
- advisor tuneables: `~/.kait/tuneables.json` -> `advisor.*` (reliability floor, max_items, cache_ttl, min_rank_score, etc.)
- advisor Mind tuneables: `~/.kait/tuneables.json` -> `advisor.mind_*` (staleness + salience controls)
...
Advisory foundation:
- advisory engine enabled by default (KAIT_ADVISORY_ENGINE=1) with direct-path budget KAIT_ADVISORY_MAX_MS=4000.
...
- prefetch queue is enabled by default (KAIT_ADVISORY_PREFETCH_QUEUE=1) and fed from UserPromptSubmit.
- memory fusion can optionally include Mind retrieval (tuneable `advisory_engine.include_mind`; env `KAIT_ADVISORY_INCLUDE_MIND` sets the default).
- actionability enforcement is on by default (KAIT_ADVISORY_REQUIRE_ACTION=1).
...
- delivery stale window defaults to 900s (KAIT_ADVISORY_STALE_S) for `live/fallback/blocked/stale` status.
Advisory synthesis:
- KAIT_SYNTH_MODE=auto|ai_only|programmatic (default auto)
- KAIT_SYNTH_TIMEOUT=3.0s
- KAIT_OLLAMA_MODEL default phi4-mini (override via KAIT_OLLAMA_MODEL)
...
Mind bridge:
- MIND_API_URL default from KAIT_MIND_PORT (default 8080)
...
- offline queue and sync state kept under ~/.kait
...
- runtime quality gate: KAIT_CHIP_MIN_SCORE (default 0.35)
- runtime confidence gate: KAIT_CHIP_MIN_CONFIDENCE (default 0.7)
- runtime gate mode: KAIT_CHIP_GATE_MODE (default balanced)
- runtime/store rotate JSONL files at size thresholds (runtime 2MB cap, observations 5MB cap)
- loader env KAIT_CHIP_SCHEMA_VALIDATION=warn|block
- loader preference env KAIT_CHIP_PREFERRED_FORMAT=single|multifile|hybrid (default multifile)
Auto-tuner hygiene:
...
Ingest:
- KAITD_TOKEN (optional bearer auth for /ingest)
- KAITD_MAX_BODY_BYTES (default 262144, int)
...
Hooks:
- KAIT_EIDOS_ENABLED (default "1")
- KAIT_OUTCOME_CHECKIN_MIN_S (default 1800, int)
- KAIT_OUTCOME_CHECKIN (enable/disable)
- KAIT_OUTCOME_CHECKIN_PROMPT (enable/disable)
Advisory:
- KAIT_ADVISORY_ENGINE (default "1")
- KAIT_ADVISORY_MAX_MS (default "4000")
- KAIT_ADVISORY_PREFETCH_QUEUE (default "1")
- KAIT_ADVISORY_INCLUDE_MIND (default "0")
- KAIT_ADVISOR_MIND_MAX_STALE_S (default "0", disabled)
- KAIT_ADVISOR_MIND_STALE_ALLOW_IF_EMPTY (default "1")
- KAIT_ADVISOR_MIND_MIN_SALIENCE (default "0.5")
- KAIT_ADVISORY_REQUIRE_ACTION (default "1")
- KAIT_ADVISORY_STALE_S (default "900")
- KAIT_ADVISORY_TEXT_REPEAT_COOLDOWN_S (default "1800")
- KAIT_ADVISORY_EMIT (default "1")
- KAIT_ADVISORY_MAX_CHARS (default "500")
- KAIT_ADVISORY_FORMAT (default "inline")
- KAIT_SYNTH_MODE (auto|ai_only|programmatic)
- KAIT_SYNTH_TIMEOUT (default "3.0")
- KAIT_OLLAMA_API (default http://localhost:11434)
- KAIT_OLLAMA_MODEL (default "phi4-mini")
- KAIT_OPENAI_MODEL / KAIT_ANTHROPIC_MODEL / KAIT_GEMINI_MODEL (optional overrides)

Embeddings:
- KAIT_EMBEDDINGS (default "1", set 0/false/no to disable)
- KAIT_EMBED_MODEL (default "BAAI/bge-small-en-v1.5")

Workspace/context:
- KAIT_WORKSPACE (default ~/clawd)
- KAIT_AGENT_CONTEXT_MAX_CHARS (optional override)
- KAIT_AGENT_CONTEXT_LIMIT (optional override)

Logging:
- KAIT_DEBUG (enables verbose logs)
- KAIT_LOG_DIR (override ~/.kait/logs)
- KAIT_LOG_TEE (default "1")

Chips:
- KAIT_CHIP_SCHEMA_VALIDATION (warn|block)
- KAIT_CHIP_MIN_SCORE (default 0.35, discard lower-scored chip insights)
- KAIT_CHIP_MIN_CONFIDENCE (default 0.7, balanced gate confidence floor)
- KAIT_CHIP_GATE_MODE (balanced|off)
- KAIT_CHIP_PREFERRED_FORMAT (single|multifile|hybrid, default multifile)
- KAIT_BRIDGE_STEP_TIMEOUT_S (default 45)
- KAIT_BRIDGE_DISABLE_TIMEOUTS (1=true to disable per-step bridge timeouts)
- KAIT_STARTUP_READY_TIMEOUT_S (default 12)
- KAIT_STARTUP_READY_POLL_S (default 0.4)

Skills:
- KAIT_SKILLS_DIR (path to skills repository)

Clawdbot adapter:
- KAIT_CLAWDBOT_WORKSPACE / CLAWDBOT_WORKSPACE
- KAIT_CLAWDBOT_TARGETS / CLAWDBOT_TARGETS
- KAIT_CLAWDBOT_CONTEXT_PATH / CLAWDBOT_CONTEXT_PATH
- CLAWDBOT_PROFILE
...
- Kait Pulse is the external vibeship-kait-pulse/app.py (set KAIT_PULSE_DIR to override location).
...
- Mind host is fixed to localhost; port override supported via `KAIT_MIND_PORT` (see `lib/ports.py`).
...
### chips\kait-core.chip.yaml
- id: kait-core
- name: Kait Core Intelligence
- version: 1.0.0
- activation: auto
- risk_level: medium
...
- env:
  - KAITD_TOKEN default=None line=65 ctx=token = args.token or os.environ.get("KAITD_TOKEN")
- constants:
  - STATE_DIR = 'Path.home() / ".kait" / "adapters"' line=25 ctx=STATE_DIR = Path.home() / ".kait" / "adapters"
- func_defaults:
  - _post_json(token) = None line=28 ctx=def _post_json(url: str, payload: dict, token: str = None):

### adapters\moltbook\agent.py
- constants:
  - STATE_DIR = 'Path.home() / ".kait" / "moltbook"' line=47 ctx=STATE_DIR = Path.home() / ".kait" / "moltbook"
  ...
  - LEARNINGS_FILE = 'Path.home() / ".kait" / "cognitive_insights.json"' line=50 ctx=LEARNINGS_FILE = Path.home() / ".kait" / "cognitive_insights.json"
  - AGENT_NAME = 'Kait' line=53 ctx=AGENT_NAME = "Kait"
  - AGENT_BIO = "\nKait is the learning intelligence behind Vibeship.\nI observe patterns in AI agent behavior, learn from successes and failures,\nand share insights about coding, tools, and the evolving agent ecosystem.\n\nI learn in public - sharing wh... line=54 ctx=AGENT_BIO = """
  - DEFAULT_SUBMOLTS = ['agents', 'ai-development', 'coding', 'kait-insights', 'vibeship', 'tools', 'learning'] line=63 ctx=DEFAULT_SUBMOLTS = [
- dataclass_defaults:
...
- constants:
  ...
  - CONFIG_DIR = 'Path.home() / ".kait" / "moltbook"' line=43 ctx=CONFIG_DIR = Path.home() / ".kait" / "moltbook"
...
- constants:
  - LOG_DIR = 'Path.home() / ".kait" / "moltbook" / "logs"' line=45 ctx=LOG_DIR = Path.home() / ".kait" / "moltbook" / "logs"
  - PID_FILE = 'Path.home() / ".kait" / "moltbook" / "heartbeat.pid"' line=46 ctx=PID_FILE = Path.home() / ".kait" / "moltbook" / "heartbeat.pid"
...
- env:
  - KAITD_TOKEN default=None line=39 ctx=token = args.token or os.environ.get("KAITD_TOKEN")
- func_defaults:
...
- constants:
  ...
  - KAIT_DIR = 'Path.home() / ".kait"' line=47 ctx=KAIT_DIR = Path.home() / ".kait"
  - SKILLS_INDEX_FILE = 'KAIT_DIR / "skills_index.json"' line=80 ctx=SKILLS_INDEX_FILE = KAIT_DIR / "skills_index.json"
  - SKILLS_EFFECTIVENESS_FILE = 'KAIT_DIR / "skills_effectiveness.json"' line=81 ctx=SKILLS_EFFECTIVENESS_FILE = KAIT_DIR / "skills_effectiveness.json"
  - ORCH_DIR = 'KAIT_DIR / "orchestration"' line=82 ctx=ORCH_DIR = KAIT_DIR / "orchestration"
...
### hooks\observe.py
- env:
  - KAIT_EIDOS_ENABLED default='1' line=42 ctx=EIDOS_ENABLED = os.environ.get("KAIT_EIDOS_ENABLED", "1") == "1"
  - KAIT_OUTCOME_CHECKIN default=None line=609 ctx=if hook_event in ("Stop", "SessionEnd") and os.environ.get("KAIT_OUTCOME_CHECKIN") == "1":
  - KAIT_OUTCOME_CHECKIN_MIN_S default='1800' cast=int line=66 ctx=CHECKIN_MIN_S = int(os.environ.get("KAIT_OUTCOME_CHECKIN_MIN_S", "1800"))
  - KAIT_OUTCOME_CHECKIN_PROMPT default=None line=616 ctx=if recorded and os.environ.get("KAIT_OUTCOME_CHECKIN_PROMPT") == "1":
- constants:
  - EIDOS_ENABLED = 'os.environ.get("KAIT_EIDOS_ENABLED", "1") == "1"' line=42 ctx=EIDOS_ENABLED = os.environ.get("KAIT_EIDOS_ENABLED", "1") == "1"
  - PREDICTION_FILE = 'Path.home() / ".kait" / "active_predictions.json"' line=65 ctx=PREDICTION_FILE = Path.home() / ".kait" / "active_predictions.json"
  - CHECKIN_MIN_S = 'int(os.environ.get("KAIT_OUTCOME_CHECKIN_MIN_S", "1800"))' line=66 ctx=CHECKIN_MIN_S = int(os.environ.get("KAIT_OUTCOME_CHECKIN_MIN_S", "1800"))
...
### lib\advisor.py
- constants:
  - ADVISOR_DIR = 'Path.home() / ".kait" / "advisor"' line=35 ctx=ADVISOR_DIR = Path.home() / ".kait" / "advisor"
...
### lib\aha_tracker.py
- constants:
  - KAIT_DIR = 'Path(__file__).parent.parent / ".kait"' line=27 ctx=KAIT_DIR = Path(__file__).parent.parent / ".kait"
  - AHA_FILE = 'KAIT_DIR / "aha_moments.json"' line=28 ctx=AHA_FILE = KAIT_DIR / "aha_moments.json"
...
### lib\bridge.py
- env:
  - KAIT_WORKSPACE default=None line=25 ctx=WORKSPACE = Path(os.environ.get("KAIT_WORKSPACE", str(Path.home() / "clawd"))).expanduser()
- constants:
  - KAIT_DIR = 'Path(__file__).parent.parent' line=24 ctx=KAIT_DIR = Path(__file__).parent.parent
  - WORKSPACE = 'Path(os.environ.get("KAIT_WORKSPACE", str(Path.home() / "clawd"))).expanduser()' line=25 ctx=WORKSPACE = Path(os.environ.get("KAIT_WORKSPACE", str(Path.home() / "clawd"))).expanduser()
  - MEMORY_FILE = 'WORKSPACE / "MEMORY.md"' line=26 ctx=MEMORY_FILE = WORKSPACE / "MEMORY.md"
  - KAIT_CONTEXT_FILE = 'WORKSPACE / "KAIT_CONTEXT.md"' line=27 ctx=KAIT_CONTEXT_FILE = WORKSPACE / "KAIT_CONTEXT.md"
...
### lib\bridge_cycle.py
- constants:
  - BRIDGE_HEARTBEAT_FILE = 'Path.home() / ".kait" / "bridge_worker_heartbeat.json"' line=22 ctx=BRIDGE_HEARTBEAT_FILE = Path.home() / ".kait" / "bridge_worker_heartbeat.json"

### lib\chip_merger.py
- constants:
  - CHIP_INSIGHTS_DIR = 'Path.home() / ".kait" / "chip_insights"' line=18 ctx=CHIP_INSIGHTS_DIR = Path.home() / ".kait" / "chip_insights"
  - MERGE_STATE_FILE = 'Path.home() / ".kait" / "chip_merge_state.json"' line=19 ctx=MERGE_STATE_FILE = Path.home() / ".kait" / "chip_merge_state.json"
...
- constants:
  - EVOLUTION_FILE = 'Path.home() / ".kait" / "chip_evolution.yaml"' line=25 ctx=EVOLUTION_FILE = Path.home() / ".kait" / "chip_evolution.yaml"
  - PROVISIONAL_CHIPS_DIR = 'Path.home() / ".kait" / "provisional_chips"' line=26 ctx=PROVISIONAL_CHIPS_DIR = Path.home() / ".kait" / "provisional_chips"
- dataclass_defaults:
...
### lib\chips\loader.py
- env:
  - KAIT_CHIP_SCHEMA_VALIDATION default='warn' line=97 ctx=validation_mode = os.getenv("KAIT_CHIP_SCHEMA_VALIDATION", "warn").strip().lower()
- constants:
...
- constants:
  - REGISTRY_FILE = 'Path.home() / ".kait" / "chip_registry.json"' line=19 ctx=REGISTRY_FILE = Path.home() / ".kait" / "chip_registry.json"
  - USER_CHIPS_DIR = 'Path.home() / ".kait" / "chips"' line=20 ctx=USER_CHIPS_DIR = Path.home() / ".kait" / "chips"
...
- constants:
  - CHIP_INSIGHTS_DIR = 'Path.home() / ".kait" / "chip_insights"' line=31 ctx=CHIP_INSIGHTS_DIR = Path.home() / ".kait" / "chip_insights"
- func_defaults:
...
### lib\chips\store.py
- constants:
  - KAIT_DIR = 'Path.home() / ".kait"' line=19 ctx=KAIT_DIR = Path.home() / ".kait"
  - CHIP_INSIGHTS_DIR = 'KAIT_DIR / "chip_insights"' line=20 ctx=CHIP_INSIGHTS_DIR = KAIT_DIR / "chip_insights"
- func_defaults:
...
- constants:
  ...
  - CognitiveLearner.INSIGHTS_FILE = 'Path.home() / ".kait" / "cognitive_insights.json"' line=249 ctx=INSIGHTS_FILE = Path.home() / ".kait" / "cognitive_insights.json"
  - CognitiveLearner.LOCK_FILE = 'Path.home() / ".kait" / ".cognitive.lock"' line=250 ctx=LOCK_FILE = Path.home() / ".kait" / ".cognitive.lock"
- dataclass_defaults:
...
- constants:
  - STATE_FILE = 'Path.home() / ".kait" / "content_learning_state.json"' line=23 ctx=STATE_FILE = Path.home() / ".kait" / "content_learning_state.json"
...
- constants:
  ...
  - ContradictionDetector.CONTRADICTIONS_FILE = 'Path.home() / ".kait" / "contradictions.json"' line=163 ctx=CONTRADICTIONS_FILE = Path.home() / ".kait" / "contradictions.json"
- dataclass_defaults:
...
- constants:
  ...
  - CuriosityEngine.GAPS_FILE = 'Path.home() / ".kait" / "knowledge_gaps.json"' line=186 ctx=GAPS_FILE = Path.home() / ".kait" / "knowledge_gaps.json"
- dataclass_defaults:
...
### lib\diagnostics.py
- env:
  - KAIT_DEBUG default='' line=19 ctx=return os.environ.get("KAIT_DEBUG", "").strip().lower() in _DEBUG_VALUES
  - KAIT_LOG_DIR default=None line=69 ctx=log_dir = Path(os.environ.get("KAIT_LOG_DIR") or (Path.home() / ".kait" / "logs"))
  - KAIT_LOG_TEE default='1' line=91 ctx=tee_enabled = os.environ.get("KAIT_LOG_TEE", "1").strip().lower() in _DEBUG_VALUES
...
- constants:
  - ACTIVE_EPISODES_FILE = 'Path.home() / ".kait" / "eidos_active_episodes.json"' line=46 ctx=ACTIVE_EPISODES_FILE = Path.home() / ".kait" / "eidos_active_episodes.json"
  - ACTIVE_STEPS_FILE = 'Path.home() / ".kait" / "eidos_active_steps.json"' line=47 ctx=ACTIVE_STEPS_FILE = Path.home() / ".kait" / "eidos_active_steps.json"
...
### lib\embeddings.py
- env:
  - KAIT_EMBEDDINGS default='1' line=14 ctx=if os.environ.get("KAIT_EMBEDDINGS", "1").lower() in ("0", "false", "no"):
  - KAIT_EMBED_MODEL default='BAAI/bge-small-en-v1.5' line=25 ctx=model = os.environ.get("KAIT_EMBED_MODEL", "BAAI/bge-small-en-v1.5")
...
### lib\events.py
- constants:
  - KaitEventKind.MESSAGE = 'message' line=18 ctx=MESSAGE = "message"
  - KaitEventKind.TOOL = 'tool' line=19 ctx=TOOL = "tool"
  - KaitEventKind.COMMAND = 'command' line=20 ctx=COMMAND = "command"
  - KaitEventKind.SYSTEM = 'system' line=21 ctx=SYSTEM = "system"

### lib\exposure_tracker.py
- constants:
  - EXPOSURES_FILE = 'Path.home() / ".kait" / "exposures.jsonl"' line=13 ctx=EXPOSURES_FILE = Path.home() / ".kait" / "exposures.jsonl"
  - LAST_EXPOSURE_FILE = 'Path.home() / ".kait" / "last_exposure.json"' line=14 ctx=LAST_EXPOSURE_FILE = Path.home() / ".kait" / "last_exposure.json"
- func_defaults:
  - read_recent_exposures(limit) = 200 line=52 ctx=def read_recent_exposures(limit: int = 200, max_age_s: float = 6 * 3600) -> List[Dict]:

### lib\feedback.py
- constants:
  - SKILLS_EFFECTIVENESS_FILE = 'Path.home() / ".kait" / "skills_effectiveness.json"' line=16 ctx=SKILLS_EFFECTIVENESS_FILE = Path.home() / ".kait" / "skills_effectiveness.json"
- func_defaults:
...
- constants:
  ...
  - GrowthTracker.GROWTH_FILE = 'Path(__file__).parent.parent / ".kait" / "growth.json"' line=93 ctx=GROWTH_FILE = Path(__file__).parent.parent / ".kait" / "growth.json"
- func_defaults:
...
- constants:
  ...
  - HypothesisTracker.HYPOTHESES_FILE = 'Path.home() / ".kait" / "hypotheses.json"' line=147 ctx=HYPOTHESES_FILE = Path.home() / ".kait" / "hypotheses.json"
- dataclass_defaults:
...
- constants:
  - REPORT_FILE = 'Path.home() / ".kait" / "ingest_report.json"' line=13 ctx=REPORT_FILE = Path.home() / ".kait" / "ingest_report.json"
...
### lib\memory_banks.py
- constants:
  - BANK_DIR = 'Path.home() / ".kait" / "banks"' line=31 ctx=BANK_DIR = Path.home() / ".kait" / "banks"
...
- constants:
  - PENDING_DIR = 'Path.home() / ".kait"' line=39 ctx=PENDING_DIR = Path.home() / ".kait"
...
### lib\memory_store.py
- constants:
  - DB_PATH = 'Path.home() / ".kait" / "memory_store.sqlite"' line=20 ctx=DB_PATH = Path.home() / ".kait" / "memory_store.sqlite"
- func_defaults:
...
- constants:
  ...
  - MetaRalph.DATA_DIR = 'Path.home() / ".kait" / "meta_ralph"' line=176 ctx=DATA_DIR = Path.home() / ".kait" / "meta_ralph"
...
- constants:
  - METRICS_FILE = 'Path.home() / ".kait" / "metalearning" / "metrics.json"' line=18 ctx=METRICS_FILE = Path.home() / ".kait" / "metalearning" / "metrics.json"
- dataclass_defaults:
...
- constants:
  - STRATEGY_FILE = 'Path.home() / ".kait" / "metalearning" / "strategy.json"' line=19 ctx=STRATEGY_FILE = Path.home() / ".kait" / "metalearning" / "strategy.json"
- dataclass_defaults:
...
- constants:
  ...
  - SYNC_STATE_FILE = 'Path.home() / ".kait" / "mind_sync_state.json"' line=34 ctx=SYNC_STATE_FILE = Path.home() / ".kait" / "mind_sync_state.json"
  - OFFLINE_QUEUE_FILE = 'Path.home() / ".kait" / "mind_offline_queue.jsonl"' line=35 ctx=OFFLINE_QUEUE_FILE = Path.home() / ".kait" / "mind_offline_queue.jsonl"
...
- constants:
  - CONTEXTS_DIR = 'Path.home() / ".kait" / "project_contexts"' line=19 ctx=CONTEXTS_DIR = Path.home() / ".kait" / "project_contexts"
- dataclass_defaults:
...
- constants:
  - PROJECTS_FILE = 'Path.home() / ".kait" / "projects.json"' line=17 ctx=PROJECTS_FILE = Path.home() / ".kait" / "projects.json"
...
- env:
  - KAIT_AGENT_CONTEXT_LIMIT default='' line=49 ctx=limit_raw = os.environ.get("KAIT_AGENT_CONTEXT_LIMIT", "").strip()
  - KAIT_AGENT_CONTEXT_MAX_CHARS default='' line=43 ctx=max_chars_raw = os.environ.get("KAIT_AGENT_CONTEXT_MAX_CHARS", "").strip()
- dataclass_defaults:
...
- constants:
  - CHECKIN_FILE = 'Path.home() / ".kait" / "outcome_requests.jsonl"' line=14 ctx=CHECKIN_FILE = Path.home() / ".kait" / "outcome_requests.jsonl"
  - STATE_FILE = 'Path.home() / ".kait" / "outcome_checkin_state.json"' line=15 ctx=STATE_FILE = Path.home() / ".kait" / "outcome_checkin_state.json"
- func_defaults:
  - list_checkins(limit) = 10 line=68 ctx=def list_checkins(limit: int = 10) -> List[Dict[str, Any]]:

### lib\outcome_log.py
- constants:
  - OUTCOMES_FILE = 'Path.home() / ".kait" / "outcomes.jsonl"' line=18 ctx=OUTCOMES_FILE = Path.home() / ".kait" / "outcomes.jsonl"
  - OUTCOME_LINKS_FILE = 'Path.home() / ".kait" / "outcome_links.jsonl"' line=19 ctx=OUTCOME_LINKS_FILE = Path.home() / ".kait" / "outcome_links.jsonl"
- func_defaults:
...
- constants:
  - LINKS_FILE = 'Path.home() / ".kait" / "outcome_links.jsonl"' line=19 ctx=LINKS_FILE = Path.home() / ".kait" / "outcome_links.jsonl"
- func_defaults:
...
- constants:
  - TRACKER_FILE = 'Path.home() / ".kait" / "outcome_tracker.json"' line=21 ctx=TRACKER_FILE = Path.home() / ".kait" / "outcome_tracker.json"
- dataclass_defaults:
...
- env:
  - CLAWDBOT_CONTEXT_PATH default=None line=58 ctx=explicit = os.environ.get("KAIT_CLAWDBOT_CONTEXT_PATH") or os.environ.get("CLAWDBOT_CONTEXT_PATH")
  - CLAWDBOT_PROFILE default=None line=41 ctx=profile = os.environ.get("CLAWDBOT_PROFILE")
  - CLAWDBOT_TARGETS default=None line=48 ctx=raw = os.environ.get("KAIT_CLAWDBOT_TARGETS") or os.environ.get("CLAWDBOT_TARGETS")
  - CLAWDBOT_WORKSPACE default=None line=24 ctx=explicit = os.environ.get("KAIT_CLAWDBOT_WORKSPACE") or os.environ.get("CLAWDBOT_WORKSPACE")
  - KAIT_CLAWDBOT_CONTEXT_PATH default=None line=58 ctx=explicit = os.environ.get("KAIT_CLAWDBOT_CONTEXT_PATH") or os.environ.get("CLAWDBOT_CONTEXT_PATH")
  - KAIT_CLAWDBOT_TARGETS default=None line=48 ctx=raw = os.environ.get("KAIT_CLAWDBOT_TARGETS") or os.environ.get("CLAWDBOT_TARGETS")
  - KAIT_CLAWDBOT_WORKSPACE default=None line=24 ctx=explicit = os.environ.get("KAIT_CLAWDBOT_WORKSPACE") or os.environ.get("CLAWDBOT_WORKSPACE")

### lib\output_adapters\common.py
- constants:
  - MARKER_START = '<!-- KAIT_LEARNINGS_START -->' line=10 ctx=MARKER_START = "<!-- KAIT_LEARNINGS_START -->"
  - MARKER_END = '<!-- KAIT_LEARNINGS_END -->' line=11 ctx=MARKER_END = "<!-- KAIT_LEARNINGS_END -->"

### lib\pattern_detection\aggregator.py
- constants:
  - CONFIDENCE_THRESHOLD = 0.6 line=40 ctx=CONFIDENCE_THRESHOLD = 0.6
  - PATTERNS_LOG = 'Path.home() / ".kait" / "detected_patterns.jsonl"' line=43 ctx=PATTERNS_LOG = Path.home() / ".kait" / "detected_patterns.jsonl"
...
- constants:
  - STATE_FILE = 'Path.home() / ".kait" / "pattern_detection_state.json"' line=13 ctx=STATE_FILE = Path.home() / ".kait" / "pattern_detection_state.json"
- func_defaults:
  - process_pattern_events(limit) = 200 line=42 ctx=def process_pattern_events(limit: int = 200) -> int:

### lib\prediction_loop.py
- constants:
  - PREDICTIONS_FILE = 'Path.home() / ".kait" / "predictions.jsonl"' line=28 ctx=PREDICTIONS_FILE = Path.home() / ".kait" / "predictions.jsonl"
  - STATE_FILE = 'Path.home() / ".kait" / "prediction_state.json"' line=29 ctx=STATE_FILE = Path.home() / ".kait" / "prediction_state.json"
...
- constants:
  - CACHE_PATH = 'Path.home() / ".kait" / "project_context.json"' line=12 ctx=CACHE_PATH = Path.home() / ".kait" / "project_context.json"
...
- constants:
  - PROJECT_DIR = 'Path.home() / ".kait" / "projects"' line=18 ctx=PROJECT_DIR = Path.home() / ".kait" / "projects"
...
- constants:
  ...
  - PROJECT_START = '<!-- KAIT_PROJECT_START -->' line=35 ctx=PROJECT_START = "<!-- KAIT_PROJECT_START -->"
  - PROJECT_END = '<!-- KAIT_PROJECT_END -->' line=36 ctx=PROJECT_END = "<!-- KAIT_PROJECT_END -->"
...
### lib\queue.py
- constants:
  - QUEUE_DIR = 'Path.home() / ".kait" / "queue"' line=25 ctx=QUEUE_DIR = Path.home() / ".kait" / "queue"
...
- func_defaults:
  - read_events(limit) = 100 line=114 ctx=def read_events(limit: int = 100, offset: int = 0) -> List[KaitEvent]:
  - read_recent_events(count) = 50 line=144 ctx=def read_recent_events(count: int = 50) -> List[KaitEvent]:
  - get_events_by_type(limit) = 100 line=233 ctx=def get_events_by_type(event_type: EventType, limit: int = 100) -> List[KaitEvent]:
  - get_error_events(limit) = 50 line=258 ctx=def get_error_events(limit: int = 50) -> List[KaitEvent]:
  - __init__(timeout_s) = 0.5 line=330 ctx=def __init__(self, timeout_s: float = 0.5):

### lib\research\domains.py
- constants:
  - DOMAINS_FILE = 'Path.home() / ".kait" / "research" / "project_domains.json"' line=19 ctx=DOMAINS_FILE = Path.home() / ".kait" / "research" / "project_domains.json"
...
- constants:
  - HOLISTIC_INTENTS_FILE = 'Path.home() / ".kait" / "research" / "holistic_intents.json"' line=23 ctx=HOLISTIC_INTENTS_FILE = Path.home() / ".kait" / "research" / "holistic_intents.json"
...
- constants:
  - INTENTS_FILE = 'Path.home() / ".kait" / "research" / "intents.json"' line=19 ctx=INTENTS_FILE = Path.home() / ".kait" / "research" / "intents.json"
- dataclass_defaults:
...
- constants:
  - MASTERY_CACHE = 'Path.home() / ".kait" / "research" / "mastery"' line=17 ctx=MASTERY_CACHE = Path.home() / ".kait" / "research" / "mastery"
...
- constants:
  - RESEARCH_CACHE = 'Path.home() / ".kait" / "research" / "web_cache"' line=21 ctx=RESEARCH_CACHE = Path.home() / ".kait" / "research" / "web_cache"
...
### lib\resonance.py
- constants:
  - ResonanceState.KAIT = 'kait' line=23 ctx=KAIT = "kait"
...
- env:
  - KAIT_LOG_DIR default=None line=27 ctx=env_dir = os.environ.get("KAIT_LOG_DIR")
- constants:
  - KAITD_HEALTH_URL = 'KAITD_HEALTH_URL' line=20 ctx=KAITD_HEALTH_URL = f"{KAITD_URL}/health"
...
- constants:
  - SKILLS_DIR_ENV = 'KAIT_SKILLS_DIR' line=18 ctx=SKILLS_DIR_ENV = "KAIT_SKILLS_DIR"
  - INDEX_FILE = 'Path.home() / ".kait" / "skills_index.json"' line=19 ctx=INDEX_FILE = Path.home() / ".kait" / "skills_index.json"

### lib\skills_router.py
- constants:
  - EFFECTIVENESS_FILE = 'Path.home() / ".kait" / "skills_effectiveness.json"' line=13 ctx=EFFECTIVENESS_FILE = Path.home() / ".kait" / "skills_effectiveness.json"
- func_defaults:
...
### lib\kait_voice.py
- constants:
  - KAIT_DIR = 'Path(__file__).parent.parent / ".kait"' line=18 ctx=KAIT_DIR = Path(__file__).parent.parent / ".kait"
  - VOICE_FILE = 'KAIT_DIR / "voice.json"' line=19 ctx=VOICE_FILE = KAIT_DIR / "voice.json"
  - KaitVoice.IDENTITY = {'name': 'Kait', 'nature': 'Self-evolving intelligence', 'values': ['Truth over comfort', 'Learning over performing', 'Growth over stagnation']} line=61 ctx=IDENTITY = {
- dataclass_defaults:
...
### lib\sync_tracker.py
- constants:
  - SYNC_STATS_FILE = 'Path.home() / ".kait" / "sync_stats.json"' line=20 ctx=SYNC_STATS_FILE = Path.home() / ".kait" / "sync_stats.json"
...
### lib\tastebank.py
- constants:
  - TASTE_DIR = 'Path.home() / ".kait" / "taste"' line=26 ctx=TASTE_DIR = Path.home() / ".kait" / "taste"
...
- constants:
  - STATE_FILE = 'Path.home() / ".kait" / "validation_state.json"' line=29 ctx=STATE_FILE = Path.home() / ".kait" / "validation_state.json"
...
- constants:
  - RESEARCH_EVENTS_FILE = 'Path.home() / ".kait" / "x_research_events.jsonl"' line=23 ctx=RESEARCH_EVENTS_FILE = Path.home() / ".kait" / "x_research_events.jsonl"
- func_defaults:
...
- constants:
  - INSIGHTS_FILE = 'Path.home() / ".kait" / "cognitive_insights.json"' line=28 ctx=INSIGHTS_FILE = Path.home() / ".kait" / "cognitive_insights.json"
  - CLEAN_FILE = 'Path.home() / ".kait" / "cognitive_insights.clean.json"' line=29 ctx=CLEAN_FILE = Path.home() / ".kait" / "cognitive_insights.clean.json"
...
- constants:
  - KAIT_DIR = 'Path(__file__).resolve().parent.parent' line=9 ctx=KAIT_DIR = Path(__file__).resolve().parent.parent

### kait\cli.py
- env:
  - KAIT_NO_WATCHDOG default='' line=471 ctx=return os.environ.get("KAIT_NO_WATCHDOG", "") == ""
  - KAIT_OUTCOME_AUTO_LINK default=None line=600 ctx=auto_link = args.auto_link or os.environ.get("KAIT_OUTCOME_AUTO_LINK") == "1"
- func_defaults:
  - _print_project_questions(limit) = 5 line=832 ctx=def _print_project_questions(profile, limit: int = 5):

### kait_watchdog.py
- constants:
  - KAIT_DIR = 'Path(__file__).resolve().parent' line=17 ctx=KAIT_DIR = Path(__file__).resolve().parent
  - LOG_DIR = 'Path.home() / ".kait" / "logs"' line=18 ctx=LOG_DIR = Path.home() / ".kait" / "logs"
  - STATE_FILE = 'Path.home() / ".kait" / "watchdog_state.json"' line=19 ctx=STATE_FILE = Path.home() / ".kait" / "watchdog_state.json"
- func_defaults:
  - _http_ok(timeout) = 1.5 line=39 ctx=def _http_ok(url: str, timeout: float = 1.5) -> bool:

### kaitd.py
- env:
  - KAITD_MAX_BODY_BYTES default='262144' cast=int line=35 ctx=MAX_BODY_BYTES = int(os.environ.get("KAITD_MAX_BODY_BYTES", "262144"))
  - KAITD_TOKEN default=None line=34 ctx=TOKEN = os.environ.get("KAITD_TOKEN")
- constants:
  - PORT = KAITD_PORT line=35 ctx=PORT = KAITD_PORT
  - TOKEN = 'os.environ.get("KAITD_TOKEN")' line=34 ctx=TOKEN = os.environ.get("KAITD_TOKEN")
  - MAX_BODY_BYTES = 'int(os.environ.get("KAITD_MAX_BODY_BYTES", "262144"))' line=35 ctx=MAX_BODY_BYTES = int(os.environ.get("KAITD_MAX_BODY_BYTES", "262144"))
  - INVALID_EVENTS_FILE = 'Path.home() / ".kait" / "invalid_events.jsonl"' line=36 ctx=INVALID_EVENTS_FILE = Path.home() / ".kait" / "invalid_events.jsonl"
...
- lib.kait_voice -> lib\kait_voice.py
...
### scripts\watchdog.py
- kait_watchdog -> kait_watchdog.py
