# Kait Chips: Pluggable Intelligence for Every Domain

> *"Chips are to Kait what apps are to a smartphone — they teach it how to think about your world."*
...
- `planned/vision`:
  ...
  - some reference examples and sidekick docs may live in archived docs or external repos

## Executive Summary

Kait Chips transform Kait from a monolithic learning system into an **open platform for domain-specific intelligence**. Instead of Kait knowing only how to learn from code and tools, chips teach Kait how to learn from *anything* — marketing campaigns, sales deals, factory operations, team dynamics, financial decisions, and beyond.

**The Problem Today:**
- Kait learns one way about everything (tool patterns, user preferences)
...
**The Chips Solution:**
- Pluggable modules that teach Kait *how to think* about a domain
...
A **Chip** is a YAML specification that teaches Kait how to be intelligent about a specific domain.

Think of it like this:
- A smartphone is powerful, but apps make it *useful*
- Kait is the learning engine, but chips make it *intelligent about your world*
...
| Kait Component | How Chips Integrate |
|-----------------|---------------------|
...
```yaml
# ~/.kait/chips/marketing.chip.yaml
...
chip:
  ...
  version: 1.0.0
  description: |
    Makes Kait intelligent about marketing outcomes.
...
# ============================================================
context:
  # What to include in KAIT_CONTEXT.md / CLAUDE.md
  format: |
...
**Value:** The team stops guessing. Kait tells them what's actually working based on their specific audience.
...
**Scenario:** A solo professional wants Kait to learn their personal productivity patterns.
...
│   │
│   ├── official/             # Bundled with Kait
│   │   ├── core.chip.yaml    # Default Kait learning
...
- Require KaitEventV1 fields (v, source, kind, ts, session_id, payload).
...
### 6) CLI and packaging (kait-chips repo)
- `kait chips validate` to lint spec and check permissions.
- `kait chips run` and `kait chips replay` for local testing.
- Chips can be installed from file, URL, or registry.

### Repo boundary note
- Kait core stays stable and minimal.
- The chips runtime lives in the `vibeship-kait-chips` repo.
- Kait core depends on a thin compatibility shim (loader + router hooks).
...
# Install a chip
kait chips install marketing.chip.yaml
kait chips install https://kait-hub.io/chips/sales-b2b

# Activate/deactivate
kait chips activate marketing
kait chips deactivate marketing

# Check chip status
kait chips status marketing

# View chip insights
kait chips insights marketing --limit 10

# Trigger chip evolution
kait chips evolve marketing

# Export chip with learned data
kait chips export marketing --include-insights

# Create new chip from template
kait chips create my-chip --template business
```

### Chip Python API

```python
from kait.chips import ChipLoader, ChipRunner

# Load a chip
loader = ChipLoader()
marketing_chip = loader.load("~/.kait/chips/marketing.chip.yaml")
...
| Term | Definition |
|------|------------|
| **Chip** | A YAML specification that teaches Kait how to learn about a domain |
...
A: Chips are declarative specifications. You describe *what* to learn, not *how* to learn it. Kait handles the learning mechanics. This makes chips accessible to non-programmers and consistent across domains.
...
A: Yes. Chips work at the Kait layer, which is LLM-agnostic. The insights chips generate can enhance any AI assistant (Claude, GPT, Gemini, local models, etc.).
...
*Kait Chips: Teaching AI how to think about your world.*
