...
This is the support readiness pack for Kait Intelligence (public alpha).
...
3. Collect required info:
   - OS + Python version
   - `kait health` output
   - `kait services` output
   - relevant logs from `~/.kait/logs`
4. Assign owner and label:
   - `area:kaitd`, `area:hooks`, `area:eidos`, `area:docs`, `area:installer`
5. Close loop:
