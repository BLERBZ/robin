...
This document explains how project-level knowledge flows into Kait's learning,
...
`~/.kait/projects/<project_key>.json`
...
## Flow overview
1) **Questioning**
   - `kait project init` seeds domain questions.
   - `kait project questions` shows unanswered + dynamic questions.
   ...
   - If a reference exists without a transfer, Kait asks for a transfer principle.

2) **Capture**
   - `kait project capture` records insights, milestones, done criteria, etc.
...
6) **Promotion**
   - `kait promote` writes a `PROJECT.md` block from the project profile.
...
## Example
```bash
kait project init --domain game_dev
kait project answer game_core_loop --text "Core loop is satisfying at 60% grab success."
kait project capture --type insight --text "Grip strength vs weight balance matters"
kait project capture --type done --text "Game feels complete when core loop is satisfying"
```
