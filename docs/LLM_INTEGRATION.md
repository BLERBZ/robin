...
Kait Intelligence uses Claude (via Claude Code CLI with OAuth) for:
...
1. **Write** prompt to `~/.kait/llm_prompt.txt`
...
3. **Read** response from `~/.kait/llm_response.txt` with `utf-8-sig` encoding (handles BOM from PowerShell's `Set-Content`)
...
1. **Advisory synthesis** — triggers when `≥5 patterns detected` OR `≥2 insights merged`
   ...
   - Writes output to `KAIT_ADVISORY.md` in `~/.kait/` and OpenClaw workspace
...
2. **EIDOS distillation** — triggers every **10th productive cycle**
   ...
   - Appends to `~/.kait/eidos_distillations.jsonl`

### Rate Limiting

- **30 calls/hour** (tracked in `~/.kait/llm_calls.json`)
...
|----------|---------|-------------|
| `KAIT_EMBEDDINGS` | `"1"` | Set to `"0"` to disable fastembed (REQUIRED for bridge_worker — prevents 8GB+ memory leak) |
...
### Rate limited
Check `~/.kait/llm_calls.json` — if >30 entries in the last hour, wait or increase `_MAX_CALLS_PER_HOUR` in `lib/llm.py`.
