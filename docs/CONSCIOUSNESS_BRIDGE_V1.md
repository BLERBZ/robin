...
## Purpose
Define how **kait-intel** ingests emotional context from **vibeship-kait-consciousness** in a controlled, fallback-safe way.
...
2. **Secondary fallback:** Local `KaitEmotions().decision_hooks()` from `lib.kait_emotions`.
3. **Final fallback:** neutral defaults in `advisory_synthesizer` (`balanced`, `medium`, `grounded_warm`, no forced clarifier).

Pulse sidekick HTTP state (`lib.soul_upgrade`) remains allowed as contextual metadata (mission/mood preface), but it is **not authoritative** for bridge strategy override.
