...
Goal: make Kait Intelligence understandable in 30 seconds and believable in 2 minutes.

## 30-Second Positioning

Kait Intelligence is a self-evolution layer for AI coding agents.
...
Subhead:
- "Kait Intelligence turns real work into validated learnings, then reuses them so your agent stops repeating the same mistakes."
...
- Screenshot: Kait Pulse dashboard
- Screenshot: `kait status` output
...
- Show: `kait up` (or `start_kait.bat`)

25-40s: Health + visibility
- Show: `kait health` and `/status`
...
- Mention: `KAIT_EIDOS_ENFORCE_BLOCK=1` exists for strict hosts

80-90s: CTA
- "Install, start services, code normally. Kait learns in the background."
...
- `kait health` output (green)
- `kait services` output (shows ports + running components)
- Pulse dashboard landing view
- Meta-Ralph telemetry view (if public-safe)
- A “before/after” context file change (`KAIT_CONTEXT.md` or equivalent)
