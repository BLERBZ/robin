# Kait Intelligence Launch Scope + Gates
...
### Core value
- Local-first self-evolution layer for AI coding agents (Kait + EIDOS) that improves through validated learnings.
- New user can install, start services, and see the system working within 10 minutes.

### Operator surface area
- Service runner: `start_kait.bat` (Windows) + `kait up/down/services` (CLI).
- Observability: Kait Pulse (web), Obsidian Observatory (file-based), CLI scripts.
- Health endpoints: `kaitd /health` and Mind `/health` (when enabled).
...
- Cloud sync / multi-tenant hosted Kait
...
- Large-scale multi-user realtime collaboration features (presence/chat/rooms) beyond what’s required for Kait’s own dashboards
...
- Start services.
- Verify health:
  - `http://127.0.0.1:${KAITD_PORT:-8787}/health` returns 200 (liveness, plain `ok`).
  - `http://127.0.0.1:${KAITD_PORT:-8787}/status` returns 200 with JSON (readiness + pipeline fields).
  - If Mind enabled, `http://127.0.0.1:${KAIT_MIND_PORT:-8080}/health` returns 200.
...
6. `python -m pytest -q tests/test_production_loop_gates.py tests/test_advisor_effectiveness.py tests/test_kaitd_hardening.py`
...
PASS only if:
- Mutating `kaitd` endpoints require bearer auth by default (`KAITD_TOKEN` or `~/.kait/kaitd.token`).
...
- `KAIT_EIDOS_ENFORCE_BLOCK=1` behavior is documented and works end-to-end.
