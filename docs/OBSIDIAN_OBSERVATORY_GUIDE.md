...
The Observatory transforms Kait Intelligence's internal state files into a human-readable Obsidian vault, showing the full 12-stage intelligence pipeline — from event capture through quality gates through cognitive learning through EIDOS through advisory to promotion.
...
- Python 3.10+
- Kait Intelligence repository cloned
...
This creates ~460+ markdown files in your vault directory (default: `~/Documents/Obsidian Vault/Kait-Intelligence-Observatory`).

### Open in Obsidian

1. Open Obsidian
2. **Open folder as vault** — select `Kait-Intelligence-Observatory`
3. Navigate to `_observatory/flow.md` — this is your entry point

### Auto-Sync

When Kait's pipeline is running, the observatory auto-refreshes every 120 seconds (configurable). No manual regeneration needed during normal operation.
...
5. **Source Files** — links to the Python module and `~/.kait/` state files
...
|---------|-----------|---------------|-------------|
| **Cognitive Insights** | `~/.kait/cognitive_insights.json` | Yes (per insight) | Key, category, reliability, validations, evidence |
| **EIDOS Distillations** | `~/.kait/eidos.db` | Yes (per distillation) | Statement, type, confidence, domains, triggers |
| **EIDOS Episodes** | `~/.kait/eidos.db` | Yes (per episode + steps) | Goal, outcome, phase, prediction vs evaluation |
| **Meta-Ralph Verdicts** | `~/.kait/meta_ralph/roast_history.json` | Yes (per verdict) | Score breakdown, input text, issues found |
| **Promotion Log** | `~/.kait/promotion_log.jsonl` | No (index table) | Target distribution, recent activity |
| **Advisory** | `~/.kait/advisor/effectiveness.json` | No (index table) | Source effectiveness, follow rate, recent advice |
| **Retrieval Routing** | `~/.kait/advisor/retrieval_router.jsonl` | No (index table) | Route distribution, reasons, complexity scores |
| **Tuneable Evolution** | `~/.kait/auto_tune_log.jsonl` | No (index table) | Parameter changes, impact analysis (before/after follow rate) |
| **Advisory Decisions** | `~/.kait/advisory_decision_ledger.jsonl` | No (index table) | Emit/suppress/block decisions, suppression reasons, source counts |
| **Implicit Feedback** | `~/.kait/advisor/implicit_feedback.jsonl` | No (index table) | Followed/ignored signals, per-tool follow rates, source effectiveness |
...
The observatory automatically regenerates when Kait's bridge cycle runs, subject to a cooldown. This is configured in `lib/bridge_cycle.py` and requires no manual setup.
...
| File | Purpose |
|------|---------|
| `~/.kait/tuneables.json` | Runtime config (takes priority) |
...
{
  "observatory": {
    "vault_dir": "D:\\MyVaults\\KaitObservatory"
  }
}
```
...
WHERE type = "kait-cognitive-insight" AND reliability >= 0.9
...
WHERE type = "kait-eidos-episode" AND outcome = "success"
SORT started DESC
LIMIT 15
```
````
...
WHERE type = "kait-metaralph-verdict" AND total_score >= 7
...
FROM "/_observatory/explore/cognitive"
WHERE type = "kait-cognitive-insight" AND promoted = true
SORT reliability DESC
```
````
...
WHERE type = "kait-eidos-distillation"
...
FROM "/_observatory/explore/episodes"
WHERE type = "kait-eidos-episode" AND outcome = "failure"
SORT started DESC
```
````
...
The observatory reads from `~/.kait/` state files. If these don't exist yet:
1. Run Kait's pipeline at least once to generate state files
...
3. If persistent, check for stale `.cognitive.lock` files in `~/.kait/`
