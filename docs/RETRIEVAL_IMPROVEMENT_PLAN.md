...
Kait advisory delivers ~40% noise: research tweets, DEPTH training logs, user quotes, and generic placeholders surface as tool advice because the insight pool (256 items) is flat — no metadata filtering by tool type, no actionability scoring, no memory tiering. Semantic similarity alone can't distinguish "actionable directive" from "interesting observation."
