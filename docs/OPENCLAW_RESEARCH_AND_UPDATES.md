...
- **Changes made:**
  ...
  - `kait_watchdog.py`
    ...
    - `kaitd` restart/recovery remains active in plugin-only mode.
...
- **Changes made:**
  - `lib/context_sync.py`
    ...
    - added runtime controls: `KAIT_SYNC_MODE`, `KAIT_SYNC_TARGETS`, `KAIT_SYNC_DISABLE_TARGETS`
    - added tuneables section support: `sync.mode`, `sync.adapters_enabled`, `sync.adapters_disabled`
  - `lib/advisory_engine.py`
    - packet no-emit fallback is now opt-in (`KAIT_ADVISORY_PACKET_FALLBACK_EMIT`, `advisory_engine.packet_fallback_emit_enabled`)
...
- **Validation result:** better
  - advisory-focused and regression tests passed in kait-intelligence
...
- **Goal:** close remaining broad Kait cleanup items (B1-B6) with measurable churn reduction.
- **Changes made:**
  - `lib/auto_tuner.py`
...
- **Changes made:**
  - `lib/bridge_cycle.py`
  - Added `_prune_redundant_advisory(...)` before writing `KAIT_ADVISORY.md`
...
- **Changes made:**
  ...
  - Added `KAIT_CHIP_EVENT_ACTIVE_LIMIT` (default 6)
...
- **Changes made (cron behavior text):**
  - Job: `kait-context-refresh` (`56a7f5be-e734-47a7-a526-73c3dc9bde1a`)
...
- **Changes made:**
  - `hooks/observe.py`
  - Added `KAIT_OBSERVE_PRETOOL_BUDGET_MS` (default `2500ms`).
...
- **Changes made:**
  ...
  - Canonicalized char-budget var to `KAIT_AGENT_CONTEXT_MAX_CHARS`.
  - Added backward-compatible alias: `KAIT_AGENT_CONTEXT_LIMIT` (same semantic: max chars).
  - Introduced `KAIT_AGENT_CONTEXT_ITEM_LIMIT` for compact-context item count.
