...
Kait's Opportunity Scanner generates self-improvement opportunities while you work.
...
- Guardrailed: respects Kait consciousness kernel checks (non-harm, service, clarity).
...
- `kait_global`: improves Kait itself across all work
...
- `KAIT_OPPORTUNITY_LLM_COOLDOWN_S=900` (15 minutes)
...
All artifacts live under `~/.kait/opportunity_scanner/`:
...
```bash
python -m kait.cli opportunities accept <id-prefix> --note "why we accept this"
```

Dismiss one (reduces repeats for dismissed question keys for a TTL window):
```bash
python -m kait.cli opportunities dismiss <id-prefix> --note "why we dismiss this"
```
