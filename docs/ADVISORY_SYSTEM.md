...
6. Emit (advisory_emitter.py)
   Print to stdout: "[KAIT] advice text"
  |
  v
...
Tracks session context across hook invocations via JSON files in `~/.kait/advisory_state/`.
...
**7 retrieval sources:**
1. **Cognitive insights** (`cognitive_learner.py`) — Kait's learned patterns
...
| WARNING | >= 0.80 | `[KAIT ADVISORY] text` |
| NOTE | >= 0.42 | `[KAIT] text` |
| WHISPER | >= 0.30 | `(kait: text)` (if enabled) |
...
Caches advisory packets for faster subsequent delivery. Stored at `~/.kait/advice_packets/`.
...
All tunable via `~/.kait/tuneables.json` (sections: `advisor`, `advisory_engine`, `advisory_gate`, `synthesizer`).
...
|----------|---------|---------|
| `KAIT_ADVISORY_ENGINE` | 1 | Enable/disable engine |
| `KAIT_ADVISORY_EMIT_WHISPERS` | 1 | Surface whisper-level advice |
| `KAIT_ADVISORY_GLOBAL_DEDUPE` | 1 | Cross-session dedupe |
| `KAIT_ADVISORY_GLOBAL_DEDUPE_COOLDOWN_S` | 600 | Dedupe window |
...
2. Set Kait export target:
   - `python scripts/set_obsidian_watchtower.py --vault-dir "C:\\Users\\USER\\Documents\\Obsidian Vault\\Kait-Intelligence-Observatory" --enable --auto-export --write`
...
- `.\Kait-Intelligence-Observatory\watchtower.md` updates with:
  - summary
...
- `.\Kait-Intelligence-Observatory\packets\index.md` remains the packet catalog.
...
| File | Content | Cap |
|------|---------|-----|
| `~/.kait/advisory_engine.jsonl` | Engine events (retrieve, gate, emit, errors) | 500 lines |
| `~/.kait/advisory_emit.jsonl` | What was emitted to stdout | 500 lines |
| `~/.kait/outcomes.jsonl` | Outcome tracker rows (linked to trace IDs/insights) | 5000 lines |
| `~/.kait/outcome_links.jsonl` | Outcome-to-insight linkage rows | 5000 lines |
| `~/.kait/advisor/implicit_feedback.jsonl` | Implicit outcome tracker rows for advisory post-tool signals | 5000 lines |
| `~/.kait/advisor/retrieval_router.jsonl` | Which sources hit per query | 800 lines |
| `~/.kait/advisor/advice_log.jsonl` | All advice retrieval | 500 lines |
| `~/.kait/advisor/recent_advice.jsonl` | Last 20min deliveries | 200 lines |
| `~/.kait/advisory_global_dedupe.jsonl` | Cross-session dedupe log | 5000 lines |
| `~/.kait/advisory_state/*.json` | Per-session state files | 2h TTL |
| `~/.kait/advice_packets/index.json` | Packet registry | 2000 packets |
