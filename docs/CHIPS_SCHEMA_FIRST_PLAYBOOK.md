...
Kait OSS default keeps chip runtime disabled. Set `KAIT_CHIPS_ENABLED=1`
...
python scripts/run_chip_schema_experiments.py \
  ...
  --chips kait-core,marketing,game-dev \
  --events-per-chip 24 \
...
Policy file:
- `~/.kait/chip_observer_policy.json`

After apply:
- restart `kaitd` and `bridge_worker` so runtime loads updated observer blocklist.

## Migration Plan

1. Promoted runtime profile (`R3` controlled rollout):
- `KAIT_CHIP_REQUIRE_LEARNING_SCHEMA=1`
- `KAIT_CHIP_OBSERVER_ONLY=1`
- `KAIT_CHIP_MIN_LEARNING_EVIDENCE=2`
- `KAIT_CHIP_MIN_CONFIDENCE=0.65`
- `KAIT_CHIP_MIN_SCORE=0.25`
- `KAIT_CHIP_MERGE_MIN_CONFIDENCE=0.65`
- `KAIT_CHIP_MERGE_MIN_QUALITY=0.62`
