...
This doc maps each benchmark to the *existing* Kait Intelligence harnesses/logs where possible, and calls out gaps where we still need a dedicated test.
...
| 3 | Timing / Interruption Quality | Advice arrives on the correct tool boundary, respects cooldowns, and does not derail flow | Partial: cooldown + dedupe in `lib/advisory_gate.py` / engine logs; needs a direct timing bench | `~/.kait/advisory_engine.jsonl` + (gap: a timing-only benchmark) |
| 4 | Learning Capture Quality (Write) | New learnings are distilled (not raw telemetry), deduped, non-contradictory, and scoped | `tests/test_cognitive_capture.py`, `tests/test_cognitive_validation_hygiene.py` | `~/.kait/cognitive_insights.json`, `~/.kait/meta_ralph/*` |
...
| 6 | Contextual Gating / Scope Control | Learnings apply only in-scope; similar-but-wrong cases do not trigger | `tests/test_advisory_intent_taxonomy.py`, `tests/test_advisory_gate_config.py`, realism suppress cases | realism reports + `~/.kait/advisory_engine.jsonl` |
...
| 10 | End-to-End Outcomes (Online) | Improves completion rate/time/error recovery without annoyance | `tests/test_learning_utilization.py`, `tests/test_advisor_effectiveness.py`, dashboard | `~/.kait/advisor/effectiveness.json`, dashboard endpoints |
...
- Artifact: `~/.kait/meta_ralph/outcome_tracking.json`
...
- Artifacts:
  ...
  - `~/.kait/advisory_global_dedupe.jsonl`
- Tuneables:
  - `KAIT_ADVISORY_GLOBAL_DEDUPE=0|1`
  - `KAIT_ADVISORY_GLOBAL_DEDUPE_BY_TEXT=0|1` (default 1, suppress repeated advice even if `advice_id` churns)
  - `KAIT_ADVISORY_GLOBAL_DEDUPE_COOLDOWN_S=600` (default 10 minutes)
...
- Global dedupe: engine emits `global_dedupe_suppressed` when repeats are blocked; log at `~/.kait/advisory_global_dedupe.jsonl`.
...
2. Spot-check that packets created before advice-id stabilization do not reintroduce churn:
   ...
   - Inspect `~/.kait/advisor/recent_advice.jsonl` for repeated `insight_key` with changing `advice_id`.
...
3. Check outcome application is flowing back to cognitive reliability:
   - verify `~/.kait/meta_ralph/outcome_tracking.json` records include `insight_key` and `source=cognitive` for cognitive items
   - verify `~/.kait/cognitive_insights.json` reliability/validations are not fragmenting due to old random advice IDs
...
- Advisory engine event log: `~/.kait/advisory_engine.jsonl`
- Retrieval routing telemetry: `~/.kait/advisor/retrieval_router.jsonl`
- Recent advice feed: `~/.kait/advisor/recent_advice.jsonl`
- Advisor effectiveness: `~/.kait/advisor/effectiveness.json`
- Meta-Ralph outcome tracking: `~/.kait/meta_ralph/outcome_tracking.json`
