...
- `kaitd` now enforces token auth on all mutating `POST` endpoints by default (token from `KAITD_TOKEN` or `~/.kait/kaitd.token`).
- `kaitd` now applies per-IP rate limiting and bounded invalid-event quarantine retention.
