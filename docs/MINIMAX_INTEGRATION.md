...
> How Kait Intelligence uses MiniMax M2.5, and how to use it as a subagent in any test or pipeline.
...
msg = client.messages.create(
    model="MiniMax-M2.5",
    max_tokens=1500,
    messages=[{"role": "user", "content": "Hello from Kait"}],
)
...
MiniMax M2.5 is integrated into 4 Kait subsystems via the **OpenAI-compatible** chat completions API:
...
|----------|-----|-----|----------|
| **OpenAI-compat** | `https://api.minimax.io/v1/chat/completions` | Raw HTTP / OpenAI SDK | All current Kait integrations |
...
|----------|---------|-------------|
| `KAIT_MINIMAX_BASE_URL` | `https://api.minimax.io/v1` | OpenAI-compat base URL |
| `KAIT_MINIMAX_MODEL` | `MiniMax-M2.5` | Model ID |
| `KAIT_SYNTH_PREFERRED_PROVIDER` | `""` | Set to `minimax` to prioritize |
| `KAIT_SYNTH_TIMEOUT` | `8.0` | Advisory synthesizer timeout (seconds) |
| `AUTO_SCORER_MINIMAX_TIMEOUT_S` | `15` | Effect evaluator timeout (seconds) |
| `KAIT_OPPORTUNITY_LLM_PROVIDER` | `""` | Set to `minimax` for opportunity scanning |
| `KAIT_OPPORTUNITY_LLM_TIMEOUT_S` | `12` | Opportunity scanner timeout (seconds) |
...
KAIT_MINIMAX_BASE_URL=https://api.minimax.io/v1
KAIT_MINIMAX_MODEL=MiniMax-M2.5
KAIT_SYNTH_PREFERRED_PROVIDER=minimax
...
KAIT_OPPORTUNITY_LLM_ENABLED=1
KAIT_OPPORTUNITY_LLM_PROVIDER=minimax
```
...
**Provider chain position:** 4th (after Ollama, Gemini, then MiniMax). Override with `KAIT_SYNTH_PREFERRED_PROVIDER=minimax` to make it primary.
...
THINK_RE = re.compile(r"<think>.*?</think>", re.DOTALL | re.IGNORECASE)
API_KEY = os.environ["MINIMAX_API_KEY"]
BASE_URL = os.getenv("KAIT_MINIMAX_BASE_URL", "https://api.minimax.io/v1").rstrip("/")
MODEL = os.getenv("KAIT_MINIMAX_MODEL", "MiniMax-M2.5")
...
**Cause:** `~/.kait/tuneables.json` may have `force_programmatic_synth: true` or `ai_timeout_s` set too low.

**Fix:** Check tuneables:
```bash
python -c "
import json, pathlib
t = pathlib.Path.home() / '.kait' / 'tuneables.json'
