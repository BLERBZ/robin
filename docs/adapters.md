# Kait Adapters (Input Integrations)
Navigation hub: `docs/GLOSSARY.md`

Kait is **runtime-agnostic**.

Kait Core only understands **KaitEventV1** (see `lib/events.py`).
Everything else is an **adapter** that converts some platform’s logs/events into Kait events.
...
`kaitd` enforces bearer auth on mutating `POST` endpoints by default.
Adapters resolve tokens in this order: `--token`, `KAITD_TOKEN`, then `~/.kait/kaitd.token`.
...
- Reads newline-delimited JSON KaitEventV1 objects from stdin.
- Posts to `kaitd /ingest`.

This lets any tool (Cursor/VSCode tasks, shell scripts, CI) feed Kait
...
Notes:
- `adapters/stdin_ingest.py` defaults to `KAITD_URL` or `KAITD_PORT`.
...
Practical pattern:
- Put a small script in your repo that emits KaitEventV1 when you run a task.
...
Options:
- Copy/paste “high signal” learnings manually into `kait learn ...`
- Export conversation and run an offline importer that emits KaitEventV1 events.

---

## Design rule
Adapters are allowed to be messy.
**Kait core must stay clean and stable.**
