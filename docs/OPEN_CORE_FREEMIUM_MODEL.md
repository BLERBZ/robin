...
Goal: keep Kait's core open source for growth and auditability, while offering premium modules (chips, benchmark systems like Forge/DEPTH, curated packs) without weakening safety.
...
- social-tooling surfaces (social networks/research surfaces), kept opt-in behind `KAIT_PREMIUM_TOOLS=1`.
...
- This repo: `vibeship-kait` OSS core (MIT; premium modules remain separate).
- Private packages:
  - `vibeship-kait-chips-premium`
  - `vibeship-kait-forge` (or a hosted Forge API)
  - `vibeship-kait-depth` (often best as hosted due to datasets/controls)
