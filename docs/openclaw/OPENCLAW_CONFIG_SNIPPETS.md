...
```json
{
  "plugins": {
    "load": {
      "paths": [
        "/path/to/kait-intel\\extensions\\openclaw-kait-telemetry"
      ]
    },
    "entries": {
      "kait-telemetry-hooks": {
        "enabled": true,
        "config": {
          "spoolFile": "<USER_HOME>\\.kait\\openclaw_hook_events.jsonl",
...
```powershell
python adapters\openclaw_tailer.py --agent main --hook-events-file <KAIT_HOME>\openclaw_hook_events.jsonl
```
