...
Maintain an auditable backlog for Kait x OpenClaw integration changes, with:
...
- [x] Wire `llm_input`/`llm_output` hook ingestion path to Kait telemetry.
...
3. Runtime health
- [x] `kait-health-alert-watch` cron runs cleanly every hour.
