...
Use this prompt in OpenClaw when you want an honest live benchmark of Kait Intelligence (learning -> advisory -> action/outcome), not just unit tests.

## Prompt (paste as-is)

```text
Run a real-time end-to-end Kait Intelligence benchmark in this environment.
...
2) Run this command first and use its JSON/MD outputs as the base evidence:
   python scripts/openclaw_realtime_e2e_benchmark.py --window-minutes 90 --run-canary --canary-agent kait-speed --settle-seconds 12
3) Then validate these points explicitly from files/metrics:
   - Hook telemetry is live (`llm_input` and `llm_output`) and ingested to Kait queue.
   ...
   - Kait context/advisory files are present in active OpenClaw workspace(s).
