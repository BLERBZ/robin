...
This log tracks Kait x OpenClaw integration changes that should be easy to audit later.
...
- Added plugin-based hook telemetry capture:
  - `extensions/openclaw-kait-telemetry/`
  - captures `llm_input` + `llm_output` to redacted local spool JSONL.
- Added hook spool ingestion to Kait adapter:
