# AGI Scientist Matrix (Carmack-Style Mapping To Kait)

Date: 2026-02-15
Repo: vibeforge1111/kait-intel
Goal: A "who / what / contribution / how it maps / what to steal" matrix for AGI-adjacent researchers,
written to be usable for Kait engineering decisions (bounded, measurable, low-complexity).

This is not a popularity list. It is a set of lenses that match Kait's actual surfaces:
...
How Kait connects:
- Kait already has a fast path (packet lookup) and a slow path (live advisor retrieval).
- Kait already has budgets (timeouts, rate caps, max_queries) but must keep them "default-on".
- The advisory system is the policy surface Carmack would demand be bounded and measurable.

Where Kait differs:
- Kait is a runtime system around an external LLM/tooling interface, not an end-to-end trained agent.
...
Where Kait differs:
- Kait does not learn its embedding space end-to-end from outcomes
...
How Kait connects:
- Kait is not training a foundation model, but it is still optimizing policies in a noisy environment
...
How Kait connects:
- Kait can treat advice as a prediction about what will work next; outcomes validate or refute it
...
How Kait connects:
- Kait's advice evaluation should avoid naive correlation (some advice appears helpful only because context differed)
...
How Kait connects:
- Kait is a production system; tail failures matter more than average wins
...
## 10) Kait-Specific Translation: The Only Questions That Matter

When reading any AGI research or scientist commentary, translate it into:
- Which Kait surface does this affect: routing, memory, advisory, outcomes, or stability?
