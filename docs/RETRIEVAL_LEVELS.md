...
This defines a 3-level operating model for Kait/OpenClaw memory retrieval with the new auto-router in `lib/advisor.py`.
...
- Route decisions are logged to `~/.kait/advisor/retrieval_router.jsonl`.

Controls:
- Env:
  - `KAIT_RETRIEVAL_LEVEL=1|2|3`
  - `KAIT_RETRIEVAL_MODE=auto|embeddings_only|hybrid_agentic`
- Tuneables: `~/.kait/tuneables.json` -> `retrieval.level` + `retrieval.overrides.*` (canonical).
...
Setup:
- Kait embeddings backend: `tfidf` (default) or `fastembed` only if RAM allows.
- Retrieval mode: `auto` with conservative escalation.
- OpenClaw memorySearch: `local` provider (or `off` if you only want Kait-side memory for now).
...
1. Pick a level (`1/2/3`) and apply `KAIT_RETRIEVAL_LEVEL`.
2. Confirm route logs are being written (`~/.kait/advisor/retrieval_router.jsonl`).
