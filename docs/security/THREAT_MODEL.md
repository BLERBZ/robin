# Kait Intelligence Threat Model (Public Alpha)

Date: 2026-02-15
Scope: vibeforge1111/kait-intel (local-first services + hooks + dashboards)

This threat model is intentionally practical. It focuses on likely failures and abuse paths for a public alpha.

## System Summary

Kait Intelligence is a local-first learning/evolution layer for coding agents. It:
- Captures local agent/tool events (hooks + tailers)
- Stores/derives learnings (memory, distillation, advisories)
- Exposes local services (kaitd, dashboards, optional Mind bridge)
...
- Service availability:
  - kaitd, bridge workers, dashboards
...
- Malicious user (local):
  - disables guardrails and uses Kait as a wrapper for automation they already intended
...
- Boundary B: Local HTTP APIs (kaitd/dashboards)
...
- Require token auth for mutating endpoints by default (`KAITD_TOKEN` or `~/.kait/kaitd.token`).
...
- Optional strict enforcement:
  - `KAIT_EIDOS_ENFORCE_BLOCK=1` makes blocked decisions exit non-zero.
...
Risk: users run Kait in higher autonomy modes without understanding limits.
Mitigations:
...
- `tests/test_safety_guardrails.py` and `tests/test_kaitd_hardening.py` pass
