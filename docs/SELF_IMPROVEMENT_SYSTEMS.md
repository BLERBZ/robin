# Kait Intelligence: Self-Improvement Systems
...
**Purpose**: Close the feedback loops that turn Kait from a one-shot learning system into a continuously self-improving intelligence
...
### What Kait Intelligence Is Trying to Achieve

Kait Intelligence is a self-evolving AI learning system. Its core promise is that every interaction makes the next one better — not through brute-force data accumulation, but through **structured recursive improvement loops**.
...
Before these 10 systems were built, Kait had most of the individual components (Meta-Ralph quality gate, cognitive learner, advisor, hypothesis tracker, etc.) but **the loops between them were broken**. Specifically:
...
Each of these 10 systems closes one gap. Together, they form a recursive improvement loop where Kait continuously observes its own performance, adjusts its parameters, improves its knowledge quality, and measures the results.
...
The `AutoTuner` class in `lib/auto_tuner.py` can measure system health, compute parameter recommendations, and apply changes to `~/.kait/tuneables.json`. But its `enabled` flag defaulted to `False`, and nothing ever called it. This script wraps the AutoTuner with a **safety harness** that makes it safe to run autonomously.
...
2. **Snapshot**: Copies current `tuneables.json` to `~/.kait/tuneable_rollbacks/pre_tune_YYYYMMDDTHHMMSS.json` (keeps last 10)
...
Turns Kait's parameters from static constants into a self-adjusting system. If advice isn't being followed, the system can lower thresholds to be less strict. If noise is creeping in, it can tighten filters. All changes are verified against retrieval quality and rolled back if harmful.
...
**Depends on**: `~/.kait/advisor/effectiveness.json`
...
4. **Effectiveness update**: Writes to `~/.kait/advisor/effectiveness.json` with per-source follow/unhelpful counts
...
- **Writes to**: `~/.kait/advisor/effectiveness.json` (per-source stats), `~/.kait/advisor/implicit_feedback.jsonl` (event log)
...
**Depends on**: `~/.kait/cognitive_insights.json`
...
1. **`sweep()`**: Reads all cognitive insights, classifies each one, writes tier metadata (`_tier` field) back into the insight, saves tier state to `~/.kait/memory_tiers.json`
...
| File | Purpose |
|------|---------|
| `~/.kait/memory_tiers.json` | Tier state (tier assignments, last sweep timestamp, counts) |
| `~/.kait/memory_tier_log.jsonl` | Sweep history log |
...
Retrieval precision improves because the advisor no longer wastes slots on stale/contradicted insights. The 139 pinned insights represent Kait's most reliable knowledge and always get priority.
...
**Depends on**: `~/.kait/aha_moments.json`, `lib/depth_trainer.py` (optional), `lib/cognitive_learner.py`

### What It Does

The Aha tracker captures "surprise moments" — domains where Kait's predictions were significantly wrong (high confidence_gap). But these surprises were never fed back into training. This bridge:
...
Creates a direct feedback path from "where Kait is wrong" to "targeted improvement." Instead of training randomly, Kait focuses on its actual weak spots. The consolidated lessons from surprise moments become cognitive insights that improve future advisory.
...
| File | Purpose |
|------|---------|
| `~/.kait/retrieval_metrics_history.jsonl` | Full measurement history (capped at 365 entries) |
...
**Depends on**: `~/.kait/cognitive_insights.json`, CLAUDE.md, AGENTS.md, TOOLS.md, SOUL.md
...
1. **Scans target files** for `<!-- KAIT_LEARNINGS_START -->` sections
...
6. **Logs all demotions** to `~/.kait/demotion_log.jsonl`
...
Transforms Kait's pattern detection from passive observation to active hypothesis testing. Instead of just storing observations, the system forms predictions, tests them against reality, and promotes the ones that prove reliable. Invalidated hypotheses generate warnings that prevent the same mistakes.
...
**Depends on**: `lib/meta_ralph.py`, `lib/cognitive_learner.py`, `~/.kait/advisor/effectiveness.json`
...
| File | Purpose |
|------|---------|
| `~/.kait/domain_evolution_state.json` | Per-domain state (follow rates, last evolved) |
| `~/.kait/domain_evolution_log.jsonl` | Evolution events log |
...
Creates a unified self-improvement loop across all domains, not just X. The gap diagnosis tells you exactly where Kait's knowledge is thin or ineffective. Cross-domain pattern detection enables transferring successful strategies from one domain to another.

### Current Status

**Operational.** 6 domains analyzed, 6 evolution events detected, 5 gaps identified. The gaps are expected — Kait's learning has been concentrated in code/general domains. As the system accumulates more domain-specific data, the evolution engine will detect and act on trends.
...
| File | Purpose |
|------|---------|
| `~/.kait/self_improvement_benchmark.jsonl` | Benchmark history (timestamp, pass/fail, per-stage latency) |
...
### How the 10 Systems Connect to Existing Kait Architecture

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         EXISTING KAIT ARCHITECTURE                         │
│                                                                             │
...
### Auto-Tuner (`~/.kait/tuneables.json` → `auto_tuner` section)
...
1. **Mapped Kait's self-improvement landscape**: Identified 17+ existing components and 14+ gaps in the recursive improvement loop
...
5. **Audit trails**: Every system logs its actions to JSONL files in `~/.kait/` for inspection.
