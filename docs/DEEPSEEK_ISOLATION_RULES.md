...
FORBIDDEN:
  ...
  - Any Kait advisory, cognitive, or memory operations
...
No system prompt reveals what Kait is, what Vibeship is, what the training loop does, or why this question is being asked. DeepSeek sees a generic evaluation question and answers it. That's all.
...
def handle_deepseek_response(raw_response: str) -> str:
    """
    Process DeepSeek answer before it enters the Kait pipeline.
    """
...
- Used to modify Kait configuration
- Fed back into DeepSeek as context for future calls
- Stored with any metadata linking them to Kait internals
...
DEPTH_ANSWER_HEADERS:
  ...
  # No User-Agent strings mentioning Kait/Vibeship
...
DEEPSEEK_LOG_PATH = "~/.kait/logs/deepseek_calls.jsonl"
...
# - Any Kait context or scores
# - Session IDs that link to Kait internals
```
...
DeepSeek V3.2 (answer generation):
  ...
  Returns: Text answer
  Knows:   Nothing about Kait, Vibeship, Seedify, or training loop
...
- [ ] DeepSeek prompt contains NO Kait/Vibeship references
