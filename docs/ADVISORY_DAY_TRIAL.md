...
This runbook gives you a no-dashboard, CLI-only loop to validate Kait advisory quality in real usage for one day.
...
```bash
python scripts/advisory_day_trial.py start --trial-id kait_day1
```
...
```bash
python scripts/advisory_day_trial.py snapshot --trial-id kait_day1
```
...
```bash
python scripts/advisory_day_trial.py close --trial-id kait_day1
```
