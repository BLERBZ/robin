...
This document defines how Kait's prediction/outcome loop works and how to plug
...
Kait should not only store insights. It should test them against outcomes.
...
1. `build_predictions()`
- reads recent exposures from `~/.kait/exposures.jsonl`
- creates prediction rows in `~/.kait/predictions.jsonl`
- enforces source budgets and total budget to avoid one source dominating.

2. `build_project_predictions()`
- adds project-done and project-milestone predictions from `~/.kait/projects/*.json`.

3. `collect_outcomes()`
- consumes queue events and appends outcomes to `~/.kait/outcomes.jsonl`.
- captures:
...
Stored in `~/.kait/predictions.jsonl`.
...
6. External scorer outcomes (Kait Forge, Opus/Codex evaluators)
...
now = time.time()
append_outcome({
    "outcome_id": make_outcome_id("kait-forge", "run-123", "codex"),
...
- `python -m kait.cli outcome-stats`
- `python -m kait.cli outcome-unlinked --limit 20`
- `python -m kait.cli auto-link --min-similarity 0.25 --limit 80`
- `python -m kait.cli eval --days 7`

Manual explicit outcome:
- `python -m kait.cli outcome --result yes --text "worked" --auto-link`

## Prediction Loop Tuneables

Prediction generation budgets:
- `KAIT_PREDICTION_TOTAL_BUDGET`
- `KAIT_PREDICTION_DEFAULT_SOURCE_BUDGET`
- `KAIT_PREDICTION_SOURCE_BUDGETS`
  - format example: `chip_merge=80,kait_inject=60,sync_context=40`

Auto-link scheduling:
- `KAIT_PREDICTION_AUTO_LINK`
- `KAIT_PREDICTION_AUTO_LINK_INTERVAL_S`
- `KAIT_PREDICTION_AUTO_LINK_LIMIT`
- `KAIT_PREDICTION_AUTO_LINK_MIN_SIM`

Namespace control:
- `KAIT_NAMESPACE=prod|test`

## KPI Definitions (Prod-Only)

Exposed in `kait status` and `kait outcome-stats`.
...
## Integration Checklist (Kait Intelligence + Kait Forge)
...
5. Run `kait auto-link` on a schedule if unlinked outcomes grow.
6. Monitor `kait outcome-stats` KPIs weekly and adjust budgets/tuneables.
