...
This system is a foundational advisory layer for the full Kait session lifecycle, not only coding tool prompts.
...
4. Learning Integrator (feedback spine)
   - binds advice -> action -> outcome -> memory updates across Kait stores.
...
Hot-path advisory remains concise, but background packets can include task-plane context so Kait steers toward the correct end result for that plane.
...
## 6.7 Memory Fusion Contract (Kait-wide context bundle)

Advisory consumes a normalized memory bundle so it can use existing Kait intelligence instead of isolated prompt text.
...
4. chip insights (`~/.kait/chip_insights/*.jsonl`, `lib/chip_merger.py`)
5. outcomes and linkers (`lib/outcome_log.py`, `lib/outcomes/*`)
6. orchestration state (`lib/orchestration.py`, `~/.kait/orchestration/*` if present)
...
- Local: Ollama (`KAIT_OLLAMA_MODEL`, `KAIT_OLLAMA_API`)
...
- `KAIT_ADVISORY_PROVIDER_MODE=local|oauth|hybrid`
...
To ensure any model (local or OAuth) can pick up Kait context reliably, synthesis calls should use a strict structured payload.
...
- `KAIT_ADVISORY_FOUNDATION=1`
- `KAIT_ADVISORY_DIRECT_PATH=1`
- `KAIT_ADVISORY_BACKGROUND_PATH=1`
- `KAIT_ADVISORY_TASK_PLANES=build_delivery,team_management,orchestration_execution,research_decision`
- `KAIT_ADVISORY_GOAL_MAX_ACTIVE=3`
- `KAIT_ADVISORY_GOTCHA_ENABLE=1`
- `KAIT_ADVISORY_PREFETCH_MAX_JOBS=2`
- `KAIT_ADVISORY_PREFETCH_TOOL_MIN_PROB=0.70`
- `KAIT_ADVISORY_PROVIDER_MODE=hybrid`
- `KAIT_ADVISORY_MEMORY_REQUIRED=1`
...
1. add lightweight feedback endpoint/event in kaitd/pulse path.
...
- write to `~/.kait/advisory_provider_audit.jsonl`.
...
- `KAIT_ADVISORY_TEAM_CONTEXT_ENABLE=1`
- `KAIT_ADVISORY_ORCH_GRAPH_ENABLE=1`
- `KAIT_ADVISORY_OUTCOME_SCOPE_ENABLE=1`
- `KAIT_ADVISORY_CONFLICT_RESOLVER_ENABLE=1`
- `KAIT_ADVISORY_FEEDBACK_ENABLE=1`
- `KAIT_ADVISORY_BENCH_PLANES=build_delivery,team_management,orchestration_execution,research_decision`
- `KAIT_ADVISORY_PROVIDER_AUDIT_ENABLE=1`
- `KAIT_ADVISORY_PROGRAM_GOALS_ENABLE=1`
...
This blueprint can fit Kait well and produce meaningful gains if implemented with strict gates.  
