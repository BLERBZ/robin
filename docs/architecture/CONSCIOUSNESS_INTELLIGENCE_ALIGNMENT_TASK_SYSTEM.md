...
- `<REPO_ROOT>`
- `<OPENCLAW_HOME>\workspace-kait-speed`

Focus is architecture-doc coherence plus runtime contract wiring for Kait Consciousness and Kait Intelligence.
...
- HTTP sidekick contract: `lib/soul_upgrade.py`, `docs/04-integration-contracts.md`.

Impact:

- Unclear source-of-truth between `bridge.v1` file payload and pulse sidekick API payload.

### F3: Emotion state path is inconsistent with wider runtime conventions

Evidence:

- Most runtime modules write/read `~/.kait/*`.
- `lib/kait_emotions.py` defaults to repo-local `.kait/emotion_state.json`.
- Research brief explicitly notes missing deployed `<KAIT_HOME>\emotion_state.json` in `docs/kait-consciousness/emotion-memory-unity-research.md`.
...
- `<OPENCLAW_HOME>\workspace-kait-speed\docs\04-integration-contracts.md` has malformed markdown and stale schema shape.
- `<OPENCLAW_HOME>\workspace-kait-speed\KAIT_ADVISORY.md` is a transient session advisory snapshot, not a stable architecture contract.
- `docs/KAIT_CHIPS_ARCHITECTURE.md` references `docs/CHIP_VIBECODING.md` which does not exist in active docs.
...
- [x] `A1` Define canonical precedence: `bridge.v1` file contract vs pulse sidekick API contract, and document fallback order.
- [x] `A2` Repair `workspace-kait-speed/docs/04-integration-contracts.md` formatting and align fields to current contract reality (or mark as legacy).
- [x] `A3` Reclassify `workspace-kait-speed/KAIT_ADVISORY.md` as runtime snapshot output (not architecture spec), with pointer to canonical architecture docs.
...
- [x] `C1` Align `lib/kait_emotions.py` default persistence path with `~/.kait/emotion_state.json` (with env override for tests/dev).
- [x] `C2` Add one-time migration logic from repo-local `.kait/emotion_state.json` if present.
- [x] `C3` Update docs to match final path and lifecycle behavior.

Definition of Done:

- Emotion state is persisted in the same runtime surface used by other Kait modules.
