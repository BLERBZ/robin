...
Goal: make Kait advisory memory/share behavior consistently distill → transform → deliver usable advice in the right context.
...
9. [x] **Task 9 — Emit machine-readable advisory artifact with sync writes**
   ...
   - Goal: every sync target gets a deterministic JSON artifact (`KAIT_ADVISORY_PAYLOAD.json`) with advisory-readiness, reliability, and source metadata.
   - Done: payload emitted to adapters as `KAIT_ADVISORY_PAYLOAD.json` alongside text context.
...
12. [x] **Task 12 — Add advisory quarantine sink for dropped/suppressed rows**
   ...
   - Done: drops are written as compact rows to `~/.kait/advisory_quarantine/advisory_quarantine.jsonl` with `source`, `stage`, `reason`, and bounded `text_snippet`.
