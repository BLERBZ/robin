# Trend Intake → Build Queue Flow (Kait Scheduler + kait-x-builder + OpenClaw)

This file defines the contract for the **daily trend pipeline** after the latest automation changes.

- `kait-x-builder` produces trend intelligence and build candidates.
- `kait_scheduler` converts that output into a routed build queue for OpenClaw/Clawdbot.
...
- `kait-x-builder/scripts/daily_trend_research.py`
  - Produces a JSON payload (`TrendBuilderResult`) with trending topics + build candidates.
- `kait_scheduler.task_daily_research`
  - Reads the payload from child process output.
  - Persists handoff payload to `~/.kait/claw_integration/latest_trend_handoff.json`.
  - Generates queue manifest to `~/.openclaw/workspace/kait_build_queue/latest_build_queue.json`.

---

## 2) Artifact locations

- **Handoff file:** `~/.kait/claw_integration/latest_trend_handoff.json`
- **Queue manifest:** `~/.openclaw/workspace/kait_build_queue/latest_build_queue.json`
- **Queue snapshots:** `~/.openclaw/workspace/kait_build_queue/trend_build_queue_<RUN_ID>.json`
- **Dispatch log:** `~/.kait/claw_integration/build_dispatch_log.jsonl`
...
Inside scheduler (`kait_scheduler.py`) a candidate is queued only if:
...
Inside `kait-x-builder/scripts/daily_trend_research.py`:
...
- Persist result to:
  - `.../kait_build_queue/latest_build_queue.json`
