#!/usr/bin/env python3
"""kaitd - Kait daemon (platform-agnostic ingest)

Minimal HTTP server:
  GET  /health
  GET  /status
  POST /ingest  (KaitEventV1 JSON)

Stores events into the existing Kait queue (events.jsonl) so the rest of Kait
can process them.
...
from lib.events import KaitEventV1, validate_event_dict
...
from lib.ports import KAITD_PORT
...
TOKEN = os.environ.get("KAITD_TOKEN")
MAX_BODY_BYTES = int(os.environ.get("KAITD_MAX_BODY_BYTES", "262144"))
INVALID_EVENTS_FILE = Path.home() / ".kait" / "invalid_events.jsonl"
TUNEABLES_FILE = Path.home() / ".kait" / "tuneables.json"
RATE_LIMIT_PER_MIN = int(os.environ.get("KAITD_RATE_LIMIT_PER_MIN", "240"))
RATE_LIMIT_WINDOW_S = int(os.environ.get("KAITD_RATE_LIMIT_WINDOW_S", "60"))
INVALID_EVENTS_MAX_LINES = int(os.environ.get("KAITD_INVALID_EVENTS_MAX_LINES", "2000"))
INVALID_EVENTS_MAX_PAYLOAD_CHARS = int(os.environ.get("KAITD_INVALID_EVENTS_MAX_PAYLOAD_CHARS", "4000"))
...
def _resolve_token() -> str:
    if env_token := (os.environ.get("KAITD_TOKEN")):
    ...
    if lock_file.exists():
        try:
            ...
            if existing_pid != pid and _pid_is_alive(existing_pid):
                print(f"[KAIT] {name} already running with pid {existing_pid}; exiting duplicate instance")
                return None
        except Exception:
            pass
...
def _load_openclaw_runtime_config(*, force: bool = False) -> dict:
    ...
    env_advisory = os.environ.get("KAITD_OPENCLAW_ADVISORY_BRIDGE_ENABLED") or os.environ.get("KAITD_OPENCLAW_ADVISORY_BRIDGE_ENABLED")
    ...
    env_emotion = os.environ.get("KAITD_OPENCLAW_EMOTION_UPDATES_ENABLED") or os.environ.get("KAITD_OPENCLAW_EMOTION_UPDATES_ENABLED")
    ...
    env_intensity = os.environ.get("KAITD_OPENCLAW_EMOTION_TRIGGER_INTENSITY") or os.environ.get("KAITD_OPENCLAW_EMOTION_TRIGGER_INTENSITY")
    if env_intensity is not None:
        cfg["emotion_trigger_intensity"] = _safe_float(
            env_intensity,
            cfg["emotion_trigger_intensity"],
        )
    env_async = os.environ.get("KAITD_OPENCLAW_BRIDGE_ASYNC_ENABLED") or os.environ.get("KAITD_OPENCLAW_BRIDGE_ASYNC_ENABLED")
...
def _resolve_queue_event_type(evt: KaitEventV1) -> EventType:
...
def _emotion_register_trigger(trigger: str, *, intensity: float = 0.7, note: str = "") -> None:
    from lib.kait_emotions import KaitEmotions

    KaitEmotions().register_trigger(trigger, intensity=float(intensity), note=note)


def _emotion_recover() -> None:
    from lib.kait_emotions import KaitEmotions

    KaitEmotions().recover()


def _maybe_bridge_openclaw_runtime(evt: KaitEventV1, event_type: EventType) -> None:
...
def _dispatch_openclaw_runtime_bridge(evt: KaitEventV1, event_type: EventType) -> None:
    source = str(getattr(evt, "source", "") or "").strip().lower()
    if source != "openclaw":
        return
    cfg = _load_openclaw_runtime_config()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        allow_remote = (os.environ.get('KAITD_ALLOW_REMOTE_POST') or os.environ.get('KAITD_ALLOW_REMOTE_POST') or '').strip().lower() in {'1','true','yes','on'}
        ...
        try:
            evt = KaitEventV1.from_dict(data)
        except Exception as e:
...
def main():
    setup_component_logging("kaitd")
    lock_file = _acquire_single_instance_lock("kaitd")
    if lock_file is None:
        return

    print(f"kaitd listening on http://127.0.0.1:{PORT}")
