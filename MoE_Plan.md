# MoE Plan for Kait (Local-First Intelligence)

This document captures the proposed Mixture-of-Experts (MoE)-inspired routing design for Kait's local LLM upgrade, plus the retrieval pipeline flow. It is intentionally implementation-ready while keeping the system optional so Kait continues to work with minimal or no local AI by default.

## Goals
- Preserve Kait's current rules-based intelligence as the default.
...
## Kait Intelligence Features (Summary)
...
The router is a thin layer between Kait features and the local LLM runtime (Ollama). It decides:
- which model to use
