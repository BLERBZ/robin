<p align="center">
  <a href="https://kait.vibeship.co"><img src="header.png" alt="Kait Intelligence" width="100%"></a>
</p>
<p align="center">
  <a href="https://github.com/vibeforge1111/kait-intel/blob/main/LICENSE"><img src="https://img.shields.io/badge/license-MIT-blue?style=flat-square" alt="License"></a>
...
Runs 100% on your machine as a local AI sidekick that turns past work into future-ready behavior.
It is designed to be beyond a learning loop.

`You do work` -> `Kait captures memory` -> `Kait distills and transforms it` -> `Kait delivers advisory context` -> `You act with better context` -> `Outcomes re-enter the loop`

## What is Kait?

Kait Intelligence is a self-evolving AI sidekick designed to grow smarter through use.
...
```bash
python -m pip install kait-intel[services]
python -m kait.cli up
```
...
# View what Kait has learned
python -m kait.cli learnings
```

Windows: run `start_kait.bat` from the repo root.
...
- **Self-evolving sidekick behavior** — adapts from your sessions instead of staying static.
...
- **Observability surfaces** — Obsidian Observatory in-repo, plus optional external Kait Pulse and local Meta-Ralph views.
- **CLI** — `kait status`, `kait learnings`, `kait promote`, `kait up/down`, and more.
...
Kait ships with an [Obsidian](https://obsidian.md) integration that turns the entire intelligence pipeline into a human-readable vault you can browse, search, and query — every insight, every decision, every quality verdict, visible in one place.
...
```bash
# From the kait-intelligence repo:
python scripts/generate_observatory.py --force --verbose
```

This reads your `~/.kait/` state files and generates ~465+ markdown pages in under 1 second.

**Default vault location:** `~/Documents/Obsidian Vault/Kait-Intelligence-Observatory`

To change it, edit `observatory.vault_dir` in `~/.kait/tuneables.json` or `config/tuneables.json`.

### Open the Vault

1. In Obsidian: **File > Open vault > Open folder as vault**
2. Select `Kait-Intelligence-Observatory`
...
When Kait's pipeline is running, the observatory auto-refreshes every 120 seconds. No manual regeneration needed. To regenerate manually:
...
Edit `~/.kait/tuneables.json` (runtime) or `config/tuneables.json` (version-controlled).
...
- **Website**: [kait.vibeship.co](https://kait.vibeship.co)
