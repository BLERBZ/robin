#!/usr/bin/env bash
# Kait bootstrap for macOS/Linux
# Clone (if needed) -> create venv -> install -> start services

set -euo pipefail

REPO_URL="https://github.com/vibeforge1111/kait-intel.git"
TARGET_DIR="$(pwd)/kait-intel"
SKIP_UP=0
...
echo "============================================="
echo "  KAIT - macOS/Linux bootstrap"
echo "============================================="
echo
...
echo "Installing Kait (services extras)..."
...
if [[ "$SKIP_UP" -eq 1 ]]; then
  ...
  echo "Start later with: $VENV_PY -m kait.cli up"
  exit 0
fi

echo
echo "Starting Kait services..."
"$VENV_PY" -m kait.cli up
