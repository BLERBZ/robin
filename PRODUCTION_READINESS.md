# Kait Intelligence v1 - Production Readiness
...
- `kaitd` enforces token auth on mutating `POST` endpoints when `KAITD_TOKEN` is set.
- `kaitd` now has per-IP rate limiting and bounded invalid-event quarantine retention.
...
1. Add structured JSON logs in `hooks/observe.py`, `kaitd.py`, and `lib/bridge_cycle.py`.
