#!/usr/bin/env python3
# ruff: noqa: S603,S607
"""Lightweight Kait watchdog: restarts critical workers and warns on queue growth."""
...
KAIT_DIR = Path(__file__).resolve().parent
LOG_DIR = Path.home() / ".kait" / "logs"
STATE_FILE = Path.home() / ".kait" / "watchdog_state.json"
PID_FILE = Path.home() / ".kait" / "pids" / "watchdog.pid"
PLUGIN_ONLY_SENTINEL = Path.home() / ".kait" / "plugin_only_mode"
...
LOG_MAX_BYTES = int(os.environ.get("KAIT_LOG_MAX_BYTES", "10485760"))
LOG_BACKUPS = int(os.environ.get("KAIT_LOG_BACKUPS", "5"))
    ...
    try:
        ...
        for pid, cmd in snapshot:
            if pid == os.getpid():
                continue
            if (
                "-m kait_watchdog" in cmd
                or "kait_watchdog.py" in cmd
...
def _pulse_keyword_sets() -> list[list[str]]:
    sets: list[list[str]] = [["kait/pulse", "app.py"], ["kait/pulse", "app.py"], ["vibeship-kait-pulse", "app.py"], ["vibeship-kait-pulse", "app.py"]]
    try:
    ...
    sets.append([KAIT_PULSE_DIR.name, "app.py"])
    sets.append([app_str])
...
def _queue_counts() -> tuple[int, int]:
    sys.path.insert(0, str(KAIT_DIR))
...
def _plugin_only_mode_enabled() -> bool:
    env = (os.environ.get("KAIT_PLUGIN_ONLY") or "").strip().lower()
...
def _env_disabled(service: str) -> bool:
    ...
    if service == "pulse":
        ...
        v = os.environ.get("KAIT_NO_SCHEDULER") or os.environ.get("KAIT_NO_SCHEDULER")
    elif service == "openclaw_tailer":
        v = os.environ.get("KAIT_NO_OPENCLAW_TAILER") or os.environ.get("KAIT_NO_OPENCLAW_TAILER")
    if v is None:
        return False
...
def main() -> None:
    ...
    # that are still initializing from `kait up`
    ...
    while not stop_event.is_set():
        ...
        # kaitd
        manage_kaitd = _restart_allowed("kaitd", plugin_only_mode) and not _env_disabled("kaitd")
        kaitd_ok = _http_ok(KAITD_HEALTH_URL)
        kaitd_fail = _bump_fail("kaitd", kaitd_ok or not manage_kaitd)
        if manage_kaitd and (not kaitd_ok):
            # Match either invocation style:
            # - python -m kaitd
            # - python kaitd.py
            kaitd_pids = _find_pids_by_any_keywords(
                [["kaitd.py"], ["-m kaitd"]],
                snapshot,
            )
            if kaitd_pids and kaitd_fail < args.fail_threshold:
                _log(f"kaitd unhealthy (fail {kaitd_fail}/{args.fail_threshold}) but process exists")
            elif not args.no_restart and _can_restart(state, "kaitd"):
                if kaitd_pids:
                    _terminate_pids(kaitd_pids)
                if _start_process("kaitd", [sys.executable, "-m", "kaitd"]):
                    _record_restart(state, "kaitd")
                    failures["kaitd"] = 0

        # kait pulse -- unified startup via service_control
        ...
        if manage_pulse and (not pulse_ok):
            ...
            if pulse_running and pulse_fail < args.fail_threshold:
                ...
                try:
                    ...
                    pulse_cmd = None
                if pulse_cmd and _start_process("pulse", pulse_cmd, cwd=KAIT_PULSE_DIR):
        ...
        if not sched_ok:
            ...
            if not manage_sched:
                ...
                sched_pids = _find_pids_by_keywords(["kait_scheduler.py"], snapshot)
                if sched_pids and sched_fail < args.fail_threshold:
                    ...
                    if _start_process(
                        "scheduler",
                        [sys.executable, str(KAIT_DIR / "kait_scheduler.py")],
                    ):
                        _record_restart(state, "scheduler")
                        failures["scheduler"] = 0

        # openclaw_tailer (integration adapter)
        # If OpenClaw Gateway is up, keep the OpenClaw->Kait tailer alive so Kait can learn from sessions.
        try:
            ...
            if manage_tailer and (not tailer_ok):
                if tailer_fail < args.fail_threshold:
                    ...
                    kaitd_port = os.environ.get("KAITD_PORT") or os.environ.get("KAITD_PORT", "8787")
                    kaitd_url = f"http://127.0.0.1:{kaitd_port}"
                    hb_env = {}
                    # Enable tailer heartbeat by default (low frequency) unless operator overrides.
                    if "KAIT_OPENCLAW_HEARTBEAT" not in os.environ:
                        hb_env["KAIT_OPENCLAW_HEARTBEAT"] = "1"
                    if "KAIT_OPENCLAW_HEARTBEAT_MINUTES" not in os.environ:
                        hb_env["KAIT_OPENCLAW_HEARTBEAT_MINUTES"] = "15"

                    if _start_process(
                        "openclaw_tailer",
                        [
                            sys.executable,
                            str(KAIT_DIR / "adapters" / "openclaw_tailer.py"),
                            "--kaitd",
                            kaitd_url,
                            "--agent",
                            "main",
