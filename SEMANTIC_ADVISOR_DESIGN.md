...
- `TUNEABLES.md` + `~/.kait/trigger_rules.yaml` for configuration

Ops notes:
- Enable via `semantic.enabled=true` and `triggers.enabled=true` in `~/.kait/tuneables.json`
- Rebuild index with `python scripts/semantic_reindex.py`
- Use `kait up --lite` when you want only core services (no dashboards/pulse/watchdog)
...
2. Load in advisor from `~/.kait/tuneables.json`
3. Expose in Kait Pulse dashboard
...
#### Tuneable Configuration (~/.kait/tuneables.json)
...
#### Static Rules (~/.kait/trigger_rules.yaml)
...
# Learned triggers (auto-populated by Kait)
learned:
...
| `~/.kait/tuneables.json` | EXTEND | None | Low |
| `~/.kait/trigger_rules.yaml` | NEW | None | Low |
