...
If you're using Claude Code with Kait hooks configured, EIDOS is **automatically active**.
...
| File | Purpose |
|------|---------|
| `~/.kait/eidos.db` | SQLite database (episodes, steps, distillations) |
| `~/.kait/truth_ledger.json` | Claims, facts, rules with evidence |
| `~/.kait/policy_patches.json` | Behavior change rules |
| `~/.kait/acceptance_plans.json` | Definition of Done for episodes |
| `~/.kait/eidos_active_episodes.json` | Session â†’ Episode mapping |
...
import sys
sys.path.insert(0, '/path/to/kait-intel')
...
|----------|---------|-------------|
| `KAIT_EIDOS_ENABLED` | `1` | Enable/disable EIDOS |
| `KAIT_OUTCOME_CHECKIN` | `0` | Enable outcome check-ins |

### Disable EIDOS (if needed)

```bash
# Temporarily disable
set KAIT_EIDOS_ENABLED=0

# Or in Python
import os
os.environ["KAIT_EIDOS_ENABLED"] = "0"
```
...
dir %USERPROFILE%\.kait\eidos.db

# View raw stats
python -c "
import sqlite3
conn = sqlite3.connect('<USER_HOME>/.kait/eidos.db')
...
```bash
# Backup first!
copy %USERPROFILE%\.kait\eidos.db %USERPROFILE%\.kait\eidos.db.backup

# Delete to reset
del %USERPROFILE%\.kait\eidos.db
```

### View Debug Logs

```bash
# Check Kait debug log
type %USERPROFILE%\.kait\debug.log
```
...
â”‚                                                                â”‚
â”‚  KEY FILES:                                                    â”‚
â”‚  ~/.kait/eidos.db          - Main database                    â”‚
â”‚  ~/.kait/truth_ledger.json - Claims/facts/rules               â”‚
â”‚  ~/.kait/policy_patches.json - Behavior rules                 â”‚
â”‚                                                                â”‚
