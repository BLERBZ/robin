# Kait Intelligence â€” Optimization Checker
...
- For GC behavior specifically, you can override without rollback: `KAIT_BRIDGE_GC_EVERY=1`.
...
Expect:
- `kaitd/dashboard/pulse/meta_ralph/bridge_worker/scheduler/watchdog` â†’ `running: true`
- bridge_worker + scheduler heartbeats should be fresh (typically < 60s)

### API health endpoints
- kaitd: `http://127.0.0.1:8787/health` â†’ `ok`
- kaitd status: `http://127.0.0.1:8787/status` â†’ JSON, no obvious errors

### Logs directory: last modified
```powershell
Get-ChildItem $env:USERPROFILE\.kait\logs | Sort-Object LastWriteTime -Descending | Select-Object -First 10 Name,Length,LastWriteTime
```
...
**Change:** `KAIT_ADVISORY_AGREEMENT_GATE=1` makes WARNING-level advisories require agreement across multiple sources.
...
**Knobs:**
- `KAIT_ADVISORY_AGREEMENT_GATE=1|0`
- `KAIT_ADVISORY_AGREEMENT_MIN_SOURCES=2` (default)

---

#### I) Importance sampling under backlog (skip low-priority pattern detection)
**Change:** `KAIT_PIPELINE_IMPORTANCE_SAMPLING=1` skips LOW-priority events in the expensive pattern detection loop when backlog is `critical|emergency`.
...
**Checks:**
- `~/.kait/pipeline_metrics.json`:
  - under backlog: processing_rate_eps â†‘, queue_depth_after stabilizes
- Watchdog log should not show new error loops.

**Knobs:**
- `KAIT_PIPELINE_IMPORTANCE_SAMPLING=1|0`
- `KAIT_PIPELINE_LOW_KEEP_RATE=0.25` (default)

---

#### J) Macro mining (temporal tool-sequence abstractions)
**Change:** `KAIT_MACROS_ENABLED=1` mines frequent successful tool n-grams and stores at most one macro insight per cycle.

**What to look for:**
- Over time, you should see macro-style insights appear in cognitive insights (META_LEARNING).

**Checks:**
- Inspect `~/.kait/cognitive_insights.json` for entries starting with `Macro (often works):`.
- Make sure cognitive insights don't explode in count (bounded by min_count + the â€œone per cycleâ€ cap).

**Knobs:**
- `KAIT_MACROS_ENABLED=1|0`
- `KAIT_MACRO_MIN_COUNT=3` (default)
...
**Change:** `KAIT_MEMORY_PATCHIFIED=1` stores long memories as multiple chunk entries for more precise retrieval, while returning at most one hit per parent group.
...
**Checks:**
- DB exists: `%USERPROFILE%\.kait\memory_store.sqlite`
...
**Knobs:**
- `KAIT_MEMORY_PATCHIFIED=1|0`
- `KAIT_MEMORY_PATCH_MAX_CHARS=600` (default)
- `KAIT_MEMORY_PATCH_MIN_CHARS=120` (default)

---

#### L) Delta memory compaction (store updates as deltas when near-duplicate)
**Change:** `KAIT_MEMORY_DELTAS=1` attempts to store only the â€œdeltaâ€ when a new memory is very similar to a recent one (same scope/project/category).
...
- Search for delta entries:
  ```powershell
  python -c "import sqlite3; from pathlib import Path; db=Path.home()/'.kait'/'memory_store.sqlite'; conn=sqlite3.connect(str(db)); rows=conn.execute(\"select memory_id, content from memories where content like 'Update (delta from %' order by created_at desc limit 5\").fetchall(); print(rows); conn.close()"
  ```

**Knobs:**
- `KAIT_MEMORY_DELTAS=1|0`
- `KAIT_MEMORY_DELTA_MIN_SIM=0.86` (default)
...
**Change:** `KAIT_OUTCOME_PREDICTOR=1` enables a tiny success/failure predictor keyed by `(phase, intent_family, tool)`.

**What to look for:**
- Cautionary advisories become slightly more insistent when Kait has evidence that a tool/intent combo often fails.
- No added latency spikes on PreTool (predictor is just counters + cache).

**Checks:**
- Predictor file exists and grows slowly:
  - `%USERPROFILE%\.kait\outcome_predictor.json`
...
**Change(s):**
- Rotate `~/.kait/logs/semantic_retrieval.jsonl` when it exceeds a size cap.
...
```powershell
$path = "$env:USERPROFILE\.kait\logs\semantic_retrieval.jsonl"; if(Test-Path $path){ (Get-Item $path).Length } else { "missing" }
```
2) Confirm rotation artifacts appear when large:
- `semantic_retrieval.jsonl.1`, `.2`, ...

**Knobs:**
- `~/.kait/tuneables.json` â†’ `semantic.log_retrievals` (on/off)
- `~/.kait/tuneables.json` â†’ `semantic.log_max_bytes`, `semantic.log_backups`, `semantic.log_sample_rate`
...
2) `~/.kait/advisor/*.jsonl` still gets entries.
...
Inspect:
```powershell
Get-ChildItem $env:USERPROFILE\.kait\queue -Force | Select-Object Name,LastWriteTime
```
...
**Checks:**
1) `kaitd /status` shows bridge health OK.
2) `bridge_worker_heartbeat.json` stays fresh.
3) Watchdog does not repeatedly restart bridge worker.

**Knobs:**
- `KAIT_BRIDGE_STEP_EXECUTOR_WORKERS` (default 4)
- `KAIT_BRIDGE_STEP_TIMEOUT_S`
...
2) Check `~/.kait/logs/watchdog.log` for restart loops.
...
**Validation Today:**
Confirm Kait still runs; visuals/out is regenerable; advisory auto score can be regenerated.
