#!/usr/bin/env python3
"""
Kait CLI - Command-line interface for Kait
...
Usage:
    ...
    python -m kait.cli process    # Run bridge worker cycle / drain backlog
    python -m kait.cli validate   # Run validation scan
    python -m kait.cli learnings  # Show recent learnings
    python -m kait.cli promote    # Run promotion check
    python -m kait.cli write      # Write learnings to markdown
    python -m kait.cli health     # Health check
    python -m kait.cli memory     # Memory capture suggestions
    python -m kait.cli advisory   # Guided advisory setup (2 questions)
    python -m kait.cli outcome    # Record explicit outcome check-in
    python -m kait.cli advice-feedback  # Record explicit advice helpfulness
    python -m kait.cli eval       # Evaluate predictions vs outcomes
    python -m kait.cli validate-ingest  # Validate recent queue events
    python -m kait.cli project    # Project questioning + capture
    python -m kait.cli personality-evolution  # Inspect/apply/reset personality evolution V1
"""
...
from lib.kait_voice import get_kait_voice
...
def _premium_tools_enabled() -> bool:
    ...
    val = os.getenv("KAIT_PREMIUM_TOOLS", "") or os.getenv("KAIT_PREMIUM_TOOLS", "")
...
def cmd_start(args):
    """Launch the Kait AI sidekick with avatar."""
    import sys
    sidekick = Path(__file__).resolve().parent.parent / "kait_ai_sidekick.py"
    cmd = [sys.executable, str(sidekick)]
...
def cmd_status(args):
    ...
    voice = get_kait_voice()
    
    print("\n" + "=" * 60)
    print("  KAIT - Self-Evolving Intelligence Layer")
    print("=" * 60)
...
def cmd_queue(args):
    ...
    bridge = get_mind_bridge()
    print("[KAIT] Processing offline queue...")
...
def cmd_process(args):
    ...
    while True:
        ...
        if errors:
            print(f"[KAIT] Cycle errors: {', '.join(errors)}")
    ...
    print(f"[KAIT] bridge_worker cycles: {iterations}, patterns processed: {processed}")


def cmd_validate(args):
    """Run validation loop scan on recent events."""
    stats = process_validation_events(limit=args.limit)
    print("[KAIT] Validation scan")
...
def cmd_promote(args):
    ...
    print(f"[KAIT] Checking for promotable insights (dry_run={dry_run})...")
...
def cmd_write(args):
    ...
    print("[KAIT] Writing learnings to markdown...")
...
def cmd_decay(args):
    ...
    if args.apply:
        ...
        print(f"[KAIT] Pruned {pruned} stale insights")
        return
...
def cmd_up(args):
    """Start Kait background services."""
    lite_env = (os.environ.get("KAIT_LITE", "") or os.environ.get("KAIT_LITE", "")).lower() in ("1", "true", "yes")
    ...
    if args.sync_context:
        ...
        print(f"[kait] sync-context: {project_dir}")


def cmd_ensure(args):
    """Ensure Kait services are running (start any missing)."""
    cmd_up(args)


def cmd_down(args):
    """Stop Kait background services."""
    results = stop_services()
    print("")
    print("[kait] stopping services")
    for name, result in results.items():
        print(f"  {name}: {result}")
    print("")
...
def cmd_opportunities(args):
    ...
    if sub == "list":
        ...
        print(f"[KAIT] Opportunities (showing {len(rows)})")
        for r in rows:
            oid = str(r.get("opportunity_id") or "").strip()
        ...
        if not row:
            print(f"[KAIT] Opportunity not found for id/prefix: {prefix}")
            return
        ...
        if action == "accept":
            ...
            print(f"[KAIT] Accepted {oid} -> {out_path}")
        else:
            print(f"[KAIT] Dismissed {oid}")
    else:
        print("[KAIT] Unknown opportunities subcommand. Use: kait opportunities list|accept|dismiss")
...
def cmd_capture(args):
    ...
    if args.scan or (not args.list and not args.accept and not args.reject):
        ...
        print("[KAIT] Memory capture scan")
...
def cmd_outcome(args):
    ...
    if args.pending:
        ...
        if not items:
            print("[KAIT] No pending check-ins found.")
            return
        print("[KAIT] Recent check-in requests:")
        for item in items:
    ...
    if link_count > 0:
        ...
        auto_link = args.auto_link or (os.environ.get("KAIT_OUTCOME_AUTO_LINK") or os.environ.get("KAIT_OUTCOME_AUTO_LINK")) == "1"
        if auto_link:
    ...
    append_outcome(row)
    print(f"[KAIT] Outcome recorded: {row.get('result')} (polarity={polarity})")
...
def cmd_advice_feedback(args):
    ...
    if args.pending:
        ...
        if not items:
            print("[KAIT] No pending advice feedback requests.")
            return
        print(f"[KAIT] Advice Feedback Requests ({len(items)}):")
        for row in items:
    ...
    if args.analyze:
        ...
        if summary.get("total_feedback", 0) == 0:
            print("[KAIT] No advice feedback yet.")
            return
        print("[KAIT] Advice Feedback Summary")
    ...
    if result.get("status") == "ok":
        ...
        print(f"[KAIT] Advice feedback recorded for {len(ids)} item(s){' on ' + tool if tool else ''}.")
    else:
        print(f"[KAIT] Advice feedback failed: {result.get('message')}")
...
def cmd_outcome_link(args):
    ...
    print(f"[KAIT] Link created: {link.get('link_id')}")
...
def cmd_outcome_stats(args):
    ...
    if kpis:
        print("[KAIT] Prediction/Outcome Loop KPIs")
...
def cmd_outcome_unlinked(args):
    ...
    if not outcomes:
        print("[KAIT] No unlinked outcomes found.")
        return

    print(f"[KAIT] Unlinked Outcomes ({len(outcomes)}):")
    for o in outcomes:
...
def cmd_outcome_links(args):
    ...
    if not links:
        print("[KAIT] No links found.")
        return

    print(f"[KAIT] Outcome-Insight Links ({len(links)}):")
    for link in links:
...
def cmd_auto_link(args):
    ...
    if preview:
        ...
        if not candidates:
            print("[KAIT] No linkable candidates found.")
            return
        print(f"[KAIT] Linkable candidates ({len(candidates)}):")
        for c in candidates:
    ...
    print(f"[KAIT] Auto-Link Outcomes ({mode})")
    print(f"   Processed: {stats['processed']}")
...
def cmd_sync_banks(args):
    ...
    mode = "DRY RUN" if dry_run else "APPLIED"
    print(f"[KAIT] Sync Insights to Banks ({mode})")
    print(f"   Processed: {stats['processed']}")
...
def cmd_memory_purge_telemetry(args):
    ...
    print(f"[KAIT] Memory Store Telemetry Purge ({mode})")
...
def cmd_eidos_purge_telemetry(args):
    ...
    mode = "DRY RUN" if getattr(args, "dry_run", False) else "APPLIED"
    print(f"[KAIT] EIDOS Distillation Telemetry Purge ({mode})")
...
def _print_project_questions(profile, limit: int = 5):
    ...
    if not suggested:
        print("[KAIT] No unanswered questions.")
        return
    print("[KAIT] Suggested questions:")
    for q in suggested:
...
def cmd_project_init(args):
    ...
    print(f"[KAIT] Project: {profile.get('project_key')}  Domain: {profile.get('domain')}")
    if added:
        print(f"[KAIT] Added {added} domain questions")
...
def cmd_project_status(args):
    profile = load_profile(Path(args.project) if args.project else None)
    score = completion_score(profile)
    print(f"[KAIT] Project: {profile.get('project_key')}")
...
def cmd_project_answer(args):
    ...
    if not entry:
        print("[KAIT] Answer not recorded (missing id or text).")
        return
...
def cmd_project_capture(args):
    ...
    print(f"[KAIT] Captured {entry_type}.")
...
def cmd_project_phase(args):
    ...
    if args.set_phase:
        ...
        ensure_questions(profile)
        print(f"[KAIT] Phase set: {profile.get('phase')}")
    else:
        print(f"[KAIT] Phase: {profile.get('phase')}")
...
def cmd_voice(args):
    """Show or interact with Kait's personality."""
    voice = get_kait_voice()
    ...
    if args.opinions:
        ...
        print(f"\nðŸŽ­ Kait's Opinions ({len(opinions)} total)\n")
        for o in opinions:
...
def cmd_bridge(args):
    ...
    if args.update:
        update_kait_context(query=args.query)
        print("âœ“ Updated KAIT_CONTEXT.md with active learnings")
    elif args.promote:
...
def cmd_curiosity(args):
    ...
    if args.fill:
        ...
        print(f"[KAIT] Gap {gap_id} filled. Valuable: {valuable}")
        return
...
def cmd_hypotheses(args):
    ...
    if args.outcome:
        ...
        if len(parts) != 2:
            print("[KAIT] Invalid format. Use: --outcome <hypothesis_id>:<prediction_index>")
            return
        h_id, idx = parts[0], int(parts[1])
        correct = args.correct
        tracker.record_outcome(h_id, idx, correct, args.notes or "")
        print(f"[KAIT] Outcome recorded: {'correct' if correct else 'incorrect'}")
        return
...
def cmd_contradictions(args):
    ...
    if args.resolve is not None:
        ...
        print(f"[KAIT] Contradiction {idx} resolved as: {res_type}")
        return
...
def _print_advisory_preferences(preferences: dict) -> None:
    ...
    print("[KAIT] Advisory Preferences")
...
_ADVISORY_PROVIDER_META = {
    ...
    "minimax": {
        "service": "MiniMax",
        "key_envs": ["MINIMAX_API_KEY", "KAIT_MINIMAX_API_KEY", "KAIT_MINIMAX_API_KEY"],
...
def _advisory_doctor_snapshot() -> dict:
    ...
    recommendations = []
    if not runtime_up:
        recommendations.append("kait up")
    if not replay_on:
        recommendations.append("kait advisory on")
    if drift.get("has_drift"):
        recommendations.append("kait advisory repair")
    preferred = str(runtime.get("preferred_provider") or "auto")
    if not bool(runtime.get("synth_ai_available")):
        if preferred == "minimax":
            recommendations.append("Set MINIMAX_API_KEY (or KAIT_MINIMAX_API_KEY), then rerun doctor")
        else:
            recommendations.append("kait advisory quality --profile enhanced --provider ollama")
...
def cmd_advisory(args):
    ...
    source = str(getattr(args, "source", "") or f"kait_cli_{advisory_cmd}")
    if advisory_cmd == "repair":
        ...
        print("[KAIT] Advisory Profile Repair")
    ...
    if advisory_cmd == "quality":
        ...
        print("[KAIT] Advisory Quality Uplift")
    ...
    print("Use: kait advisory [setup|show|set|on|off|repair|doctor|quality]")
...
def cmd_memory(args):
    ...
    if args.set_mode:
        ...
        print(applied)
        print("\nNext: run `clawdbot memory index --agent main` (or use `kait memory --status`).")
        return
...
def cmd_chips(args):
    """Manage Kait chips - domain-specific intelligence modules."""
    from pathlib import Path
    ...
    def _insight_count(chip_id: str) -> int:
        path = Path.home() / ".kait" / "chip_insights" / f"{chip_id}.jsonl"
        if not path.exists():
            return 0
        try:
    ...
    if args.action == "list":
        ...
        if not chips:
            print("\n[KAIT] No chips installed.")
            print("        Use 'kait chips install <path>' to install a chip.")
            return

        print(f"\n{'=' * 50}")
        print("  KAIT CHIPS - Domain Intelligence")
        print(f"{'=' * 50}\n")
        ...
        if not args.path:
            print("[KAIT] Use --path to specify chip YAML file")
            return

        path = Path(args.path).expanduser()
        if not path.exists():
            print(f"[KAIT] File not found: {path}")
            return

        try:
            chip = registry.install(path)
            if not chip:
                print(f"[KAIT] Install failed: {path}")
                return
            print(f"[KAIT] Installed chip: {chip.id} v{chip.version}")
            print(f"        Name: {chip.name}")
            print(f"        Use 'kait chips activate {chip.id}' to enable")
        except Exception as e:
            print(f"[KAIT] Install failed: {e}")

    elif args.action == "uninstall":
        if not args.chip_id:
            print("[KAIT] Specify chip ID to uninstall")
            return

        if registry.uninstall(args.chip_id):
            print(f"[KAIT] Uninstalled chip: {args.chip_id}")
        else:
            print(f"[KAIT] Chip not found: {args.chip_id}")

    elif args.action == "activate":
        if not args.chip_id:
            print("[KAIT] Specify chip ID to activate")
            return

        if registry.activate(args.chip_id):
            print(f"[KAIT] Activated chip: {args.chip_id}")
        else:
            print(f"[KAIT] Chip not found: {args.chip_id}")

    elif args.action == "deactivate":
        if not args.chip_id:
            print("[KAIT] Specify chip ID to deactivate")
            return

        if registry.deactivate(args.chip_id):
            print(f"[KAIT] Deactivated chip: {args.chip_id}")
        else:
            print(f"[KAIT] Chip not found: {args.chip_id}")
        ...
        if not chip:
            print(f"[KAIT] Chip not found: {chip_id}")
            return
        ...
        chip_id = args.chip_id
        if not chip_id:
            print("[KAIT] Specify chip ID to view insights")
            return

        insights = runtime.get_insights(chip_id, limit=args.limit or 10)

        if not insights:
            print(f"\n[KAIT] No insights for chip: {chip_id}")
            return

        print(f"\n[KAIT] Insights from {chip_id} (showing {len(insights)})\n")
        for i in insights:
            print(f"  {i.content}")
        ...
        chip_id = args.chip_id
        if not chip_id:
            print("[KAIT] Specify chip ID to test")
            return
        ...
        if chip_id:
            ...
            if not chip.questions:
                print(f"[KAIT] Chip {chip_id} has no questions defined")
                return

            print(f"\n[KAIT] Questions from {chip_id}:\n")
            ...
            if not questions:
                print("[KAIT] No questions from active chips.")
                print("        Activate chips with 'kait chips activate <chip_id>'")
                return

            print(f"\n[KAIT] Questions from Active Chips:\n")
            for q in questions:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Kait CLI - Self-evolving intelligence layer",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Commands:
  start       Launch the AI sidekick with avatar
...
Examples:
  kait start
  kait start --no-avatar
  kait status
  kait services
  kait up --sync-context
  kait sync
  kait promote --dry-run
  kait learnings --limit 20
  kait capture --list
  kait capture --accept <id>
  kait advisory
  kait advisory on
  kait advisory doctor
  kait advisory repair
  kait advisory quality --profile enhanced
"""
    )
    ...
    def _add_up_args(p):
        ...
        p.add_argument("--no-pulse", action="store_true", help="do not start kait pulse")
    ...
    # start (AI sidekick with avatar)
    start_parser = subparsers.add_parser("start", help="Launch the Kait AI sidekick with avatar")
    start_parser.add_argument("--no-avatar", action="store_true", help="Disable graphical avatar window")
    start_parser.add_argument("--daemon", action="store_true", help="Run in daemon mode with auto-reconnect")
    ...
    opps_list.add_argument("--scope-type", choices=["project", "operation", "kait_global"], help="Filter by scope_type")
    ...
    advisory_setup.add_argument("--source", default="kait_cli_setup", help="Source label for metadata")
    ...
    advisory_set.add_argument("--source", default="kait_cli_set", help="Source label for metadata")
    ...
    advisory_on.add_argument("--source", default="kait_cli_on", help="Source label for metadata")
    ...
    advisory_off.add_argument("--source", default="kait_cli_off", help="Source label for metadata")

    advisory_repair = advisory_sub.add_parser("repair", help="Re-apply profile defaults and clear drift")
    advisory_repair.add_argument("--source", default="kait_cli_repair", help="Source label for metadata")
    ...
    advisory_quality.add_argument("--source", default="kait_cli_quality", help="Source label for metadata")
    ...
    eidos_parser.add_argument("--migrate", action="store_true", help="Run migration from old Kait to EIDOS")
    ...
    # voice
    voice_parser = subparsers.add_parser("voice", help="Kait's personality")
    voice_parser.add_argument("--introduce", "-i", action="store_true", help="Introduce Kait")
    ...
    bridge_parser.add_argument("--update", "-u", action="store_true", help="Update KAIT_CONTEXT.md")
    ...
    capture_parser = subparsers.add_parser("capture", help="Capture important statements into Kait learnings")
    ...
    chips_parser = subparsers.add_parser("chips", help="Manage Kait chips - domain-specific intelligence")
