# Carmack Kait Review Prompt (Run Every 2-4 Hours)
...
Your job is to evaluate Kait Intelligence performance for the latest work block.
...
python -c "from pathlib import Path;import json,collections; p=Path.home()/'.kait'/'advisory_engine.jsonl'; lines=p.read_text(encoding='utf-8').splitlines()[-500:]; c=collections.Counter(json.loads(l).get('event','unknown') for l in lines if l.strip()); print(dict(c))"
python -c "from pathlib import Path;import json; p=Path.home()/'.kait'/'sync_stats.json'; print(json.loads(p.read_text(encoding='utf-8')))"
python -c "from pathlib import Path;import json; p=Path.home()/'.kait'/'chip_merge_state.json'; print(json.loads(p.read_text(encoding='utf-8')).get('last_stats'))"
```
