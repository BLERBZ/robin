# Carmack Kait Review Prompt V2 (Advanced)
...
python -c "from pathlib import Path;import json,collections; p=Path.home()/'.kait'/'advisory_engine.jsonl'; lines=p.read_text(encoding='utf-8').splitlines()[-800:]; c=collections.Counter(json.loads(l).get('event','unknown') for l in lines if l.strip()); print(dict(c))"
