...
- **Input**:
  ...
  - optional state file `~/.openclaw/workspace/kait_build_queue/worker_state.json`
- **Output**:
...
- **Priority**: P3
- **Scope**: OpenClaw + kait-x-builder mapping
- **Task**:
...
- Scheduler/queue behavior verified in `kait_scheduler.py`
- Candidate generation and controls in `scripts/daily_trend_research.py` (kait-x-builder repo)
