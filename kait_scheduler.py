#!/usr/bin/env python3
# ruff: noqa: S603
"""kait_scheduler -- periodic X intelligence tasks.
...
logger = logging.getLogger("kait.scheduler")

KAIT_DIR = Path.home() / ".kait"
SCHEDULER_DIR = KAIT_DIR / "scheduler"
HEARTBEAT_FILE = SCHEDULER_DIR / "heartbeat.json"
# Back-compat path used by some status checks and older scripts.
LEGACY_HEARTBEAT_FILE = KAIT_DIR / "scheduler_heartbeat.json"
STATE_FILE = SCHEDULER_DIR / "state.json"
DRAFT_REPLIES_FILE = KAIT_DIR / "multiplier" / "draft_replies.json"
MULTIPLIER_DB_PATH = KAIT_DIR / "multiplier" / "scored_mentions.db"
TUNEABLES_FILE = KAIT_DIR / "tuneables.json"
OPENCLAW_HANDOFF_DIR = KAIT_DIR / "claw_integration"
...
TREND_BUILD_QUEUE_DIR = Path(
    os.getenv(
        ...
        str(Path.home() / ".openclaw" / "workspace" / "kait_build_queue"),
    )
)
OPENCLAW_WORKSPACE = Path(
    os.getenv(
        "KAIT_OPENCLAW_WORKSPACE",
...
def _emit_build_queue(trend_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    manifest = {
        ...
        "source": "kait_scheduler.daily_research",
    ...
    if TREND_NOTIFY_OPENCLAW:
        try:
            ...
            if TREND_WAKE_OPENCLAW and len(jobs) >= 1:
                wake_agent(
                    "Kait trend build jobs are ready. Review the latest_queue manifest in "
                    "~/.openclaw/workspace/kait_build_queue/latest_build_queue.json."
                )
        except Exception as exc:
...
def _run_external_trend_builder() -> Dict[str, Any]:
    """Run trend research in the standalone kait-x-builder repo."""
    builder_root = Path(
        os.getenv("KAIT_X_BUILDER_PATH")
        or os.getenv(
            "KAIT_X_BUILDER_PATH",
            str(Path(__file__).resolve().parent.parent / "kait-x-builder"),
        )
    ).expanduser()
    script_path = builder_root / "scripts" / "daily_trend_research.py"
    if not script_path.exists():
        return {"error": f"Kait X Builder script missing: {script_path}"}
...
def task_mention_poll(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Resolve kait-multiplier path
    multiplier_candidates = [
        Path(__file__).resolve().parent.parent / "kait-multiplier",
        Path(__file__).resolve().parent.parent / "kait-multiplier",
    ]
    ...
    if not multiplier_path:
        return {"error": "kait-multiplier not found"}
...
def task_daily_research(state: Dict[str, Any]) -> Dict[str, Any]:
    """Delegate research and candidate generation to the standalone kait-x-builder."""
...
def task_niche_scan(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report_file = KAIT_DIR / "research_reports" / "latest.json"
...
def main():
    ap = argparse.ArgumentParser(description="Kait X Intelligence Scheduler")
    ...
    logger.info("Kait scheduler starting")
