# Kait Intelligence Tuneable Parameters
...
- Live: `~/.kait/tuneables.json` -> `retrieval.overrides.*`
...
Tuneable surface:
- Live: `~/.kait/tuneables.json` -> `memory_emotion.*`
- Environment overrides:
  - `KAIT_MEMORY_EMOTION_WRITE_CAPTURE`
  - `KAIT_MEMORY_EMOTION_ENABLED`
  - `KAIT_MEMORY_EMOTION_WEIGHT`
  - `KAIT_MEMORY_EMOTION_MIN_SIM`
  - `KAIT_ADVISORY_MEMORY_EMOTION_ENABLED`
  - `KAIT_ADVISORY_MEMORY_EMOTION_WEIGHT`
  - `KAIT_ADVISORY_MEMORY_EMOTION_MIN_SIM`
...
**disabled by default** unless enabled in `~/.kait/tuneables.json` or via
`KAIT_SEMANTIC_ENABLED=1`.
...
| `semantic.log_retrievals` | **true** | Log semantic retrieval events to `~/.kait/logs/semantic_retrieval.jsonl` |
...
| `triggers.rules_file` | **~/.kait/trigger_rules.yaml** | YAML rules file |

Environment overrides:
- `KAIT_SEMANTIC_ENABLED=1`
- `KAIT_TRIGGERS_ENABLED=1`
...
The Event Queue captures all Kait events (tool calls, user prompts, errors) with < 10ms latency. Background processing handles the heavy lifting.
...
|----------|---------|-------------|
| `KAIT_NO_WATCHDOG` | `false` | **Disable watchers.** Set to `true` to turn off all watcher enforcement. Use for debugging only. |
| `KAIT_OUTCOME_AUTO_LINK` | `true` | **Auto-link outcomes.** Automatically link outcomes to their originating Steps. |
| `KAIT_AGENT_CONTEXT_MAX_CHARS` | `1200` | **Agent context char budget.** Canonical max chars for injected compact context. |
| `KAIT_AGENT_CONTEXT_LIMIT` | alias | Backward-compatible alias for `KAIT_AGENT_CONTEXT_MAX_CHARS`. |
| `KAIT_AGENT_CONTEXT_ITEM_LIMIT` | unset | Optional max number of compact context items (falls back to function default). |
| `KAIT_DEBUG` | `false` | **Debug mode.** Enables verbose logging across all components. |
| `KAIT_MIND_PORT` | `8080` | **Mind API port.** Port for Mind persistent memory service. |
| `KAITD_PORT` | `8787` | **kaitd port.** Port for ingest/health. |
| `KAIT_PULSE_PORT` | `8765` | **Kait Pulse port.** |
| `KAIT_LOG_DIR` | `~/.kait/logs` | **Log directory.** Overrides log output directory. |
| `KAIT_LOG_MAX_BYTES` | `10485760` | **Log rotation size.** Bytes before rotating. |
| `KAIT_LOG_BACKUPS` | `5` | **Log rotation backups.** Number of rotated files to keep. |
| `KAIT_QUEUE_MAX_EVENTS` | `10000` | **Queue event cap.** Rotate after this many events. |
| `KAIT_QUEUE_MAX_BYTES` | `10485760` | **Queue size cap.** Rotate after this many bytes. |

### Usage

```bash
# Disable watchers for debugging
export KAIT_NO_WATCHDOG=true

# Enable debug logging
export KAIT_DEBUG=true

# Use non-default Mind port
export KAIT_MIND_PORT=8081
```
...
| `KAIT_CHIP_SCHEMA_VALIDATION` | **warn** | **Schema validation mode**: `warn` (default) or `block` to reject invalid chips. |
...
| CI wants strict validation | Set `KAIT_CHIP_SCHEMA_VALIDATION=block` |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `KAIT_OPPORTUNITY_SCANNER` | `1` | Master enable/disable for scanner runtime. |
| `KAIT_OPPORTUNITY_SELF_MAX` | `3` | Max self-opportunities emitted per cycle. |
| `KAIT_OPPORTUNITY_USER_MAX` | `2` | Max user-facing opportunities when user scan is enabled. |
| `KAIT_OPPORTUNITY_USER_SCAN` | `0` | Opt-in switch for user-facing opportunity prompts. Self-scan remains active by default. |
| `KAIT_OPPORTUNITY_HISTORY_MAX` | `500` | Max retained JSONL lines per scanner file. |
| `KAIT_OPPORTUNITY_SELF_DEDUP_WINDOW_S` | `14400` | Recent-window de-dup horizon for self-opportunity questions. |
| `KAIT_OPPORTUNITY_SELF_RECENT_LOOKBACK` | `240` | Number of recent rows inspected for de-dup checks. |
| `KAIT_OPPORTUNITY_SELF_CATEGORY_CAP` | `1` | Per-cycle cap per category during diversity selection. |
| `KAIT_OPPORTUNITY_OUTCOME_WINDOW_S` | `21600` | Max age window for evaluating acted outcomes against recent opportunities. |
| `KAIT_OPPORTUNITY_OUTCOME_LOOKBACK` | `200` | Number of recent opportunities/outcomes scanned for attribution. |
| `KAIT_OPPORTUNITY_PROMOTION_MIN_SUCCESSES` | `2` | Minimum good outcomes required before promotion candidate generation. |
| `KAIT_OPPORTUNITY_PROMOTION_MIN_EFFECTIVENESS` | `0.66` | Minimum good/acted ratio for promotion eligibility. |
| `KAIT_OPPORTUNITY_PROMOTION_LOOKBACK` | `400` | Rows scanned when building promotion candidates. |
| `KAIT_OPPORTUNITY_LLM_ENABLED` | `1` | Enable LLM-backed opportunity proposal path (deterministic path still runs as fallback). |
| `KAIT_OPPORTUNITY_LLM_PROVIDER` | `auto` | Preferred provider for scanner LLM path (`minimax`, `ollama`, `openai`, `anthropic`, `gemini`). |
| `KAIT_OPPORTUNITY_LLM_TIMEOUT_S` | `2.5` | Per-provider timeout for scanner LLM synthesis. |
| `KAIT_OPPORTUNITY_LLM_MAX_ITEMS` | `3` | Max sanitized LLM opportunity candidates merged each cycle. |
...
python -c "from pathlib import Path; b=Path.home()/'.kait'/'opportunity_scanner'; print('self', (b/'self_opportunities.jsonl').exists(), 'outcomes', (b/'outcomes.jsonl').exists(), 'promotions', (b/'promoted_opportunities.jsonl').exists())"
```
...
```bash
# Test specific text
kait importance --text "Remember this: always use dark theme"

# Should return CRITICAL tier

kait importance --text "okay got it"
...
**Files:** `lib/meta_ralph.py`, `lib/production_gates.py` (vibeship-kait)
...
|----------|---------|--------|
| `KAIT_ADVISORY_ENGINE` | `1` | Master on/off for advisory engine path. |
| `KAIT_ADVISORY_MAX_MS` | `4000` | Total budget for one advisory hook execution. |
| `KAIT_ADVISORY_STALE_S` | `900` | Delivery badge stale window for `live|fallback|blocked|stale` classification. |
| `KAIT_ADVISORY_TEXT_REPEAT_COOLDOWN_S` | `1800` | Suppress re-emitting same advisory text during cooldown window. |
| `KAIT_ADVISORY_REQUIRE_ACTION` | `1` | Enforce actionable next-check text when advisory is too generic. |
| `KAIT_ADVISORY_FORCE_PROGRAMMATIC_SYNTH` | `1` | Force programmatic synthesis (no AI/network) on the pre-tool hot path. Tuneable `advisory_engine.force_programmatic_synth` (if set) overrides this. |
| `KAIT_ADVISORY_SESSION_KEY_INCLUDE_RECENT_TOOLS` | `0` | Include recent tool sequence in packet exact-keying (higher specificity, lower cache hit rate). Tuneable `advisory_engine.session_key_include_recent_tools` (if set) overrides this. |
| `KAIT_ADVISORY_PREFETCH_QUEUE` | `1` | Enables enqueueing background prefetch jobs from user prompts. |
| `KAIT_ADVISORY_PACKET_FALLBACK_EMIT` | `0` | Enables packet no-emit deterministic fallback emission. Default `0` keeps fallback output opt-in. |
| `KAIT_ADVISORY_FALLBACK_RATE_GUARD` | `1` | Enables rate guard for packet no-emit fallback emissions. |
| `KAIT_ADVISORY_FALLBACK_RATE_MAX_RATIO` | `0.55` | Maximum allowed fallback share in recent delivered advisories. |
| `KAIT_ADVISORY_FALLBACK_RATE_WINDOW` | `80` | Rolling advisory event window used by fallback rate guard. |
| `KAIT_ADVISORY_INCLUDE_MIND` | `0` | Default for Mind retrieval inclusion. Tuneable `advisory_engine.include_mind` (if set) overrides this. |
| `KAIT_ADVISORY_EMIT` | `1` | Enables writing advisory text to stdout hook output. |
| `KAIT_ADVISORY_MAX_CHARS` | `500` | Caps emitted advisory length. |
| `KAIT_ADVISORY_FORMAT` | `inline` | Advisory formatting style (`inline` or `block`). |

### Context Sync Target Policy (env)

| Variable | Default | Effect |
|----------|---------|--------|
| `KAIT_SYNC_MODE` | `core` | Sync adapter mode: `core` writes only `openclaw` + `exports`; `all` enables all adapters. |
| `KAIT_SYNC_TARGETS` | _(unset)_ | Explicit comma-list of enabled adapters (overrides mode). |
| `KAIT_SYNC_DISABLE_TARGETS` | _(unset)_ | Comma-list of adapters to force-disable after mode/list resolution. |

### Synthesis Route Knobs (env + tuneables)

| Variable/Key | Default | Effect |
|--------------|---------|--------|
| `KAIT_SYNTH_MODE` / `synthesizer.mode` | `auto` | `auto`, `ai_only`, or `programmatic`. |
| `KAIT_SYNTH_TIMEOUT` / `synthesizer.ai_timeout_s` | `3.0` | AI synthesis timeout ceiling. |
| `KAIT_OLLAMA_MODEL` | `phi4-mini` | Default local model for synthesis. |
| `KAIT_OLLAMA_API` | `http://localhost:11434` | Local Ollama endpoint. |
| `KAIT_SYNTH_PREFERRED_PROVIDER` | _(unset)_ | Env override for preferred provider (`ollama`, `gemini`, `minimax`, `openai`, `anthropic`). |
| `KAIT_MINIMAX_MODEL` | `MiniMax-M2.5` | MiniMax model for synthesis when provider route is `minimax`. |
| `KAIT_MINIMAX_BASE_URL` | `https://api.minimax.io/v1` | MiniMax OpenAI-compatible base URL. |
...
### Recommended `~/.kait/tuneables.json` block
...
```json
{
  ...
  "triggers": {
    "enabled": true,
    "rules_file": "~/.kait/trigger_rules.yaml"
  },
  "promotion": {
    "adapter_budgets": {
      "CLAUDE.md": {
...
All tuneables are now loaded from `~/.kait/tuneables.json` at module import time.
...
| `scheduler` | Kait scheduler automation | `enabled`, `mention_poll_interval`, `engagement_snapshot_interval`, `daily_research_interval`, `niche_scan_interval`, `advisory_review_interval`, `advisory_review_window_hours`, `*_enabled` task flags |
...
def _load_X_config():
    tuneables = Path.home() / ".kait" / "tuneables.json"
...
| Environment variables (`KAIT_*`) | No (after process starts) | Yes |

Practical rule: most sections now hot-apply every bridge cycle. Edit `~/.kait/tuneables.json`, wait one cycle (~60s), changes take effect.
