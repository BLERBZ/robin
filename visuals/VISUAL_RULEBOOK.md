...
- Include @Kait_coded attribution on every scene (small, bottom)
...
- Every scene: small `@Kait_coded` at bottom with green dot
...
Pull real data from the Kait research system. Never use placeholder stats.

| Data Source | Path | What It Contains |
|-------------|------|-----------------|
| Research state | `~/.kait/x_research_state.json` | Session count, tweets scanned, topic performance |
| Social-convo chip | `~/.kait/chip_insights/social-convo.jsonl` | Pattern analysis: trigger rankings, strategy rankings |
| Engagement-pulse chip | `~/.kait/chip_insights/engagement-pulse.jsonl` | Per-tweet LLM analysis: triggers, hooks, why_it_works |
...
| `Attribution` | @Kait_coded branding | `delay` |
