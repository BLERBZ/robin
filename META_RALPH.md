# Meta-Ralph: The Quality Gate for Kait's Self-Evolution

> "Evolve, don't disable. Roast until it's good."
Navigation hub: `docs/GLOSSARY.md`

Meta-Ralph is Kait's internal quality gate - a system that evaluates every proposed learning before it gets stored, ensuring only valuable cognitive insights make it through while primitive operational patterns are filtered out.
...
## THE KAIT INTELLIGENCE CONSTITUTION

### GOVERNING DOCUMENT - READ FIRST, FOLLOW ALWAYS

**This Constitution is the PRIMARY rule set for ALL Kait Intelligence work.**

Before making ANY change, fix, improvement, or tuning to Kait:
1. Read this Constitution
2. Follow all 15 rules
3. No exceptions

**Violations of these rules produce hallucinated progress - changes that look good but don't actually improve Kait.**
...
These rules govern ALL work on Kait Intelligence. They exist to ensure every improvement is real, grounded in architecture, and produces actual learning - not hallucinated progress.
...
```
Sources (observe.py, kaitd.py, adapters/*)
    â†’ Queue (~/.kait/queue/events.jsonl) [+trace_id]
...
|-----------|----------|---------------|
| Cognitive Insights | `~/.kait/cognitive_insights.json` | Direct file read |
| EIDOS Store | `~/.kait/eidos.db` | `get_store().get_stats()` |
| Pattern Detection | `~/.kait/detected_patterns.jsonl` | `get_aggregator().get_stats()` |
| Chip Insights | `~/.kait/chip_insights/` | Direct file read |
| Mind Memories | `~/.mind/lite/memories.db` | `GET /v1/stats` |
| Meta-Ralph State | `~/.kait/meta_ralph/` | `get_meta_ralph().get_stats()` |
| **Roast History** | `~/.kait/meta_ralph/roast_history.json` | Quality verdicts + scores |
| **Outcome Tracking** | `~/.kait/meta_ralph/outcome_tracking.json` | Retrieval + outcome links |
| **Advisor Effectiveness** | `~/.kait/advisor/effectiveness.json` | Advice â†’ outcome correlation |
| **Bridge Heartbeat** | `~/.kait/bridge_worker_heartbeat.json` | Bridge worker health |
...
Quick start:
1. `python -m kait.cli up`
2. **Kait Pulse** (primary web dashboard): `http://localhost:${KAIT_PULSE_PORT:-8765}`
3. **Obsidian Observatory**: `python scripts/generate_observatory.py --force` (see `docs/OBSIDIAN_OBSERVATORY_GUIDE.md`)
4. **CLI scripts**: `scripts/kait_dashboard.py`, `scripts/eidos_dashboard.py`

Key Pulse surfaces:
1. **`http://localhost:${KAIT_PULSE_PORT:-8765}`** - Kait Pulse unified operator dashboard (Mission, Learning, Rabbit, Acceptance, Ops, Trace/Run, Chips/Tune/Tools)
2. `http://localhost:${KAIT_PULSE_PORT:-8765}/api/status` - status heartbeat
3. `http://localhost:${KAIT_PULSE_PORT:-8765}/api/mission` - mission KPIs
4. `http://localhost:${KAIT_PULSE_PORT:-8765}/api/acceptance` - acceptance/validation board data
5. `http://localhost:${KAIT_PULSE_PORT:-8765}/api/ops` - ops aggregates
...
import json
f = Path.home() / '.kait' / 'cognitive_insights.json'
...
Kait was storing primitive learnings that polluted the knowledge base:
...
This loop is how Kait learns which advice actually helps.
...
| 2026-02-04 | Built meta_ralph_dashboard.py (port `KAIT_META_RALPH_PORT`, default 8586) | Need dedicated tool for advice quality analysis | Real-time quality metrics from storage |
...
| 2026-02-03 | Added KaitRunLog facade (run index) + run query API | Make run timelines queryable without new storage | Recent runs visible; /api/run resolves run detail |
...
1. **meta_ralph_dashboard.py** (port `KAIT_META_RALPH_PORT`, default 8586)
...
| mind_bridge | OPTIONAL | Requires Mind Lite running on `KAIT_MIND_PORT` (default 8080) |
...
| EIDOS migration | `~/.kait/eidos.db` | Added trace_id column to steps table |
...
- Every session that modifies Kait MUST update META_RALPH.md changelog
...
even though `~/.kait/detected_patterns.jsonl` contained 586 patterns.
...
| Distillations | from `~/.kait/eidos.db` |
| Truth ledger | from `~/.kait/truth_ledger.json` |
| Acceptance plans | from `~/.kait/acceptance_plans.json` |
...
Location: `<KAIT_LIVE_TEST_DIR>`

Files:
- `KAIT_MONITOR.py` - Real-time learning observation
...
**Goal:** Identify and fix the 10 highest-impact improvements for Kait learning.
...
### Verified Quality Items (What Kait Actually Learns)
...
| **Meta-Ralph Dashboard** | **NEW** | Port `KAIT_META_RALPH_PORT` (default 8586), dedicated quality analyzer |
...
- Meta-Ralph Quality Analyzer dashboard built (port `KAIT_META_RALPH_PORT`, default 8586)
...
4. âœ… **Meta-Ralph dashboard** - Dedicated quality analyzer on port `KAIT_META_RALPH_PORT` (default 8586)
