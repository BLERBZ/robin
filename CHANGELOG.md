...
All notable changes to Kait Intelligence are documented here.
...
|----------|---------|-------------|
| `KAIT_EIDOS_ENABLED` | `1` | Enable EIDOS integration |
...
kait eidos --distillations --type heuristic
...
kait eidos --steps
kait eidos --steps --episode <episode_id>
```
...
| **Migration** | `lib/eidos/migration.py` | Data migration from old Kait to EIDOS |
...
kait eidos --migrate --dry-run
kait eidos --migrate

# Validate migration
kait eidos --validate-migration
```
...
Kait was deciding what's important **after** learning, not **during** learning. This meant:
...
- `kait/cli.py` - Added `kait importance` command for testing

### CLI Testing

```bash
# Test rule-based importance scoring
kait importance --text "Remember this: always use forward slashes"
kait importance --examples  # See example scorings
kait importance --text "I prefer dark mode" --domain game_dev

# Test with semantic intelligence (compares to known-valuable insights)
kait importance --text "User prefers iterative fixes" --semantic

# View prediction accuracy feedback
kait importance --feedback
```
...
Removed ALL primitive/operational learning from Kait. The system now only captures and stores truly cognitive insights that a human would find useful.
...
- `vibeship-kait/lib/learner.py` - Gutted operational tracking
- `vibeship-kait/lib/learning_filter.py` - Replaced with stub
- `vibeship-kait/lib/pre_action_reflection.py` - Removed LearningFilter dependency
- `vibeship-kait/processor.py` - Removed operational logging and cognitive extraction
- `~/.kait/cognitive_insights.json` - Cleaned of primitive data
- `~/.kait/graduated_patterns.json` - Cleared
...
- Kait captured ALL tool events (success, failure, timing)
