...
- Location: `kait-x-builder/scripts/daily_trend_research.py`
- Responsibility:
...
- Location: `kait_scheduler.py` (`task_daily_research`)
- Responsibility:
  - Runs `kait-x-builder` script via subprocess
  - Parses JSON result
  - Writes handoff file
...
6. Post-run eventing
   - Emit completion event back to Kait or OpenClaw dashboard
...
- Keep trend intelligence in Kait/intelligence + kait-x-builder:
