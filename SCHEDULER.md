...
### Run scheduler alone
```
python kait_scheduler.py
```

### Run one task right now
```
python kait_scheduler.py --task mention_poll --force
python kait_scheduler.py --task engagement_snapshots --force
python kait_scheduler.py --task daily_research --force
python kait_scheduler.py --task niche_scan --force
```
`daily_research` stores build candidates into:

```
~/.kait/research_reports/latest.json
```

`daily_research` now runs inside the external repo at `kait-x-builder` (path in `KAIT_X_BUILDER_PATH`) and writes an OpenClaw/Clawdbot handoff here:

```
~/.kait/claw_integration/latest_trend_handoff.json
```
...
KAIT_X_BUILDER_PATH=<KAIT_X_BUILDER_PATH>
```

When set, the scheduler posts the exact payload in `~/.kait/claw_integration/latest_trend_handoff.json` to both endpoints after each successful run.

### Run all due tasks once and exit
```
python kait_scheduler.py --once
```

### Check status
```
python -m kait.cli status
```
...
```
~/.kait/multiplier/draft_replies.json
```
...
Edit `~/.kait/tuneables.json`, add a `"scheduler"` section:
...
```
~/.kait/trend_research_topics.json
```
...
| File | What |
|------|------|
| `kait_scheduler.py` | The daemon |
| `lib/x_client.py` | Talks to X API via tweepy |
| `~/.kait/scheduler/state.json` | Task timestamps |
| `~/.kait/scheduler/heartbeat.json` | Health signal |
| `~/.kait/multiplier/draft_replies.json` | Reply queue |
| `~/.kait/multiplier/scored_mentions.db` | Scored mentions DB |
...
**Scheduler won't start:** Check `TWITTER_BEARER_TOKEN` is set. Run `python kait_scheduler.py --once` to see errors.

**No mentions showing up:** The X API only returns mentions from the last 7 days. Run `python kait_scheduler.py --task mention_poll --force` to test.

**Watchdog keeps restarting it:** Check `~/.kait/logs/scheduler.log` for errors.

**Want to stop it:** `python -m kait.cli down` stops everything, or kill the scheduler PID from `~/.kait/pids/scheduler.pid`.
