#!/usr/bin/env python3
"""
Kait Observation Hook: Ultra-fast event capture + Surprise Detection + EIDOS Integration
...
{
  "hooks": {
    "PreToolUse": [{"matcher": "", "hooks": [{"type": "command", "command": "python /path/to/kait/hooks/observe.py"}]}],
    "PostToolUse": [{"matcher": "", "hooks": [{"type": "command", "command": "python /path/to/kait/hooks/observe.py"}]}],
    "PostToolUseFailure": [{"matcher": "", "hooks": [{"type": "command", "command": "python /path/to/kait/hooks/observe.py"}]}]
  }
}
"""
...
# EIDOS Integration
EIDOS_ENABLED = os.environ.get("KAIT_EIDOS_ENABLED", "1") == "1"
...
PREDICTION_FILE = Path.home() / ".kait" / "active_predictions.json"
CHECKIN_MIN_S = int(os.environ.get("KAIT_OUTCOME_CHECKIN_MIN_S", "1800"))
ADVICE_FEEDBACK_ENABLED = os.environ.get("KAIT_ADVICE_FEEDBACK", "1") == "1"
ADVICE_FEEDBACK_PROMPT = os.environ.get("KAIT_ADVICE_FEEDBACK_PROMPT", "1") == "1"
ADVICE_FEEDBACK_MIN_S = int(os.environ.get("KAIT_ADVICE_FEEDBACK_MIN_S", "600"))
PRETOOL_BUDGET_MS = float(os.environ.get("KAIT_OBSERVE_PRETOOL_BUDGET_MS", "2500"))
EIDOS_ENFORCE_BLOCK = os.environ.get("KAIT_EIDOS_ENFORCE_BLOCK", "0") == "1"
HOOK_PAYLOAD_TEXT_LIMIT = int(os.environ.get("KAIT_HOOK_PAYLOAD_TEXT_LIMIT", "3000"))
...
FAILURE_TRACKING_FILE = Path.home() / ".kait" / "session_failures.json"
...
def _load_tool_success_rates() -> dict:
    ...
    """
    cache_file = Path.home() / ".kait" / "tool_success_cache.json"
    try:
...
def _estimate_advisory_readiness(text: str, source: str, tool_name: str = "") -> float:
    ...
    if source and source not in {"kait", "claude_code", "unknown", ""}:
        readiness += 0.05
...
def _normalize_source(raw_source: Any) -> str:
    ...
    if not source:
        return "claude_code"
    if source in {"kait", "kait-hook", "kait-hook-json"}:
        return "kait"
...
def get_event_type(hook_event_name: str) -> EventType:
    """Map hook event name to Kait event type."""
    mapping = {
...
def main():
            ...
            try:
                ...
                if decision and not decision.allowed:
                    ...
                    if EIDOS_ENFORCE_BLOCK:
                        sys.stderr.write("[EIDOS] Enforcement enabled (KAIT_EIDOS_ENFORCE_BLOCK=1). Exiting non-zero.\n")
                        raise SystemExit(2)
            except Exception as e:
    ...
    # This keeps Kait core platform-agnostic and makes memory capture work.
    ...
    if hook_event in ("Stop", "SessionEnd") and os.environ.get("KAIT_OUTCOME_CHECKIN") == "1":
        ...
        if recorded and os.environ.get("KAIT_OUTCOME_CHECKIN_PROMPT") == "1":
            sys.stderr.write("[KAIT] Outcome check-in: run `kait outcome`\\n")
    ...
    if hook_event in ("Stop", "SessionEnd") and ADVICE_FEEDBACK_PROMPT:
        try:
            ...
            if has_recent_requests():
                sys.stderr.write("[KAIT] Advice feedback pending: run `kait advice-feedback --pending`\\n")
        except Exception:
            pass
