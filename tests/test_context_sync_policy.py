...
def test_default_sync_policy_is_core(monkeypatch):
    monkeypatch.delenv("KAIT_SYNC_MODE", raising=False)
    monkeypatch.delenv("KAIT_SYNC_TARGETS", raising=False)
    monkeypatch.delenv("KAIT_SYNC_DISABLE_TARGETS", raising=False)
...
def test_sync_policy_allows_explicit_targets_env(monkeypatch):
    monkeypatch.setenv("KAIT_SYNC_TARGETS", "openclaw,exports,cursor")
    monkeypatch.delenv("KAIT_SYNC_DISABLE_TARGETS", raising=False)
    monkeypatch.delenv("KAIT_SYNC_MODE", raising=False)
    monkeypatch.setattr(context_sync, "TUNEABLES_FILE", context_sync.Path("/nonexistent/tuneables.json"))
