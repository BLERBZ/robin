...
def test_noise_filter_rejects_lets_transcript_fragments_without_apostrophe():
    ...
    assert learner.is_noise_insight(
        "lets push gitthub, and then clean up the whole system except this kait, and at some point bring the council of 50"
    )
...
def test_noise_filter_rejects_rambling_when_using_remember_transcript():
    learner = CognitiveLearner()
    assert learner.is_noise_insight(
        "When using Bash, remember: Do it, and is this system fully connected to kait intelligence flow right now, for kait to e"
    )
