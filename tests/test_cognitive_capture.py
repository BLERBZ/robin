...
Tests the quality of data being captured by Kait's learning pipeline.
...
def _isolate_meta_ralph(tmp_path, monkeypatch):
    """Keep tests hermetic and avoid writing to shared ~/.kait state."""
...
def analyze_roast_history() -> Dict:
    ...
    roast_file = Path.home() / '.kait' / 'meta_ralph' / 'roast_history.json'
...
def save_baseline(metrics: CaptureQualityMetrics):
    ...
    baseline_file = Path.home() / '.kait' / 'capture_baseline.json'
