...
def test_load_live_metrics_reads_local_files(monkeypatch, tmp_path: Path):
    kait_dir = tmp_path / ".kait"
    (kait_dir / "advisor").mkdir(parents=True)
    (kait_dir / "chip_insights").mkdir(parents=True)

    (kait_dir / "cognitive_insights.json").write_text(
        json.dumps({"k1": {"insight": "a"}, "k2": {"insight": "b"}}),
        encoding="utf-8",
    )
    (kait_dir / "advisor" / "effectiveness.json").write_text(
        json.dumps(
            {
    ...
    (kait_dir / "chip_insights" / "demo.jsonl").write_text(
        '{"a":1}\n{"a":2}\n{"a":3}\n',
        encoding="utf-8",
    )

    monkeypatch.setattr("lib.production_gates.KAIT_DIR", kait_dir)
    monkeypatch.setattr("lib.production_gates.COGNITIVE_FILE", kait_dir / "cognitive_insights.json")
    monkeypatch.setattr("lib.production_gates.EFFECTIVENESS_FILE", kait_dir / "advisor" / "effectiveness.json")
    monkeypatch.setattr("lib.production_gates.CHIP_INSIGHTS_DIR", kait_dir / "chip_insights")
