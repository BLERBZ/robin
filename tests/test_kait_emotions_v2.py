...
import lib.kait_emotions as se
from lib.kait_emotions import KaitEmotions as KaitEmotions


def test_emotion_timeline_continuity_persists(tmp_path):
    state_file = tmp_path / "emotion_state.json"
    emotions = KaitEmotions(state_file=state_file)
    ...
    reloaded = KaitEmotions(state_file=state_file)
...
def test_trigger_mapping_updates_emotional_state(tmp_path):
    emotions = KaitEmotions(state_file=tmp_path / "emotion_state.json")
...
def test_legacy_repo_local_state_migrates_to_runtime_path(tmp_path, monkeypatch):
    legacy_state = tmp_path / "repo" / ".kait" / "emotion_state.json"
    ...
    runtime_state = tmp_path / "home" / ".kait" / "emotion_state.json"
    monkeypatch.setattr(se, "LEGACY_STATE_FILE", legacy_state)

    emotions = se.KaitEmotions(state_file=runtime_state)
