...
def test_cmd_advisory_show_json(monkeypatch, capsys):
    ...
    kait_cli.cmd_advisory(_args(advisory_cmd="show", json=True))
...
def test_cmd_advisory_setup_applies_current_when_non_interactive(monkeypatch):
    ...
    monkeypatch.setattr(kait_cli, "apply_advisory_preferences", _fake_apply)

    kait_cli.cmd_advisory(_args(advisory_cmd="setup", source="kait_cli_setup"))

    assert calls["memory_mode"] == "standard"
    assert calls["guidance_style"] == "balanced"
    assert calls["source"] == "kait_cli_setup"
...
def test_cmd_advisory_set_defaults_to_on_when_empty(monkeypatch):
    ...
    kait_cli.cmd_advisory(_args(advisory_cmd="set", source="kait_cli_set"))

    assert calls["memory_mode"] == "standard"
    assert calls["guidance_style"] == "balanced"
    assert calls["source"] == "kait_cli_set"
...
def test_cmd_advisory_off_forces_memory_mode_off(monkeypatch):
    ...
    kait_cli.cmd_advisory(
        _args(advisory_cmd="off", guidance_style="coach", source="kait_cli_off")
    )

    assert calls["memory_mode"] == "off"
    assert calls["guidance_style"] == "coach"
    assert calls["source"] == "kait_cli_off"


def test_print_advisory_preferences_uses_runtime_for_true_on_state(capsys):
    kait_cli._print_advisory_preferences(
        {
            "memory_mode": "standard",
            "guidance_style": "balanced",
            "effective": {"replay_enabled": True},
...
def test_cmd_advisory_quality_calls_uplift(monkeypatch, capsys):
    ...
    monkeypatch.setattr(kait_cli, "apply_advisory_quality_uplift", _fake_quality)
    ...
    assert calls["source"] == "kait_cli_quality"
...
def test_cmd_advisory_doctor_json(monkeypatch, capsys):
    ...
    kait_cli.cmd_advisory(_args(advisory_cmd="doctor", json=True))
    payload = json.loads(capsys.readouterr().out)
...
def test_cmd_advisory_repair_uses_preference_repair(monkeypatch, capsys):
    ...
    monkeypatch.setattr(kait_cli, "repair_advisory_profile_drift", _fake_repair)
    monkeypatch.setattr(
        kait_cli,
        "_get_advisory_runtime_state",
        lambda: {"available": True, "engine_enabled": True, "emitter_enabled": True},
    ...
    kait_cli.cmd_advisory(_args(advisory_cmd="repair", source="kait_cli_repair"))
    out = capsys.readouterr().out

    assert calls["source"] == "kait_cli_repair"
