...
from lib.research import intents, mastery, kait_research, web_research
...
def test_process_research_results_forwards_purpose_to_mastery_and_web(monkeypatch):
    ...
    monkeypatch.setattr(kait_research, "get_web_researcher", lambda: _FakeWebResearcher())
    monkeypatch.setattr(kait_research, "get_researcher", lambda: _FakeMasteryResearcher())

    out = kait_research.process_research_results(
        "api_design",
