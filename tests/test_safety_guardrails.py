...
def test_blocks_obviously_destructive_bash(monkeypatch):
    monkeypatch.delenv("KAIT_SAFETY_GUARDRAILS", raising=False)
    eng = GuardrailEngine()
...
def test_blocks_secret_file_read_by_default(monkeypatch):
    monkeypatch.delenv("KAIT_SAFETY_GUARDRAILS", raising=False)
    monkeypatch.delenv("KAIT_SAFETY_ALLOW_SECRETS", raising=False)
    eng = GuardrailEngine()
...
def test_allows_secret_file_read_with_override(monkeypatch):
    monkeypatch.setenv("KAIT_SAFETY_ALLOW_SECRETS", "1")
    eng = GuardrailEngine()
...
def test_can_disable_safety_guardrails(monkeypatch):
    monkeypatch.setenv("KAIT_SAFETY_GUARDRAILS", "0")
    eng = GuardrailEngine()
