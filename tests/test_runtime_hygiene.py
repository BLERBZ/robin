...
def test_cleanup_removes_stale_runtime_and_tmp_files(tmp_path):
    ...
    runtime_dir = tmp_path / "kait"
    pid_file = repo_root / "scripts" / ".kait_pids.json"
...
def test_cleanup_keeps_fresh_files(tmp_path, monkeypatch):
    ...
    pid_file.parent.mkdir(parents=True, exist_ok=True)
    pid_file.write_text(json.dumps({"kaitd": 999999}), encoding="utf-8")
...
def test_cleanup_removes_pid_file_when_all_pids_dead(tmp_path, monkeypatch):
    ...
    pid_file = repo_root / "scripts" / ".kait_pids.json"
    pid_file.parent.mkdir(parents=True, exist_ok=True)
    pid_file.write_text(json.dumps({"kaitd": 123, "bridge": 456}), encoding="utf-8")
