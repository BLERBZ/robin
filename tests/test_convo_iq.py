...
class TestConvoAnalyzer:
    ...
    def setup(self, tmp_path, monkeypatch):
        ...
        # Reset kait_voice state
        sv_dir = tmp_path / "kait_voice"
        sv_dir.mkdir()
        monkeypatch.setattr("lib.kait_voice.KAIT_DIR", sv_dir)
        monkeypatch.setattr("lib.kait_voice.VOICE_FILE", sv_dir / "voice.json")

        # Reset singletons
        import lib.convo_analyzer
        import lib.x_voice
        import lib.kait_voice
        lib.convo_analyzer._analyzer = None
        lib.x_voice._x_voice = None
        lib.kait_voice._voice = None
