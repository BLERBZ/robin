#!/usr/bin/env python3
"""
Kait 10 Improvements Test Suite v2 (Fixed)
"""
...
KAIT_DIR = Path(__file__).resolve().parent.parent
...
def test_2_persistence_pipeline():
    ...
    try:
        ...
        print_result("Persistence working", disk_count > 0, f"{disk_count} insights persisted")
        if disk_count == 0:
            pytest.skip("no cognitive_insights.json yet — run Kait to accumulate data")
        assert disk_count > 0, f"expected >0 persisted insights, got {disk_count}"
...
def test_5_aggregator_integration():
    ...
    try:
        ...
        observe_file = KAIT_DIR / "hooks" / "observe.py"
        observe_code = observe_file.read_text()
        pipeline_code = (KAIT_DIR / "lib" / "pipeline.py").read_text()
        has_aggregator = (
        ...
        if stats.get('steps', 0) == 0:
            pytest.skip("EIDOS has no steps yet — run Kait to accumulate episodes")
    except pytest.skip.Exception:
        raise
    except Exception as e:
...
def test_8_advisor_integration():
    ...
    try:
        ...
        if len(advice) == 0:
            pytest.skip("advisor has no guidance yet — run Kait to build insight base")
    except pytest.skip.Exception:
        raise
    except Exception as e:
...
def run_all_tests():
    ...
    print("\n" + "=" * 60)
    print("  KAIT 10 IMPROVEMENTS - TEST SUITE v2")
    print("=" * 60)
