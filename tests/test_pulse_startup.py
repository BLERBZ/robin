...
def test_start_services_uses_pulse_repo_as_cwd(monkeypatch, tmp_path):
    pulse_dir = tmp_path / "vibeship-kait-pulse"
    pulse_dir.mkdir(parents=True, exist_ok=True)
    pulse_app = pulse_dir / "app.py"
    pulse_app.write_text("print('pulse')\n", encoding="utf-8")

    monkeypatch.setattr(service_control, "KAIT_PULSE_DIR", pulse_dir)
    monkeypatch.setattr(
        service_control,
        "service_status",
...
def test_redirector_launches_external_pulse_with_repo_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(kait_pulse.subprocess, "call", _fake_call)
    ...
    try:
        ...
        raise AssertionError("Expected SystemExit from kait_pulse.main()")

    assert called["args"][0] == kait_pulse.sys.executable
...
def test_resolve_pulse_dir_prefers_inline(monkeypatch, tmp_path):
    root = tmp_path / "kait-intel"
    root.mkdir(parents=True, exist_ok=True)
    inline_pulse = root / "kait" / "pulse"
    inline_pulse.mkdir(parents=True, exist_ok=True)
    (inline_pulse / "app.py").write_text("print('pulse')\n", encoding="utf-8")

    # Also create the legacy sibling so we can confirm inline wins
    sibling_pulse = tmp_path / "vibeship-kait-pulse"
    sibling_pulse.mkdir(parents=True, exist_ok=True)
    (sibling_pulse / "app.py").write_text("print('pulse')\n", encoding="utf-8")

    monkeypatch.delenv("KAIT_PULSE_DIR", raising=False)
    monkeypatch.setattr(service_control, "ROOT_DIR", root)

    resolved = service_control._resolve_pulse_dir()
    assert resolved == inline_pulse


def test_resolve_pulse_dir_falls_back_to_sibling(monkeypatch, tmp_path):
    root = tmp_path / "kait-intel"
    root.mkdir(parents=True, exist_ok=True)
    # No inline pulse — only the legacy sibling exists
    sibling_pulse = tmp_path / "vibeship-kait-pulse"
    sibling_pulse.mkdir(parents=True, exist_ok=True)
    (sibling_pulse / "app.py").write_text("print('pulse')\n", encoding="utf-8")
...
def test_load_repo_env_parses_basic_env_file(tmp_path):
    ...
    env_file.write_text(
        "\n".join(
            [
                ...
                "export KAIT_OPPORTUNITY_LLM_PROVIDER=minimax",
                "QUOTED='hello world'",
            ]
        )
        + "\n",
    ...
    assert out["KAIT_MINIMAX_MODEL"] == "MiniMax-M2.5"
    assert out["KAIT_OPPORTUNITY_LLM_PROVIDER"] == "minimax"
