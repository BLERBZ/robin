...
class TestSynthesisFormat:
    ...
    def test_note_has_kait_prefix(self):
        from lib.advisory_emitter import format_advisory
        text = format_advisory("Use batch mode for saves.", "note")
        assert text.startswith("[KAIT]"), f"NOTE should start with [KAIT], got: {text[:30]}"
    ...
    def test_whisper_has_parenthetical(self):
        ...
        assert text.startswith("(kait:"), f"WHISPER should start with (kait:, got: {text[:30]}"
