...
into the Kait Intelligence flow and that data is coming from REAL sources.
...
def test_storage_layer():
    ...
    # Core storage files
    storage_files = [
        ("~/.kait/cognitive_insights.json", "Cognitive Insights"),
        ("~/.kait/eidos.db", "EIDOS Database"),
        ("~/.kait/queue/events.jsonl", "Event Queue"),
        ("~/.kait/bridge_worker_heartbeat.json", "Bridge Heartbeat"),
        ("~/.kait/meta_ralph/roast_history.json", "Roast History"),
        ("~/.kait/meta_ralph/outcome_tracking.json", "Outcome Tracking"),
    ]
    ...
    chip_dir = os.path.expanduser("~/.kait/chip_insights")
    ...
    if not all_pass:
        pytest.skip(
            f"storage files not yet populated (missing: {', '.join(missing)}) "
            "— run Kait to accumulate data"
        )
...
def test_bridge_worker():
    ...
    hb_path = os.path.expanduser("~/.kait/bridge_worker_heartbeat.json")
