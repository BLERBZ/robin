from lib.events import KaitEventKind, KaitEventV1
from lib.queue import EventType
import kaitd


def _evt(kind: KaitEventKind, payload: dict, *, source: str = "openclaw") -> KaitEventV1:
    return KaitEventV1(
        v=1,
        source=source,
        kind=kind,
...
def test_resolve_queue_event_type_maps_tool_pre_and_post():
    message = _evt(KaitEventKind.MESSAGE, {"role": "user", "text": "hello"})
    tool_call = _evt(KaitEventKind.TOOL, {"tool_name": "Edit", "tool_input": {"file_path": "a.py"}})
    tool_ok = _evt(KaitEventKind.TOOL, {"tool_name": "Edit", "tool_result": "done", "is_error": False})
    tool_fail = _evt(KaitEventKind.TOOL, {"tool_name": "Edit", "tool_result": "boom", "is_error": True})
    command = _evt(KaitEventKind.COMMAND, {"command": "session_start"})

    assert kaitd._resolve_queue_event_type(message) == EventType.USER_PROMPT
    assert kaitd._resolve_queue_event_type(tool_call) == EventType.PRE_TOOL
    assert kaitd._resolve_queue_event_type(tool_ok) == EventType.POST_TOOL
    assert kaitd._resolve_queue_event_type(tool_fail) == EventType.POST_TOOL_FAILURE
    assert kaitd._resolve_queue_event_type(command) == EventType.SESSION_START
...
def test_openclaw_runtime_bridge_calls_user_prompt_and_emotion(monkeypatch):
    ...
    monkeypatch.setattr(kaitd, "_emotion_recover", lambda: calls.append(("emotion_recover",)))

    evt = _evt(
        KaitEventKind.MESSAGE,
        {"role": "user", "text": "I am confused and this is not clear"},
    )
    kaitd._maybe_bridge_openclaw_runtime(evt, EventType.USER_PROMPT)
...
def test_openclaw_runtime_bridge_calls_pre_and_post_tool(monkeypatch):
    ...
    kaitd._maybe_bridge_openclaw_runtime(pre_evt, EventType.PRE_TOOL)
    kaitd._maybe_bridge_openclaw_runtime(fail_evt, EventType.POST_TOOL_FAILURE)
...
def test_dispatch_bridge_can_run_inline_when_async_disabled(monkeypatch):
    ...
    evt = _evt(KaitEventKind.MESSAGE, {"role": "user", "text": "hello"})
    kaitd._dispatch_openclaw_runtime_bridge(evt, EventType.USER_PROMPT)
