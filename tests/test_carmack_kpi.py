...
def test_build_scorecard_gates_quality_gaur_on_schema_v2(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ck,
        "_service_status_snapshot",
        lambda: {"kaitd": {"running": True}, "bridge_worker": {"running": True}, "scheduler": {"running": True}, "watchdog": {"running": True}},
    )
...
def test_core_reliability_uses_effective_service_signals():
    status = {
        "kaitd": {"running": False, "healthy": True},
    ...
    assert core["core_effective_running"]["kaitd"] is True
