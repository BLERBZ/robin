...
def test_scan_runtime_opportunities_filters_telemetry_and_persists(monkeypatch, tmp_path: Path):
    ...
    events = [
        _mk_event(
            ...
            payload={"text": "Let's improve Kait reliability and make the loop better."},
        ),
        _mk_event(
            EventType.USER_PROMPT,
    ...
    out = scanner.scan_runtime_opportunities(
        ...
        query="evolve Kait intelligence loop",
        session_id="s1",
        persist=True,
    )
...
def test_generate_user_opportunities_respects_conservative_mode(monkeypatch):
    ...
    rows = scanner.generate_user_opportunities(
        tool_name="Task",
        context="Improve Kait autonomy strategy for launch",
        task_context="",
        session_id="s1",
        persist=False,
    )
...
def test_advisor_can_surface_opportunity_source(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_bank_advice", lambda _s, _c: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_cognitive_advice", lambda _s, _t, _c, _sc=None: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_chip_advice", lambda _s, _c: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_tool_specific_advice", lambda _s, _t: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_surprise_advice", lambda _s, _t, _c: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_skill_advice", lambda _s, _c: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_convo_advice", lambda _s, _t, _c: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_engagement_advice", lambda _s, _t, _c: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_get_niche_advice", lambda _s, _t, _c: [])
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_rank_advice", lambda _s, items: list(items))
    monkeypatch.setattr(advisor_mod.KaitAdvisor, "_rank_score", lambda _s, _item: 1.0)
    ...
    advisor = advisor_mod.KaitAdvisor()
    out = advisor.advise(
        "Task",
        {},
        "improve kait opportunity loop",
        include_mind=False,
...
def test_scan_runtime_opportunities_filters_recent_repeats(monkeypatch, tmp_path: Path):
    ...
    events = [
        _mk_event(
            EventType.USER_PROMPT,
            payload={"text": "Let's improve Kait reliability and autonomy for this task."},
        ),
        _mk_event(
            EventType.POST_TOOL,
            tool_name="Edit",
    ...
    out = scanner.scan_runtime_opportunities(
        events,
        stats={"errors": ["pipeline"]},
        query="improve Kait delivery quality",
        session_id="s1",
        persist=False,
    )
...
def test_scan_runtime_opportunities_filters_dismissed_questions(monkeypatch, tmp_path: Path):
    ...
    q = "What exact outcome marks done, and how will Kait verify it?"
...
def test_scan_runtime_opportunities_merges_llm_candidates(monkeypatch, tmp_path: Path):
            ...
            [
                {
                    ...
                    "priority": "high",
                    "confidence": 0.9,
                    "question": "Which hidden assumption should Kait test first?",
    ...
    out = scanner.scan_runtime_opportunities(
        [
            _mk_event(EventType.USER_PROMPT, payload={"text": "Improve Kait improvement loop quality"}),
...
def test_generate_llm_self_candidates_blocks_deepseek(monkeypatch):
    ...
    rows, meta = scanner._generate_llm_self_candidates(
        prompts=["Improve Kait autonomy with measurable checks"],
