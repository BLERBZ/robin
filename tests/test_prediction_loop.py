...
from lib.queue import EventType, KaitEvent
...
def test_build_predictions_applies_source_budgets(tmp_path, monkeypatch):
    ...
    for i in range(4):
        exposures.append({"insight_key": f"k-inj-{i}", "text": f"inject insight {i}", "source": "kait_inject"})
    ...
    monkeypatch.setenv("KAIT_PREDICTION_TOTAL_BUDGET", "20")
    monkeypatch.setenv("KAIT_PREDICTION_DEFAULT_SOURCE_BUDGET", "10")
    monkeypatch.setenv(
        "KAIT_PREDICTION_SOURCE_BUDGETS",
        "chip_merge=2,sync_context=2,kait_inject=2",
    )
    ...
    assert by_source.get("kait_inject", 0) <= 2
...
def test_process_prediction_cycle_runs_auto_link_when_due(monkeypatch):
    ...
    monkeypatch.setenv("KAIT_PREDICTION_AUTO_LINK", "1")
    monkeypatch.setenv("KAIT_PREDICTION_AUTO_LINK_INTERVAL_S", "60")
...
def test_process_prediction_cycle_skips_auto_link_within_interval(monkeypatch):
    ...
    monkeypatch.setenv("KAIT_PREDICTION_AUTO_LINK", "1")
    monkeypatch.setenv("KAIT_PREDICTION_AUTO_LINK_INTERVAL_S", "3600")
...
def test_build_predictions_tags_test_namespace(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pl, "get_cognitive_learner", lambda: object())
    monkeypatch.setenv("KAIT_PREDICTION_TOTAL_BUDGET", "40")
    monkeypatch.delenv("KAIT_NAMESPACE", raising=False)
