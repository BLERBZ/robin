"""Embeddings helper for Kait Intelligence.

Supports three backends (chosen via KAIT_EMBED_BACKEND env var):
...
Set KAIT_EMBEDDINGS=0 to force "none" (backwards compatible).
"""
...
def _get_backend() -> str:
    ...
    if os.environ.get("KAIT_EMBEDDINGS", "1").lower() in ("0", "false", "no"):
        _BACKEND = "none"
        return _BACKEND

    _BACKEND = os.environ.get("KAIT_EMBED_BACKEND", "tfidf").lower()
    return _BACKEND
...
def _get_fastembed():
    ...
    try:
        ...
        _FASTEMBED_ERROR = e
        return None
    model = os.environ.get("KAIT_EMBED_MODEL", "BAAI/bge-small-en-v1.5")
    try:
