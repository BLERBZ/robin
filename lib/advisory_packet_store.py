...
PACKET_DIR = Path.home() / ".kait" / "advice_packets"
...
ADVISORY_ENGINE_LOG_FILE = Path.home() / ".kait" / "advisory_engine.jsonl"
ADVISORY_EMIT_FILE = Path.home() / ".kait" / "advisory_emit.jsonl"
ADVISORY_LOW_AUTH_DEDUPE_FILE = Path.home() / ".kait" / "advisory_low_auth_dedupe.jsonl"
ADVISORY_GLOBAL_DEDUPE_FILE = Path.home() / ".kait" / "advisory_global_dedupe.jsonl"
ADVICE_FEEDBACK_REQUESTS_FILE = Path.home() / ".kait" / "advice_feedback_requests.jsonl"
ADVICE_FEEDBACK_FILE = Path.home() / ".kait" / "advice_feedback.jsonl"
ADVISORY_RETRIEVAL_ROUTE_LOG_FILE = Path.home() / ".kait" / "advisor" / "retrieval_router.jsonl"
ADVISOR_ADVICE_LOG_FILE = Path.home() / ".kait" / "advisor" / "advice_log.jsonl"
ADVISOR_RECENT_ADVICE_FILE = Path.home() / ".kait" / "advisor" / "recent_advice.jsonl"
OUTCOMES_FILE = Path.home() / ".kait" / "outcomes.jsonl"
OUTCOME_LINKS_FILE = Path.home() / ".kait" / "outcome_links.jsonl"
IMPLICIT_FEEDBACK_FILE = Path.home() / ".kait" / "advisor" / "implicit_feedback.jsonl"
...
def _obsidian_payload(packet: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"tags: ['kait', 'advisory', 'watchtower', {_yaml(tool)}, {_yaml(intent)}]",
        "---",
        "",
    ...
    lines.extend(
        [
            ...
            "- check `~/.kait/advisory_engine.jsonl`, `~/.kait/advisory_emit.jsonl`, `~/.kait/advisory_decision_ledger.jsonl`, `~/.kait/advisor/retrieval_router.jsonl` for score trail",
            "",
...
def _render_obsidian_index(lines: List[str], catalog: List[Dict[str, Any]]) -> None:
    ...
    lines.append("# KAIT Advisory Packet Catalog")
    lines.append("")
    lines.extend(
        [
...
def _render_obsidian_watchtower(lines: List[str], catalog: List[Dict[str, Any]]) -> None:
    ...
    lines.extend(
        [
            ...
            f'FROM "{packets_path}"',
            "WHERE type = \"kait-advisory-packet\"",
