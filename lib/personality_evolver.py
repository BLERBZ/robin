...
def feature_enabled() -> bool:
    return str(os.environ.get("KAIT_PERSONALITY_EVOLUTION_V1", "")).strip().lower() in {"1", "true", "yes", "on"}


def observer_mode_enabled() -> bool:
    return str(os.environ.get("KAIT_PERSONALITY_EVOLUTION_OBSERVER", "")).strip().lower() in {"1", "true", "yes", "on"}


def default_state_path() -> Path:
    # Keep state in the same root convention as other runtime JSONL files.
    return Path.home() / ".kait" / "personality_evolution_v1.json"
