...
Computes normalized distance between runtime tuneables (~/.kait/tuneables.json)
and the version-controlled baseline (config/tuneables.json). Logs drift history
to ~/.kait/tuneable_drift.jsonl and alerts when threshold exceeded.
...
RUNTIME_FILE = Path.home() / ".kait" / "tuneables.json"
DRIFT_LOG_FILE = Path.home() / ".kait" / "tuneable_drift.jsonl"
...
logger = logging.getLogger("kait.tuneables_drift")
...
def log_drift(result: DriftResult) -> None:
    """Append drift result to ~/.kait/tuneable_drift.jsonl."""
    try:
