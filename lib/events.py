#!/usr/bin/env python3
"""Kait event schema (v1)

Goal: make Kait ingestion platform-agnostic.

Adapters (Clawdbot, Claude Code, webhooks, etc.) emit KaitEventV1 objects.
Kait core stores them, then distills into insights/surprises/voice.
"""
...
class KaitEventV1:
    ...
    kind: KaitEventKind        # message/tool/command/system
    ...
    @staticmethod
    def from_dict(d: Dict[str, Any]) -> "KaitEventV1":
        if int(d.get("v", 0)) != 1:
            raise ValueError("Unsupported event version")
        return KaitEventV1(
            v=1,
            source=str(d.get("source") or "unknown"),
            kind=KaitEventKind(str(d.get("kind") or "system")),
...
def validate_event_dict(d: Dict[str, Any], *, strict: bool = True) -> Tuple[bool, str]:
    """Validate a raw KaitEventV1 dict before ingestion."""
    ...
    allowed = {k.value for k in KaitEventKind}
