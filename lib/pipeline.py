"""
Kait Processing Pipeline: Adaptive, priority-aware event processing.
...
def _load_pipeline_config() -> None:
    """Load pipeline tuneables from ~/.kait/tuneables.json → "values" section."""
...
IMPORTANCE_SAMPLING_ENABLED = os.getenv("KAIT_PIPELINE_IMPORTANCE_SAMPLING", "0") == "1"
try:
    LOW_PRIORITY_KEEP_RATE = float(os.getenv("KAIT_PIPELINE_LOW_KEEP_RATE", "0.25"))
except Exception:
...
MACROS_ENABLED = os.getenv("KAIT_MACROS_ENABLED", "0") == "1"
try:
    MACRO_MIN_COUNT = max(2, min(20, int(os.getenv("KAIT_MACRO_MIN_COUNT", "3") or 3)))
except Exception:
    MACRO_MIN_COUNT = 3

# Distillation floor: ensure high-volume cycles produce at least one durable insight.
try:
    MIN_INSIGHTS_FLOOR = max(0, min(3, int(os.getenv("KAIT_PIPELINE_MIN_INSIGHTS_FLOOR", "1") or 1)))
except Exception:
    MIN_INSIGHTS_FLOOR = 1
try:
    FLOOR_EVENTS_THRESHOLD = max(1, min(200, int(os.getenv("KAIT_PIPELINE_MIN_INSIGHTS_EVENTS", "20") or 20)))
except Exception:
    FLOOR_EVENTS_THRESHOLD = 20
try:
    FLOOR_SOFT_MIN_EVENTS = max(1, min(50, int(os.getenv("KAIT_PIPELINE_SOFT_MIN_INSIGHTS_EVENTS", "2") or 2)))
except Exception:
    FLOOR_SOFT_MIN_EVENTS = 2

# Processing health metrics file
PIPELINE_STATE_FILE = Path.home() / ".kait" / "pipeline_state.json"
PIPELINE_METRICS_FILE = Path.home() / ".kait" / "pipeline_metrics.json"
...
def extract_tool_effectiveness(events: List[KaitEvent]) -> Dict[str, Any]:
...
def extract_micro_insights(events: List[KaitEvent]) -> List[Dict[str, Any]]:
...
def extract_error_patterns(events: List[KaitEvent]) -> Dict[str, Any]:
...
def extract_session_workflows(events: List[KaitEvent]) -> Dict[str, Any]:
