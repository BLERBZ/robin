...
SYNTH_CONFIG_FILE = Path.home() / ".kait" / "tuneables.json"
...
OLLAMA_API = os.getenv("KAIT_OLLAMA_API", "http://localhost:11434")
OLLAMA_MODEL = os.getenv("KAIT_OLLAMA_MODEL", "phi4-mini")  # Default quality-first local model; override via KAIT_OLLAMA_MODEL
...
OPENAI_MODEL = os.getenv("KAIT_OPENAI_MODEL", "gpt-4o-mini")  # Cost-efficient

MINIMAX_API_KEY = _load_repo_env_value("MINIMAX_API_KEY", "KAIT_MINIMAX_API_KEY")
MINIMAX_BASE_URL = os.getenv("KAIT_MINIMAX_BASE_URL", "https://api.minimax.io/v1").rstrip("/")
MINIMAX_MODEL = os.getenv("KAIT_MINIMAX_MODEL", "MiniMax-M2.5")

ANTHROPIC_API_KEY = _load_repo_env_value("ANTHROPIC_API_KEY", "CLAUDE_API_KEY")
ANTHROPIC_MODEL = os.getenv("KAIT_ANTHROPIC_MODEL", "claude-haiku-4-5-20251001")

GEMINI_API_KEY = _load_repo_env_value("GEMINI_API_KEY", "GOOGLE_API_KEY")
GEMINI_MODEL = os.getenv("KAIT_GEMINI_MODEL", "gemini-2.0-flash")

# Synthesis mode: "auto" (try AI → fall back to programmatic), "ai_only", "programmatic"
SYNTH_MODE = os.getenv("KAIT_SYNTH_MODE", "auto")

# Optional soul-upgrade context injection (disabled by default for safety/perf)
SOUL_UPGRADE_PROMPT_ENABLED = os.getenv("KAIT_SOUL_UPGRADE_PROMPT", "0") in {"1", "true", "yes", "on"}
...
AI_TIMEOUT_S = float(os.getenv("KAIT_SYNTH_TIMEOUT", "8.0"))
PREFERRED_PROVIDER_ENV = os.getenv("KAIT_SYNTH_PREFERRED_PROVIDER", "")
...
def _get_emotion_decision_hooks() -> Dict[str, Any]:
    """Best-effort fetch of live KaitEmotions decision hooks.
    ...
    try:
        ...
        hooks = KaitEmotions().decision_hooks()
...
def _emotion_shaping_prompt_block(hooks: Dict[str, Any]) -> str:
    ...
    return (
        "\nLive response-shaping hints (Kait Emotions):\n"
...
def _resolve_local_emotion_hooks() -> Optional[Dict[str, Any]]:
    """Resolve local KaitEmotions hooks (secondary fallback)."""
    try:
        from .kait_emotions import KaitEmotions
...
def _emotion_decision_hooks() -> Dict[str, Any]:
    ...
    if local_hooks:
        ...
        source_chain.append("kait_emotions")
