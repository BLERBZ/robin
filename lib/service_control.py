#!/usr/bin/env python3
# ruff: noqa: S603,S607
"""Service control helpers for Kait daemons (kaitd, bridge_worker, pulse, watchdog)."""
...
def _resolve_pulse_dir() -> Path:
    env_value = os.environ.get("KAIT_PULSE_DIR")
    if env_value:
        return Path(env_value).expanduser()

    candidates = [
        ROOT_DIR / "kait" / "pulse",              # inline (preferred)
        ROOT_DIR.parent / "vibeship-kait-pulse",   # legacy sibling
    ]
...
# Inline Kait Pulse directory (FastAPI dashboard and status API in kait/pulse/).
# Falls back to legacy sibling repo vibeship-kait-pulse if the inline version is absent.
KAIT_PULSE_DIR = _resolve_pulse_dir()
STARTUP_READY_TIMEOUT_S = float(os.environ.get("KAIT_STARTUP_READY_TIMEOUT_S", "12"))
STARTUP_READY_POLL_S = float(os.environ.get("KAIT_STARTUP_READY_POLL_S", "0.4"))


def _get_pulse_command() -> list[str]:
    """Get the command to start Kait Pulse (inline kait/pulse/ preferred)."""
    import sys
    pulse_app = KAIT_PULSE_DIR / "app.py"
    if pulse_app.exists():
        return [sys.executable, str(pulse_app)]
    raise FileNotFoundError(
        f"Kait Pulse not found at {pulse_app}. "
        f"Ensure kait/pulse/app.py exists or set KAIT_PULSE_DIR env var."
    )


def _pid_dir() -> Path:
    return Path.home() / ".kait" / "pids"


def _log_dir() -> Path:
    env_dir = os.environ.get("KAIT_LOG_DIR")
    if env_dir:
        return Path(env_dir).expanduser()
    return Path.home() / ".kait" / "logs"
...
def _pulse_process_patterns() -> list[list[str]]:
    app_path = KAIT_PULSE_DIR / "app.py"
    app_str = str(app_path)
    patterns: list[list[str]] = [
        [KAIT_PULSE_DIR.name, "app.py"],
        ["vibeship-kait-pulse", "app.py"],
        [app_str],
    ]
...
def _matrix_heartbeat_age() -> Optional[float]:
    hb_path = Path.home() / ".kait" / "matrix_worker_heartbeat.json"
...
def _env_for_child(log_dir: Path) -> dict:
    ...
    env.setdefault("KAIT_LOG_DIR", str(log_dir))
    # Child services should not pop browsers on startup when managed.
    env.setdefault("KAIT_SERVICE_MODE", "1")
    return env
...
def _start_process(name: str, args: list[str], cwd: Optional[Path] = None) -> Optional[int]:
    ...
    # Open log with raw fd (os.open) so the descriptor stays valid for
    # the child process without depending on a Python `with` block scope.
    # Redirect stdin to /dev/null so child processes are fully detached
    # from the launching terminal — prevents SIGHUP/EOF when that
    # terminal is closed.
    ...
    try:
        ...
        os.close(log_fd)  # parent's copy; child has its own via fork
...
def _is_service_ready(name: str, bridge_stale_s: int = 90) -> bool:
    if name == "kaitd":
        return _http_ok(KAITD_HEALTH_URL)
    if name == "pulse":
        return _pulse_ok()
    ...
    if name == "matrix_worker":
        hb_age = _matrix_heartbeat_age()
        return hb_age is not None and hb_age <= bridge_stale_s
    if name == "watchdog":
...
def _wait_for_service_ready(name: str, pid: Optional[int], bridge_stale_s: int = 90) -> bool:
    ...
    if name in ("watchdog", "scheduler", "matrix_worker"):
        return _pid_alive(pid)
...
def _service_cmds(
    ...
    cmds = {
        ...
        "scheduler": [sys.executable, str(ROOT_DIR / "kait_scheduler.py")],
        "matrix_worker": [sys.executable, str(ROOT_DIR / "matrix_worker.py")],
        "watchdog": [
            sys.executable,
            "-m",
            "kait_watchdog",
...
def service_status(bridge_stale_s: int = 90, include_pulse_probe: bool = True) -> dict[str, dict]:
    kaitd_ok = _http_ok(KAITD_HEALTH_URL)
    pulse_ok = _pulse_ok() if include_pulse_probe else False
    hb_age = _bridge_heartbeat_age()

    sched_hb_age = _scheduler_heartbeat_age()
    matrix_hb_age = _matrix_heartbeat_age()

    kaitd_pid = _read_pid("kaitd")
    ...
    matrix_pid = _read_pid("matrix_worker")

    snapshot = _process_snapshot()
    kaitd_keys = [["-m kaitd"], ["kaitd.py"]]
    pulse_keys = _pulse_process_patterns()
    bridge_keys = [["-m bridge_worker"], ["bridge_worker.py"]]
    scheduler_keys = [["kait_scheduler.py"]]
    watchdog_keys = [["-m kait_watchdog"], ["kait_watchdog.py"], ["scripts/watchdog.py"]]
    matrix_keys = [["matrix_worker.py"]]

    kaitd_running = (
        kaitd_ok
        or _pid_matches(kaitd_pid, kaitd_keys, snapshot)
        or _any_process_matches(kaitd_keys, snapshot)
        or _pid_alive_fallback(kaitd_pid, snapshot)
    )
    pulse_running = (
        pulse_ok
    ...
    matrix_process_running = (
        _pid_matches(matrix_pid, matrix_keys, snapshot)
        or _any_process_matches(matrix_keys, snapshot)
        or _pid_alive_fallback(matrix_pid, snapshot)
    )
    matrix_heartbeat_fresh = (matrix_hb_age is not None and matrix_hb_age <= bridge_stale_s)
    matrix_running = matrix_process_running or matrix_heartbeat_fresh

    return {
        "kaitd": {
            "running": kaitd_running,
            "healthy": kaitd_ok,
            "pid": kaitd_pid,
        },
        "pulse": {
...
def start_services(
    ...
    order = ["kaitd", "bridge_worker", "scheduler", "matrix_worker", "pulse", "watchdog"]
    if not include_pulse:
        order.remove("pulse")
    if not include_watchdog:
        order.remove("watchdog")
    if os.environ.get("KAIT_MATRIX_ENABLED", "").lower() not in ("1", "true", "yes"):
        order.remove("matrix_worker")
    ...
    for name in order:
        ...
        if name in {"bridge_worker", "scheduler", "matrix_worker"}:
        ...
        if not cmd:
            ...
            continue
        process_cwd = KAIT_PULSE_DIR if name == "pulse" else ROOT_DIR
...
def _ollama_running() -> bool:
    ...
    return _http_ok(OLLAMA_API_URL, timeout=2.0)


def ensure_ollama(timeout_s: float = 10.0) -> str:
    """Ensure Ollama is running. Starts it as a detached daemon if needed.

    Returns: "already_running", "started", or "unavailable".
    """
    if _ollama_running():
    ...
    # Start ollama serve as a fully detached daemon
    _, log_dir = _ensure_dirs()
    ...
    return "started" if _ollama_running() else "unavailable"
...
def stop_services() -> dict[str, str]:
    ...
    for name in ["watchdog", "pulse", "matrix_worker", "scheduler", "bridge_worker", "kaitd"]:
        ...
        if name == "bridge_worker":
            try:
                (Path.home() / ".kait" / "bridge_worker_heartbeat.json").unlink(missing_ok=True)
            except Exception:
                pass
        if name == "scheduler":
            try:
                kait_root = Path.home() / ".kait"
                (kait_root / "scheduler_heartbeat.json").unlink(missing_ok=True)
                (kait_root / "scheduler" / "heartbeat.json").unlink(missing_ok=True)
            except Exception:
                pass
        if name == "matrix_worker":
            try:
                (Path.home() / ".kait" / "matrix_worker_heartbeat.json").unlink(missing_ok=True)
            except Exception:
                pass
    return results
...
def format_status_lines(status: dict[str, dict], bridge_stale_s: int = 90) -> list[str]:
    lines: list[str] = []
    kaitd = status.get("kaitd", {})
    pulse = status.get("pulse", {})
    bridge = status.get("bridge_worker", {})
    scheduler = status.get("scheduler", {})
    watchdog = status.get("watchdog", {})
    matrix = status.get("matrix_worker", {})

    lines.append(
        f"[kait] kaitd: {'RUNNING' if kaitd.get('running') else 'STOPPED'}"
        + (" (healthy)" if kaitd.get("healthy") else "")
    )
    lines.append(
        f"[kait] pulse: {'RUNNING' if pulse.get('running') else 'STOPPED'}"
        + (" (healthy)" if pulse.get("healthy") else "")
    )
    hb_age = bridge.get("heartbeat_age_s")
    if hb_age is None:
        if bridge.get("running"):
            lines.append("[kait] bridge_worker: RUNNING (no heartbeat)")
        else:
            lines.append("[kait] bridge_worker: UNKNOWN (no heartbeat)")
    else:
        status_label = "RUNNING" if hb_age <= bridge_stale_s else "STALE"
        lines.append(f"[kait] bridge_worker: {status_label} (last {int(hb_age)}s ago)")
    sched_hb = scheduler.get("heartbeat_age_s")
    if sched_hb is None:
        if scheduler.get("running"):
            lines.append("[kait] scheduler: RUNNING (no heartbeat)")
        else:
            lines.append("[kait] scheduler: STOPPED")
    else:
        sched_label = "RUNNING" if sched_hb <= bridge_stale_s * 2 else "STALE"
        lines.append(f"[kait] scheduler: {sched_label} (last {int(sched_hb)}s ago)")
    matrix_hb = matrix.get("heartbeat_age_s")
    if matrix_hb is None:
        if matrix.get("running"):
            lines.append("[kait] matrix_worker: RUNNING (no heartbeat)")
        else:
            lines.append("[kait] matrix_worker: STOPPED")
    else:
        matrix_label = "RUNNING" if matrix_hb <= bridge_stale_s else "STALE"
        lines.append(f"[kait] matrix_worker: {matrix_label} (last {int(matrix_hb)}s ago)")
    lines.append(
        f"[kait] watchdog: {'RUNNING' if watchdog.get('running') else 'STOPPED'}"
    )
    log_dir = status.get("log_dir")
    if log_dir:
        lines.append(f"[kait] logs: {log_dir}")
    lines.append(
        f"[kait] pulse_dir: {KAIT_PULSE_DIR}"
        + ("" if (KAIT_PULSE_DIR / "app.py").exists() else " (app.py missing)")
    )
    lines.append(f"Kait Pulse: {PULSE_URL}")
    return lines
