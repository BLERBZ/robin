"""
Kait Mind Bridge: Connect cognitive learning to Mind Lite+

This module bridges Kait's insights to Mind's persistent memory:
    Kait Observes → Cognitive Learning → Mind Stores → Patterns Compound
...
try:
    ...
    HAS_REQUESTS = False
    print("[KAIT] requests not installed - Mind sync disabled")
...
SYNC_STATE_FILE = Path.home() / ".kait" / "mind_sync_state.json"
OFFLINE_QUEUE_FILE = Path.home() / ".kait" / "mind_offline_queue.jsonl"
MIND_TOKEN_FILE = Path.home() / ".kait" / "mind_server.token"
...
class MindBridge:
    """
    Bridge between Kait's cognitive learning and Mind's persistent memory.
    """
    ...
    def sync_insight(self, insight: CognitiveInsight) -> SyncResult:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"[KAIT] Synced to Mind: {insight.category.value} - {insight.insight[:50]}...")
                return SyncResult(
            ...
            print(f"[KAIT] Mind sync error: {error_msg}")
    ...
    def sync_all_insights(self) -> Dict[str, int]:
        ...
        print(f"[KAIT] Sync complete: {stats}")
        return stats
    ...
    def process_offline_queue(self) -> int:
        ...
        if synced > 0:
            self._save_sync_state()
            print(f"[KAIT] Processed queue: {synced} synced, {len(remaining)} remaining")
