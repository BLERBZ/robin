...
MERGE_STATE_FILE = Path.home() / ".kait" / "chip_merge_state.json"
...
LEARNING_DISTILLATIONS_FILE = Path.home() / ".kait" / "chip_learning_distillations.jsonl"

CHIP_TELEMETRY_BLOCKLIST = {"kait-core", "bench_core", "bench-core"}
TELEMETRY_MARKERS = (
    "post_tool",
    "pre_tool",
    "tool_name:",
...
CHIP_TO_CATEGORY = {
    ...
    "kait-core": CognitiveCategory.META_LEARNING,
}
...
def _load_merge_tuneables() -> Dict[str, float]:
    ...
    try:
        ...
        if (
            ...
            try:
                use_host_tuneables = TUNEABLES_FILE.resolve() != (Path.home() / ".kait" / "tuneables.json").resolve()
            except Exception:
