"""
Kait Event Queue: Ultra-fast event capture
...
# ============= Configuration =============
QUEUE_DIR = Path.home() / ".kait" / "queue"
EVENTS_FILE = QUEUE_DIR / "events.jsonl"
MAX_EVENTS = int(os.environ.get("KAIT_QUEUE_MAX_EVENTS", "10000"))  # Rotate after this many events
MAX_QUEUE_BYTES = int(os.environ.get("KAIT_QUEUE_MAX_BYTES", "10485760"))  # 10 MB
LOCK_FILE = QUEUE_DIR / ".queue.lock"
OVERFLOW_FILE = QUEUE_DIR / "events.overflow.jsonl"
QUEUE_STATE_FILE = QUEUE_DIR / "state.json"
QUEUE_COMPACT_HEAD_BYTES = int(os.environ.get("KAIT_QUEUE_COMPACT_HEAD_BYTES", str(5 * 1024 * 1024)))
...
class KaitEvent:
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> "KaitEvent":
...
def read_events(limit: int = 100, offset: int = 0) -> List[KaitEvent]:
    ...
    try:
        ...
        for line in (_iter_active_lines_iter(EVENTS_FILE) or []):
            ...
            try:
                ...
                events.append(KaitEvent.from_dict(data))
            except Exception:
                continue
...
def read_recent_events(count: int = 50) -> List[KaitEvent]:
...
def read_recent_events_raw(count: int = 50) -> List[KaitEvent]:
...
def rotate_if_needed() -> bool:
    ...
    try:
        with _queue_lock():
            ...
            _set_queue_head_bytes(0)
            _invalidate_count_cache()
            print(f"[KAIT] Rotated queue: {count} -> {keep_count} events")
            return True
...
def classify_event_priority(event: "KaitEvent") -> int:
