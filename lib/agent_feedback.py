...
These functions are designed to be called by the agent (Kait the Seer)
from within the OpenClaw session to report feedback on advisories,
decisions, and outcomes.

The agent should call these after reading KAIT_ADVISORY.md to close
the feedback loop.
"""
...
def advisory_acted(recommendation: str, outcome: str, success: bool = True) -> Path:
    ...
    Example:
        advisory_acted(
            ...
            "Set KAIT_EMBEDDINGS=0, RAM dropped from 8GB to 68MB",
            success=True
        )
    """
...
def learned_something(lesson: str, context: str = "", confidence: float = 0.8) -> Path:
    ...
    Example:
        learned_something(
            ...
            "Discovered while debugging Kait Pulse dashboard",
            confidence=0.95
        )
    """
    return _write_report(
