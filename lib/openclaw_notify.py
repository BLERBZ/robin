"""OpenClaw notification bridge: push Kait findings into the agent session."""
...
def notify_agent(message: str, priority: str = "normal") -> bool:
    """Write a notification file and update KAIT_NOTIFICATIONS.md.
    ...
    try:
        ...
        for workspace in _workspace_paths():
            notif_dir = workspace / "kait_notifications"
            notif_dir.mkdir(parents=True, exist_ok=True)
            (notif_dir / filename).write_text(
                json.dumps(payload, indent=2), encoding="utf-8"
            )
            _update_notifications_md(workspace / "KAIT_NOTIFICATIONS.md", ts_str, message)
...
def _update_notifications_md(md_path: Path, ts_str: str, message: str) -> None:
    """Append to KAIT_NOTIFICATIONS.md, keeping only last 5 entries."""
    header = "# Kait Notifications\n\nLatest findings pushed by Kait Intelligence.\n\n"
