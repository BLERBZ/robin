"""Lightweight debug logging utilities for Kait."""
...
_LOG_MAX_BYTES = int(os.environ.get("KAIT_LOG_MAX_BYTES", "10485760"))  # 10 MB
_LOG_BACKUPS = int(os.environ.get("KAIT_LOG_BACKUPS", "5"))
...
def debug_enabled() -> bool:
    """Return True when KAIT_DEBUG is set to a truthy value."""
    return os.environ.get("KAIT_DEBUG", "").strip().lower() in _DEBUG_VALUES


def _emit_log_line(component: str, message: str, exc: Optional[BaseException] = None) -> None:
    try:
        line = f"[KAIT][{component}] {message}"
        if exc is not None:
            line = f"{line}: {exc}"
...
def log_debug(component: str, message: str, exc: Optional[BaseException] = None) -> None:
    """Emit a debug log line to stderr when KAIT_DEBUG is enabled."""
...
def log_exception(component: str, message: str, exc: Optional[BaseException] = None) -> None:
    """Emit an error log line regardless of KAIT_DEBUG."""
    _emit_log_line(component, message, exc)
...
def setup_component_logging(component: str) -> Optional[Path]:
    """Ensure logs are written to ~/.kait/logs even when not started by scripts."""
    if component in _LOG_SETUP:
        return None
    log_dir = Path(os.environ.get("KAIT_LOG_DIR") or (Path.home() / ".kait" / "logs"))
    try:
    ...
    tee_enabled = os.environ.get("KAIT_LOG_TEE", "1").strip().lower() in _DEBUG_VALUES
