...
- Disabled by default unless enabled in ~/.kait/tuneables.json or KAIT_SEMANTIC_ENABLED=1
...
DEFAULT_CONFIG = {
    ...
    "exclude_categories": [],
    "trigger_rules_file": "~/.kait/trigger_rules.yaml",
    "category_caps": {
        "cognitive": 3,
        "trigger": 2,
        "default": 2,
...
class SemanticIndex:
    def __init__(self, path: Optional[Path] = None, cache_ttl_s: int = 120):
        self.path = path or (Path.home() / ".kait" / "semantic" / "insights_vec.sqlite")
...
def _load_config() -> Dict[str, Any]:
    ...
    # Env overrides
    if os.environ.get("KAIT_SEMANTIC_ENABLED", "").lower() in ("1", "true", "yes"):
        config["enabled"] = True
    if os.environ.get("KAIT_TRIGGERS_ENABLED", "").lower() in ("1", "true", "yes"):
