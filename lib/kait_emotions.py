...
Mode = Literal["kait_alive", "real_talk", "calm_focus"]

KAIT_DIR = Path.home() / ".kait"
STATE_FILE = Path(os.environ.get("KAIT_EMOTION_STATE_FILE") or os.environ.get("KAIT_EMOTION_STATE_FILE") or (KAIT_DIR / "emotion_state.json"))
LEGACY_STATE_FILE = Path(__file__).resolve().parent.parent / ".kait" / "emotion_state.json"
...
MODE_TARGETS: Dict[Mode, Dict[str, float]] = {
    "kait_alive": {"warmth": 0.78, "energy": 0.74, "calm": 0.58, "playfulness": 0.62},
...
class KaitEmotions:
    ...
    def _migrate_legacy_state_if_needed(self) -> None:
        """Copy repo-local legacy emotion state into ~/.kait path once."""
...
KaitEmotions = KaitEmotions

__all__ = ["KaitEmotions", "KaitEmotions", "EmotionState", "Mode", "TRIGGER_MAP"]
