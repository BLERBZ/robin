"""
Kait Cognitive Learner: Learning to THINK, not just to DO.
...
def _capture_emotion_state_snapshot() -> Dict[str, Any]:
    env = os.environ.get("KAIT_COGNITIVE_EMOTION_CAPTURE")
...
def classify_action_domain(insight_text: str, category: str = "", source: str = "") -> str:
    ...
    Domains:
        ...
        system          - Kait system internals, pipeline, config
    ...
    # System domain — Kait internals
    if any(kw in text_lower for kw in ("tuneables", "meta-ralph", "metaralph", "bridge_cycle", "pipeline", "cognitive_learner")):
        return "system"
    if re.search(r"\b(auto-tuner|bridge_worker|kaitd|kait daemon|queue\.py)\b", text_lower):
        return "system"
...
class CognitiveLearner:
    ...
    INSIGHTS_FILE = Path.home() / ".kait" / "cognitive_insights.json"
    LOCK_FILE = Path.home() / ".kait" / ".cognitive.lock"
    ...
    def _load_insights(self):
        ...
        if self.INSIGHTS_FILE.exists():
            try:
                ...
                print(f"[KAIT] Error loading insights: {e}")
    ...
    def _is_noise_insight(self, text: str) -> bool:
        ...
        # e.g., "[Kait Core Intelligence] pattern: awaiting one"
    ...
    def promote_to_wisdom(self) -> Dict[str, int]:
        ...
        Skips already-wisdom insights and Kait-internal meta-learning noise.
        """
        ...
        for key, insight in list(self.insights.items()):
            ...
            # Skip meta-learning about Kait internals
