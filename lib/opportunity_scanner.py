"""Opportunity Scanner for Kait self-evolution.

MIGRATION NOTE (2026-02-22): This module is being migrated to
kait-learning-systems (system 27-opportunity-scanner). Use
...
1) Runtime self-scan identifies Kait improvement opportunities from active work.
...
SCANNER_ENABLED = str(os.getenv("KAIT_OPPORTUNITY_SCANNER", "1")).strip().lower() not in {
    "0",
    "false",
    "no",
    "off",
}
SELF_MAX_ITEMS = max(1, int(os.getenv("KAIT_OPPORTUNITY_SELF_MAX", "3") or 3))
USER_MAX_ITEMS = max(1, int(os.getenv("KAIT_OPPORTUNITY_USER_MAX", "2") or 2))
MAX_HISTORY_LINES = max(50, int(os.getenv("KAIT_OPPORTUNITY_HISTORY_MAX", "500") or 500))
SELF_DEDUP_WINDOW_S = max(0.0, float(os.getenv("KAIT_OPPORTUNITY_SELF_DEDUP_WINDOW_S", "14400") or 14400.0))
SELF_RECENT_LOOKBACK = max(20, int(os.getenv("KAIT_OPPORTUNITY_SELF_RECENT_LOOKBACK", "240") or 240))
SELF_CATEGORY_CAP = max(1, int(os.getenv("KAIT_OPPORTUNITY_SELF_CATEGORY_CAP", "1") or 1))
USER_SCAN_ENABLED = str(os.getenv("KAIT_OPPORTUNITY_USER_SCAN", "0")).strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
}
SCAN_EVENT_LIMIT = max(0, int(os.getenv("KAIT_OPPORTUNITY_SCAN_EVENT_LIMIT", "120") or 120))

OPPORTUNITY_DIR = Path.home() / ".kait" / "opportunity_scanner"
...
OUTCOME_WINDOW_S = max(300.0, float(os.getenv("KAIT_OPPORTUNITY_OUTCOME_WINDOW_S", "21600") or 21600.0))
OUTCOME_LOOKBACK = max(20, int(os.getenv("KAIT_OPPORTUNITY_OUTCOME_LOOKBACK", "200") or 200))
PROMOTION_FILE = OPPORTUNITY_DIR / "promoted_opportunities.jsonl"
PROMOTION_MIN_SUCCESSES = max(1, int(os.getenv("KAIT_OPPORTUNITY_PROMOTION_MIN_SUCCESSES", "2") or 2))
PROMOTION_MIN_EFFECTIVENESS = max(
    0.0,
    min(1.0, float(os.getenv("KAIT_OPPORTUNITY_PROMOTION_MIN_EFFECTIVENESS", "0.66") or 0.66)),
)
PROMOTION_LOOKBACK = max(20, int(os.getenv("KAIT_OPPORTUNITY_PROMOTION_LOOKBACK", "400") or 400))
LLM_ENABLED = str(os.getenv("KAIT_OPPORTUNITY_LLM_ENABLED", "1")).strip().lower() not in {
    "0",
    "false",
    "no",
    "off",
}
LLM_PROVIDER = str(os.getenv("KAIT_OPPORTUNITY_LLM_PROVIDER", "") or "").strip().lower()
LLM_TIMEOUT_S = max(0.3, float(os.getenv("KAIT_OPPORTUNITY_LLM_TIMEOUT_S", "2.5") or 2.5))
LLM_MAX_ITEMS = max(1, int(os.getenv("KAIT_OPPORTUNITY_LLM_MAX_ITEMS", "3") or 3))
LLM_MIN_CONTEXT_CHARS = max(0, int(os.getenv("KAIT_OPPORTUNITY_LLM_MIN_CONTEXT_CHARS", "140") or 140))
LLM_COOLDOWN_S = max(0.0, float(os.getenv("KAIT_OPPORTUNITY_LLM_COOLDOWN_S", "300") or 300))
...
_DECISION_LOOKBACK = max(50, int(os.getenv("KAIT_OPPORTUNITY_DECISION_LOOKBACK", "500") or 500))
_DISMISS_TTL_S = max(0.0, float(os.getenv("KAIT_OPPORTUNITY_DISMISS_TTL_S", str(7 * 24 * 3600)) or (7 * 24 * 3600)))
    ...
    try:
        ...
        for provider in chain:
            ...
            if not raw:
                ...
                # KAIT_OPPORTUNITY_LLM_TIMEOUT_S or switch providers.
        ...
        rows.append(
            {
                ...
                "priority": "high",
                "confidence": 0.81,
                "question": "What exact outcome marks done, and how will Kait verify it?",
        ...
        rows.append(
            {
                ...
                "priority": "medium",
                "confidence": 0.79,
                "question": "What reusable learning from this task should Kait promote for future work?",
...
def scan_runtime_opportunities(
    ...
    *,
    ...
    kait_stats = stats if isinstance(stats, dict) else {}
    prompts: List[str] = []
    edits: List[str] = []
    telemetry_filtered = 0
    scope_hint = None
        ...
        rows.append(
            {
                ...
                "next_step": "Capture one reusable pattern and where Kait should apply it next.",
