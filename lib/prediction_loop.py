...
PREDICTIONS_FILE = Path.home() / ".kait" / "predictions.jsonl"
STATE_FILE = Path.home() / ".kait" / "prediction_state.json"
...
def _resolve_namespace(*values: Optional[str]) -> str:
    forced = str(os.environ.get("KAIT_NAMESPACE", "")).strip().lower()
...
def _load_prediction_budget_config() -> Tuple[int, int, Dict[str, int]]:
    ...
    raw_total = os.environ.get("KAIT_PREDICTION_TOTAL_BUDGET")
    if raw_total:
        try:
    ...
    raw_default = os.environ.get("KAIT_PREDICTION_DEFAULT_SOURCE_BUDGET")
    if raw_default:
        try:
    ...
    raw = os.environ.get("KAIT_PREDICTION_SOURCE_BUDGETS", "").strip()
    if not raw:
...
def _load_auto_link_config() -> Tuple[bool, float, int, float]:
    enabled = str(os.environ.get("KAIT_PREDICTION_AUTO_LINK", "1")).strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
    ...
    raw_interval = os.environ.get("KAIT_PREDICTION_AUTO_LINK_INTERVAL_S")
    if raw_interval:
        try:
    ...
    raw_limit = os.environ.get("KAIT_PREDICTION_AUTO_LINK_LIMIT")
    if raw_limit:
        try:
    ...
    raw_sim = os.environ.get("KAIT_PREDICTION_AUTO_LINK_MIN_SIM")
    if raw_sim:
        try:
