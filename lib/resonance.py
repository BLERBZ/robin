#!/usr/bin/env python3
"""
Kait Resonance - Connection depth evolution

The kait that grows brighter as resonance deepens.

States:
  ✦ Kait    (0-25%)   - Just met. Learning signals.
...
class ResonanceCalculator:
    """
    Calculates resonance level based on Kait's learning state.
    ...
    def calculate(
        ...
        """
        Calculate resonance score from Kait metrics.
...
def calculate_current_resonance() -> ResonanceScore:
    """Calculate resonance from current Kait state."""
...
def _get_next_state(current: ResonanceState) -> Optional[Dict[str, Any]]:
    ...
    order = [ResonanceState.KAIT, ResonanceState.PULSE, ResonanceState.BLAZE, ResonanceState.RADIANT]
