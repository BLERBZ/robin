"""Carmack KPI scorecard helpers for Kait runtime reviews."""
...
ADVISORY_LOG = KAIT_DIR / "advisory_engine.jsonl"
ADVICE_FEEDBACK_REQUESTS = KAIT_DIR / "advice_feedback_requests.jsonl"
EFFECTIVENESS_FILE = KAIT_DIR / "advisor" / "effectiveness.json"
SYNC_STATS_FILE = KAIT_DIR / "sync_stats.json"
CHIP_MERGE_FILE = KAIT_DIR / "chip_merge_state.json"
...
def _service_effective_running(name: str, svc: Dict[str, Any]) -> bool:
    ...
    if name in {"kaitd", "dashboard", "pulse", "meta_ralph"} and bool(svc.get("healthy")):
        return True
...
def _core_reliability(status: Dict[str, Any]) -> Dict[str, Any]:
    core_keys = ("kaitd", "bridge_worker", "scheduler", "watchdog")
    running = 0
