...
def inject_agent_context(
    ...
    *,
    ...
    """Optionally prepend Kait context to an agent prompt.

    Enable via KAIT_AGENT_INJECT=1 to avoid unexpected prompt bloat.
    """
    if not prompt:
        return prompt
    if not _env_truthy("KAIT_AGENT_INJECT"):
        return prompt

    # Backward-compatible env handling:
    # - KAIT_AGENT_CONTEXT_MAX_CHARS is canonical for char budget
    # - KAIT_AGENT_CONTEXT_LIMIT is accepted as alias for char budget
    # - KAIT_AGENT_CONTEXT_ITEM_LIMIT controls number of compact context items
    max_chars_raw = (
        os.environ.get("KAIT_AGENT_CONTEXT_MAX_CHARS", "").strip()
        or os.environ.get("KAIT_AGENT_CONTEXT_LIMIT", "").strip()
    )
    try:
    ...
    item_limit_raw = os.environ.get("KAIT_AGENT_CONTEXT_ITEM_LIMIT", "").strip()
    if item_limit_raw:
        try:
...
class KaitOrchestrator:
    def __init__(self, root_dir: Optional[Path] = None):
        self.root_dir = root_dir or (Path.home() / ".kait" / "orchestration")
    ...
    def record_handoff(self, from_agent: str, to_agent: str, context: Dict, success: Optional[bool] = None) -> str:
        ...
        if isinstance(prompt, str):
            ...
            if injected != prompt:
                ctx["prompt"] = injected
                ctx["kait_context_injected"] = True
...
# Singleton helpers
_orch: Optional[KaitOrchestrator] = None


def get_orchestrator() -> KaitOrchestrator:
    global _orch
    if _orch is None:
        _orch = KaitOrchestrator()
    return _orch
