"""
DEPTH Trainer -- Socratic reasoning gym for Kait Intelligence.

An autonomous self-learning loop that:
1. Discovers what topics Kait needs to train on (self-search)
...
Usage:
    ...
    python -m lib.depth_trainer --ingest-all-opus          # Ingest all from ~/.kait/depth_opus_sessions.jsonl
...
log = logging.getLogger("kait.depth_trainer")
...
# DeepSeek NEVER sees: Kait internals, scores, knowledge, strategies
# ──────────────────────────────────────────────

# Sanitized prompt — NO Kait/Vibeship references, NO training context
...
_DEEPSEEK_LOG_PATH = Path.home() / ".kait" / "logs" / "deepseek_calls.jsonl"
...
_ALLOWED_PROMPT_FIELDS = {
    ...
    "approach_guidance",  # Sanitized advisory hints (no Kait internals)
}
...
class DepthAnswerGenerator:
    ...
    with zero Kait/Vibeship context. Responses are treated as untrusted text.
    """
    ...
    async def _call_ollama(self, prompt: str, timeout: float, depth: int) -> Optional[str]:
        """Call local Ollama. Same sanitized prompt, no Kait context."""
...
TRAINING_LOG = KAIT_DIR / "depth_training.jsonl"
KNOWLEDGE_BASE = KAIT_DIR / "depth_knowledge.json"
TOPIC_QUEUE = KAIT_DIR / "depth_topic_queue.json"
STRATEGY_MEMORY = KAIT_DIR / "depth_strategies.json"
BENCHMARK_LOG = KAIT_DIR / "depth_benchmarks.jsonl"
GAPS_FILE = KAIT_DIR / "depth_gaps.json"
GOLDEN_ANSWERS_FILE = KAIT_DIR / "depth_golden_answers.json"
...
class KnowledgeBase:
    ...
    def _save(self):
        KAIT_DIR.mkdir(parents=True, exist_ok=True)
...
class TopicDiscovery:
    """Discovers what topics Kait needs to train on next.
...
ANSWER_PROMPT_VIBE = """You are Kait, a self-evolving AI system training on engineering reasoning.
...
ANSWER_PROMPT_CLASSIC = """You are Kait, a self-evolving AI reasoning system engaged in Socratic dialogue.
...
async def _generate_answer(
    ...
    # --- DeepSeek path: sanitized prompt, zero Kait context ---
...
async def run_training_session(
    ...
    """Run Kait through a full DEPTH session (10 classic or 15 vibe levels)."""
...
# ======================================================
# Integration: Feed results through Ralph + Kait systems
# ======================================================
...
OPUS_SESSIONS_FILE = KAIT_DIR / "depth_opus_sessions.jsonl"
...
async def train(
    ...
    """Run a full DEPTH training session with Ralph loop + Kait integration."""
    ...
    if integrate:
        if verbose:
            _safe_print("  Integrating into Kait systems...")
...
async def run_autonomous_loop(
    ...
    if verbose:
        _safe_print("\n" + "=" * 60)
        _safe_print("  KAIT DEPTH TRAINING -- AUTONOMOUS MODE")
...
def print_dashboard():
    ...
    _safe_print(f"\n{'='*60}")
    _safe_print(f"  KAIT DEPTH TRAINING DASHBOARD")
    _safe_print(f"{'='*60}")
...
async def train_self(verbose: bool = True, forge_score: bool = False) -> TrainingResult:
    """Run DEPTH on Kait's own learning -- meta self-assessment."""
    kb = KnowledgeBase()
    strategy_mem = StrategyMemory()
    return await train("how Kait learns and improves itself", verbose=verbose, kb=kb, strategy_mem=strategy_mem, forge_score=forge_score)
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        for q_data in questions:
            ...
            try:
                resp = await client.post(
                    f"{DEPTH_API}/api/descend",
                    json={"topic": test_topic, "agent_name": "kait-ab-test",
                          "domain": domain, "mode": "vibe"},
                )
...
async def _main():
    ...
    parser = argparse.ArgumentParser(
        description="DEPTH Trainer -- Engineering reasoning gym for Kait Intelligence"
    )
    ...
    parser.add_argument("--no-integrate", action="store_true", help="Skip Kait integration")
    ...
    parser.add_argument("--ingest-all-opus", action="store_true",
                        help="Ingest all from ~/.kait/depth_opus_sessions.jsonl")
