...
def _lookup_llm_api_key(provider: str) -> Optional[str]:
    ...
    if p == "minimax":
        return (
            os.getenv("KAIT_MINIMAX_API_KEY")
            or os.getenv("MINIMAX_API_KEY")
            or os.getenv("KAIT_MINIMAX_TOKEN")
        )
    if p == "openai":
        return os.getenv("OPENAI_API_KEY") or os.getenv("KAIT_OPENAI_API_KEY")
    if p == "anthropic":
        return (
            os.getenv("ANTHROPIC_API_KEY")
            or os.getenv("KAIT_ANTHROPIC_API_KEY")
...
def _lookup_llm_url(provider: str) -> str:
    p = str(provider or "").strip().lower()
    if p == "ollama":
        return str(os.getenv("KAIT_OLLAMA_API", "http://localhost:11434")).rstrip("/")
    if p == "minimax":
        return str(os.getenv("KAIT_MINIMAX_BASE_URL", PACKET_LOOKUP_LLM_URL)).rstrip("/")
    if p == "openai":
        return str(os.getenv("KAIT_OPENAI_BASE_URL", "https://api.openai.com")).rstrip("/")
    if p == "anthropic":
        return str(os.getenv("KAIT_ANTHROPIC_BASE_URL", "https://api.anthropic.com")).rstrip("/")
    if p == "gemini":
        return str(os.getenv("KAIT_GEMINI_BASE_URL", "https://generativelanguage.googleapis.com")).rstrip("/")
