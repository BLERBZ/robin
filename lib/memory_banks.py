...
- Compatible everywhere: driven by Kait queue + KaitEventV1 payloads
...
def _emotion_write_capture_enabled() -> bool:
    env = os.getenv("KAIT_MEMORY_EMOTION_WRITE_CAPTURE")
    if env is not None:
...
def store_memory(text: str, category: str, session_id: Optional[str] = None, source: str = "kait") -> Optional[BankEntry]:
    _ensure_dirs()
