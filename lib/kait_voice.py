...
VOICE_FILE = KAIT_DIR / "voice.json"


@dataclass
class Opinion:
    """Something Kait has an opinion about."""
    topic: str
    preference: str
    reason: str
...
@dataclass
class GrowthMoment:
    """A moment where Kait realized it grew."""
...
class KaitVoice:
    """
    Gives Kait a consistent voice and personality.
...
# Singleton
_voice: Optional[KaitVoice] = None

def get_kait_voice() -> KaitVoice:
    global _voice
    if _voice is None:
        _voice = KaitVoice()
    return _voice
...
get_kait_voice = get_kait_voice

# Legacy alias for KAIT_DIR
KAIT_DIR = KAIT_DIR
