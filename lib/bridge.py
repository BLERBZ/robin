#!/usr/bin/env python3
"""
Kait Bridge - Connect learnings to operational context
...
# Paths
KAIT_DIR = Path(__file__).parent.parent
WORKSPACE = Path(os.environ.get("KAIT_WORKSPACE", str(Path.home() / "clawd"))).expanduser()
MEMORY_FILE = WORKSPACE / "MEMORY.md"
KAIT_CONTEXT_FILE = WORKSPACE / "KAIT_CONTEXT.md"
...
def get_strong_opinions() -> List[Dict]:
    ...
    from lib.kait_voice import KaitVoice
...
def infer_current_focus(max_events: int = 25) -> str:
    """Infer current focus from recent Kait events.

    Works across runtimes as long as events are being ingested into Kait.
    """
    try:
        ...
        for e in reversed(events):
            if e.event_type == EventType.USER_PROMPT:
                # kaitd stores structured data under data.payload
...
def _recognition_snippet(query: str, contextual: List[Dict[str, Any]], lessons: List[Dict[str, Any]]) -> str:
    ...
    This is designed to make Kait feel *present* without being creepy.
    Keep it:
    - 0-2 sentences
...
def generate_active_context(query: Optional[str] = None) -> str:
    ...
    if contextual:
        try:
            ...
            record_exposures("kait_context:contextual", exposures, session_id=session_id, trace_id=trace_id)
        except Exception:
            pass
    ...
    if skills:
        try:
            ...
            record_exposures("kait_context:skills", exposures, session_id=session_id, trace_id=trace_id)
        except Exception:
            pass
    ...
    # Opportunity Scanner: self-Socratic path (Kait questions itself)
    self_ops = []
    try:
    ...
    if self_ops:
        ...
        for row in self_ops[:2]:
            ...
            if not question:
                continue
            if next_step:
                lines.append(f"- [Kait] {question} -> {next_step}")
            else:
                lines.append(f"- [Kait] {question}")
        lines.append("")
...
def update_kait_context(query: Optional[str] = None):
    """Update KAIT_CONTEXT.md with active, useful context.

    If query is provided, we tailor the context to that task.
    """
    context = generate_active_context(query=query)
    KAIT_CONTEXT_FILE.write_text(context, encoding="utf-8")
    return context


def inject_to_memory(insight: str, category: str = "kait") -> bool:
    ...
    # Find or create Kait Learnings section
    marker = "## Kait Learnings"
...
def bridge_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "context_file": str(KAIT_CONTEXT_FILE),
        "memory_file": str(MEMORY_FILE),
        "context_exists": KAIT_CONTEXT_FILE.exists(),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "context":
            ...
            update_kait_context()
            print("OK Updated KAIT_CONTEXT.md")
