...
def _load_sync_adapter_policy() -> Dict[str, Any]:
    mode = str(os.getenv("KAIT_SYNC_MODE", "") or "").strip().lower()
    env_targets = _parse_adapter_list(os.getenv("KAIT_SYNC_TARGETS", ""))
    env_disabled = set(_parse_adapter_list(os.getenv("KAIT_SYNC_DISABLE_TARGETS", "")))
    cfg: Dict[str, Any] = {}
    ...
    elif os.getenv("KAIT_CODEX_CMD") or os.getenv("CODEX_CMD"):
...
def _build_advisory_payload(
    *,
    ...
    payload: Dict[str, Any] = {
        "schema_version": "kait_advisory_payload_v1",
...
def _load_promoted_lines(project_dir: Path) -> List[str]:
    ...
    for name in ("CLAUDE.md", "AGENTS.md", "TOOLS.md", "SOUL.md"):
        ...
        try:
            ...
            continue
        idx = content.find("## Kait Learnings")
        if idx == -1:
            continue
        tail = content[idx + len("## Kait Learnings") :]
...
def _format_context(
    ...
    lines = [
        "## Kait Bootstrap",
        "Auto-loaded high-confidence learnings from ~/.kait/cognitive_insights.json",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap = argparse.ArgumentParser(description="Sync Kait bootstrap context to platforms")
