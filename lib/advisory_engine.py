...
ENGINE_ENABLED = os.getenv("KAIT_ADVISORY_ENGINE", "1") != "0"
ENGINE_LOG = Path.home() / ".kait" / "advisory_engine.jsonl"
ENGINE_LOG_MAX = 500
ADVISORY_DECISION_LEDGER_FILE = Path.home() / ".kait" / "advisory_decision_ledger.jsonl"
ADVISORY_DECISION_LEDGER_ENABLED = True
ADVISORY_DECISION_LEDGER_MAX = 1500
MAX_ENGINE_MS = float(os.getenv("KAIT_ADVISORY_MAX_MS", "4000"))
INCLUDE_MIND_IN_MEMORY = os.getenv("KAIT_ADVISORY_INCLUDE_MIND", "0") == "1"
ENABLE_PREFETCH_QUEUE = os.getenv("KAIT_ADVISORY_PREFETCH_QUEUE", "1") != "0"
ENABLE_INLINE_PREFETCH_WORKER = os.getenv("KAIT_ADVISORY_PREFETCH_INLINE", "1") != "0"
PACKET_FALLBACK_EMIT_ENABLED = os.getenv("KAIT_ADVISORY_PACKET_FALLBACK_EMIT", "0") == "1"
...
LIVE_QUICK_FALLBACK_ENABLED = os.getenv("KAIT_ADVISORY_LIVE_QUICK_FALLBACK", "0") == "1"
LIVE_QUICK_FALLBACK_MIN_REMAINING_MS = float(
    os.getenv("KAIT_ADVISORY_LIVE_QUICK_MIN_REMAINING_MS", "900")
)

FALLBACK_RATE_GUARD_ENABLED = os.getenv("KAIT_ADVISORY_FALLBACK_RATE_GUARD", "1") != "0"
FALLBACK_RATE_GUARD_MAX_RATIO = float(
    os.getenv("KAIT_ADVISORY_FALLBACK_RATE_MAX_RATIO", "0.55")
)
try:
    FALLBACK_RATE_GUARD_WINDOW = max(
        10, int(os.getenv("KAIT_ADVISORY_FALLBACK_RATE_WINDOW", "80") or 80)
    )
except Exception:
    FALLBACK_RATE_GUARD_WINDOW = 80
MEMORY_SCOPE_DEFAULT = str(os.getenv("KAIT_MEMORY_SCOPE_DEFAULT", "session") or "session").strip() or "session"
ACTIONABILITY_ENFORCE = os.getenv("KAIT_ADVISORY_REQUIRE_ACTION", "1") != "0"

# Action-first formatting: move the actionable "Next check" command to the first line.
ACTION_FIRST_ENABLED = os.getenv("KAIT_ADVISORY_ACTION_FIRST", "0") == "1"
...
FORCE_PROGRAMMATIC_SYNTH = os.getenv("KAIT_ADVISORY_FORCE_PROGRAMMATIC_SYNTH", "1") == "1"
...
SELECTIVE_AI_SYNTH_ENABLED = os.getenv("KAIT_ADVISORY_SELECTIVE_AI_SYNTH", "1") == "1"
SELECTIVE_AI_MIN_REMAINING_MS = float(
    os.getenv("KAIT_ADVISORY_SELECTIVE_AI_MIN_REMAINING_MS", "1800")
)
SELECTIVE_AI_MIN_AUTHORITY = str(
    os.getenv("KAIT_ADVISORY_SELECTIVE_AI_MIN_AUTHORITY", "note") or "note"
).strip().lower()
...
SESSION_KEY_INCLUDE_RECENT_TOOLS = (
    os.getenv("KAIT_ADVISORY_SESSION_KEY_INCLUDE_RECENT_TOOLS", "0") == "1"
)

DELIVERY_STALE_SECONDS = float(os.getenv("KAIT_ADVISORY_STALE_S", "900"))
ADVISORY_TEXT_REPEAT_COOLDOWN_S = float(
    os.getenv("KAIT_ADVISORY_TEXT_REPEAT_COOLDOWN_S", "600")
)
...
LOW_AUTH_GLOBAL_DEDUPE_ENABLED = (
    os.getenv(
        "KAIT_ADVISORY_LOW_AUTH_GLOBAL_DEDUPE",
        os.getenv("KAIT_ADVISORY_WHISPER_GLOBAL_DEDUPE", "1"),
    )
    != "0"
)
try:
    LOW_AUTH_GLOBAL_COOLDOWN_S = float(
        os.getenv(
            "KAIT_ADVISORY_LOW_AUTH_GLOBAL_COOLDOWN_S",
            os.getenv("KAIT_ADVISORY_WHISPER_GLOBAL_COOLDOWN_S", "600"),
        )
    )
except Exception:
    LOW_AUTH_GLOBAL_COOLDOWN_S = 600.0
LOW_AUTH_DEDUPE_LOG = Path.home() / ".kait" / "advisory_low_auth_dedupe.jsonl"
...
GLOBAL_DEDUPE_ENABLED = os.getenv("KAIT_ADVISORY_GLOBAL_DEDUPE", "1") != "0"
GLOBAL_DEDUPE_TEXT_ENABLED = os.getenv("KAIT_ADVISORY_GLOBAL_DEDUPE_BY_TEXT", "1") != "0"
try:
    GLOBAL_DEDUPE_COOLDOWN_S = float(os.getenv("KAIT_ADVISORY_GLOBAL_DEDUPE_COOLDOWN_S", "600"))
except Exception:
    GLOBAL_DEDUPE_COOLDOWN_S = 600.0
GLOBAL_DEDUPE_LOG = Path.home() / ".kait" / "advisory_global_dedupe.jsonl"
GLOBAL_DEDUPE_LOG_MAX = 5000
# Dedupe scope: global (all sessions) or tree (main + subagents under same agent tree).
GLOBAL_DEDUPE_SCOPE = str(os.getenv("KAIT_ADVISORY_GLOBAL_DEDUPE_SCOPE", "global") or "global").strip().lower()

# (pytest hygiene handled in *_recently_emitted helpers)

try:
    INLINE_PREFETCH_MAX_JOBS = max(
        1, int(os.getenv("KAIT_ADVISORY_PREFETCH_INLINE_MAX_JOBS", "1") or 1)
    )
except Exception:
...
def _low_auth_recently_emitted(
    *,
    ...
    # Keep tests hermetic: don't consult the user's real ~/.kait dedupe logs.
    # (Allow tests that monkeypatch the log path to still exercise the logic.)
    if os.getenv("PYTEST_CURRENT_TEST"):
        try:
            default_log = (Path.home() / ".kait" / "advisory_low_auth_dedupe.jsonl").resolve()
...
def _global_recently_emitted(
    *,
    ...
    if os.getenv("PYTEST_CURRENT_TEST"):
        try:
            default_log = (Path.home() / ".kait" / "advisory_global_dedupe.jsonl").resolve()
...
def _load_engine_config(path: Optional[Path] = None) -> Dict[str, Any]:
    """Load advisory engine tuneables from ~/.kait/tuneables.json."""
    tuneables = path or (Path.home() / ".kait" / "tuneables.json")
    if (
        path is None
        and "pytest" in sys.modules
        and str(os.environ.get("KAIT_TEST_ALLOW_HOME_TUNEABLES", "")).strip().lower()
        not in {"1", "true", "yes", "on"}
    ):
        try:
            if tuneables.resolve() == (Path.home() / ".kait" / "tuneables.json").resolve():
                return {}
        except Exception:
            return {}
...
def on_pre_tool(
    ...
    try:
        ...
        if not resolved_trace_id:
            ...
            resolved_trace_id = f"kait-auto-{session_id[:16]}-{tool_name.lower()}-{int(time.time()*1000)}"
...
def on_user_prompt(
    ...
    try:
        ...
        resolved_trace_id = str(trace_id or "").strip() or f"kait-auto-{session_id[:16]}-user_prompt-{int(time.time()*1000)}"
        intent_info = _intent_context(state, tool_name="*")
        project_key = _project_key()
        session_context_key = _session_context_key(state, tool_name="*")
        intent_family = state.intent_family or "emergent_other"
