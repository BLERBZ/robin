...
class HighRiskToolUseGuard:
    ...
    def __init__(self):
        self.enabled = str(os.environ.get("KAIT_SAFETY_GUARDRAILS", "1")).strip() != "0"
        self.allow_secrets = str(os.environ.get("KAIT_SAFETY_ALLOW_SECRETS", "0")).strip() == "1"
            ...
            if path and self._looks_like_secret_path(path):
                if not self.allow_secrets:
                    return GuardrailResult(
                        passed=False,
                        violation=ViolationType.HIGH_RISK_TOOL_USE,
                        message="Blocked likely-secret file access (set KAIT_SAFETY_ALLOW_SECRETS=1 to override).",
