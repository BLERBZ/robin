"""
EIDOS Migration: Transition from Current Kait to EIDOS
...
def migrate_cognitive_insights(
    ...
    Args:
        insights_path: Path to cognitive_insights.json (defaults to ~/.kait/)
    ...
    if insights_path is None:
        insights_path = Path.home() / ".kait" / "cognitive_insights.json"
...
def archive_patterns(
    ...
    if patterns_path is None:
        patterns_path = Path.home() / ".kait" / "detected_patterns.jsonl"

    if archive_dir is None:
        archive_dir = Path.home() / ".kait" / "archive"
...
def run_full_migration(dry_run: bool = False) -> Dict[str, Any]:
    """
    Run the complete migration from old Kait to EIDOS.
...
def _backup_original_files():
    ...
    backup_dir = kait_dir / "backup_pre_eidos"
...
def validate_migration() -> Dict[str, Any]:
    ...
    # Check backup exists
    backup_dir = Path.home() / ".kait" / "backup_pre_eidos"
    results["backup_exists"] = backup_dir.exists()

    # Check archive exists
    archive_path = Path.home() / ".kait" / "archive" / "patterns_pre_eidos.jsonl"
