"""Production loop gates for Kait iteration cycles.
...
COGNITIVE_FILE = KAIT_DIR / "cognitive_insights.json"
EFFECTIVENESS_FILE = KAIT_DIR / "advisor" / "effectiveness.json"
CHIP_INSIGHTS_DIR = KAIT_DIR / "chip_insights"
...
def _load_loop_thresholds_from_tuneables() -> LoopThresholds:
    """Load production gate thresholds from ~/.kait/tuneables.json when present."""
    default = LoopThresholds()
    data = _read_json(KAIT_DIR / "tuneables.json", {})
...
def format_gate_report(metrics: LoopMetrics, result: Dict[str, Any]) -> str:
    ...
    lines = []
    lines.append("=" * 66)
    lines.append(" KAIT PRODUCTION LOOP GATES")
    lines.append("=" * 66)
    lines.append(
