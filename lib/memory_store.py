...
DB_PATH = Path.home() / ".kait" / "memory_store.sqlite"
_FTS_AVAILABLE: Optional[bool] = None
TUNEABLES_FILE = Path.home() / ".kait" / "tuneables.json"

# Phase 2: patchified (chunked) memory storage
PATCHIFIED_ENABLED = os.getenv("KAIT_MEMORY_PATCHIFIED", "0") == "1"

# Phase 2: delta memory compaction (store updates as deltas when near-duplicate)
DELTAS_ENABLED = os.getenv("KAIT_MEMORY_DELTAS", "0") == "1"
try:
    DELTA_MIN_SIMILARITY = float(os.getenv("KAIT_MEMORY_DELTA_MIN_SIM", "0.86"))
except Exception:
    DELTA_MIN_SIMILARITY = 0.86
DELTAS_WINDOW_DAYS = 30

try:
    PATCH_MAX_CHARS = max(120, min(2000, int(os.getenv("KAIT_MEMORY_PATCH_MAX_CHARS", "600") or 600)))
except Exception:
    PATCH_MAX_CHARS = 600
try:
    PATCH_MIN_CHARS = max(40, min(400, int(os.getenv("KAIT_MEMORY_PATCH_MIN_CHARS", "120") or 120)))
except Exception:
    PATCH_MIN_CHARS = 120
...
def _tuneables_read_allowed() -> bool:
    if "pytest" not in sys.modules:
        return True
    if str(os.environ.get("KAIT_TEST_ALLOW_HOME_TUNEABLES", "")).strip().lower() in {"1", "true", "yes", "on"}:
        return True
    try:
        return TUNEABLES_FILE.resolve() != (Path.home() / ".kait" / "tuneables.json").resolve()
    except Exception:
        return False
...
def _load_memory_learning_config(*, force: bool = False) -> Dict[str, Any]:
    ...
    env_enabled = os.getenv("KAIT_MEMORY_LEARNING_ENABLED")
    if env_enabled is not None:
        cfg["enabled"] = _safe_bool(env_enabled, cfg["enabled"])
    env_weight = os.getenv("KAIT_MEMORY_LEARNING_WEIGHT")
    if env_weight is not None:
...
def _load_memory_retrieval_guard_config(*, force: bool = False) -> Dict[str, Any]:
    ...
    env_enabled = os.getenv("KAIT_MEMORY_RETRIEVAL_GUARD_ENABLED")
    if env_enabled is not None:
        cfg["enabled"] = _safe_bool(env_enabled, cfg["enabled"])
