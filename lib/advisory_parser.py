...
REQUESTS_FILE = Path.home() / ".kait" / "advice_feedback_requests.jsonl"
KAIT_ADVISORY_FILE = Path.home() / ".openclaw" / "workspace" / "KAIT_ADVISORY.md"
KAIT_ADVISORY_FALLBACK_FILE = Path.home() / ".kait" / "llm_advisory.md"
ENGINE_FILE = Path.home() / ".kait" / "advisory_engine.jsonl"
...
def normalize_recommendation(text: str) -> str:
    ...
    text = re.sub(r"^\[KAIT(?: ADVISORY)?\]\s*", "", text, flags=re.IGNORECASE)
    text = re.sub(r"^\(kait:\s*", "", text, flags=re.IGNORECASE)
...
def load_advisories(
    *,
    ...
    if advisory_paths is None:
        ...
        advisory_paths = discovered or [KAIT_ADVISORY_FILE]
        advisory_paths = [*advisory_paths, KAIT_ADVISORY_FALLBACK_FILE]
