...
from .config import ObservatoryConfig, kait_dir
...
def _export_advisory(explore_dir: Path, advice_limit: int) -> int:
    ...
    index.append(f"| Cognitive helpful rate | {metrics.get('cognitive_helpful_rate') or 0:.1%} |")
    index.append("")
...
def _generate_explore_index(explore_dir: Path, counts: dict[str, int], cfg: ObservatoryConfig) -> None:
    ...
    index = [_frontmatter({"type": "kait-explorer-index"})]
    index.append(f"# Explore Kait Intelligence\n")
    ...
    index.append("All limits are configurable in `~/.kait/tuneables.json` under the `observatory` section:\n")
    index.append("```json")
