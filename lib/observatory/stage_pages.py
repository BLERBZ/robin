...
def _source_files(lib_path: str, state_files: list[str]) -> str:
    ...
    for sf in state_files:
        lines.append(f"- `~/.kait/{sf}` — State storage")
    lines.append("")
    return "\n".join(lines)
...
def _gen_tuneables(d: dict, all_data: dict) -> str:
    s = _header(12, "Tuneables", "Central configuration for all pipeline stages. Supports hot-reload. Available in both runtime (~/.kait/tuneables.json) and version-controlled (config/tuneables.json) locations.", [], [])
