"""LLM interface for Kait Intelligence.
...
def _call_claude_windows(
    ...
    prompt_file = kait_dir / "llm_prompt.txt"
    response_file = kait_dir / "llm_response.txt"
    ...
    if system_prompt:
        sys_file = kait_dir / "llm_system.txt"
...
def synthesize_advisory(
    ...
    This is the key function that makes Kait's learnings actually useful.
    """
    ...
    prompt = f"""You are Kait Intelligence, observing a live coding session on {project_name}.

SYSTEM INVENTORY (what actually exists — do NOT reference anything outside this list):
- Services: kaitd (port 8787, HTTP event ingestion), bridge_worker (background processing), openclaw_tailer (captures OpenClaw sessions), Kait Pulse dashboard (port 8765, uvicorn)
...
def _ask_distillation_model(prompt: str, *, timeout_s: int = 45) -> Optional[str]:
    preferred = (os.getenv("KAIT_EIDOS_PROVIDER") or "minimax").strip().lower()
...
def _normalize_distillation_payload(raw: str) -> Optional[Dict[str, Any]]:
    ...
    return {
        "schema": "kait.eidos.v1",
        "insights": final[:3],
        "meta": {
            "provider": (os.getenv("KAIT_EIDOS_PROVIDER") or "minimax").strip().lower(),
            "count": len(final[:3]),
        },
    }
...
def distill_eidos(
    ...
    Returns JSON string with schema kait.eidos.v1 and insight entries ready
    ...
    prompt = f"""You are distilling Kait behavior observations into a structured advisory-ready format.
