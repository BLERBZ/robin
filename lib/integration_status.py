"""
Integration Status Checker for Kait Intelligence

Verifies that Kait is properly integrated with Claude Code.
...
KAIT_HOOKS_FILE = CLAUDE_DIR / "kait-hooks.json"
QUEUE_DIR = KAIT_DIR / "queue"
EVENTS_FILE = QUEUE_DIR / "events.jsonl"
ADVICE_LOG = KAIT_DIR / "advisor" / "advice_log.jsonl"
RECENT_ADVICE = KAIT_DIR / "advisor" / "recent_advice.jsonl"
EFFECTIVENESS = KAIT_DIR / "advisor" / "effectiveness.json"
CODEX_CONTEXT_FILE = Path("KAIT_CONTEXT_FOR_CODEX.md")
CODEX_PAYLOAD_FILE = Path("KAIT_ADVISORY_PAYLOAD.json")


def check_settings_json() -> Tuple[bool, str]:
    """Check if ~/.claude/settings.json exists with Kait hooks."""
...
def _codex_sync_enabled() -> bool:
    if os.getenv("KAIT_CODEX_CMD") or os.getenv("CODEX_CMD"):
        return True
    sync_targets = os.getenv("KAIT_SYNC_TARGETS", "").strip().lower()
    if not sync_targets:
        return False
...
def check_codex_sync_outputs() -> Tuple[bool, str]:
    ...
    try:
        ...
        if not isinstance(payload, dict):
            return False, "KAIT_ADVISORY_PAYLOAD.json is not a JSON object"
        if not payload.get("schema_version"):
            return False, "KAIT_ADVISORY_PAYLOAD.json missing schema_version"
    except Exception as e:
        return False, f"Error reading KAIT_ADVISORY_PAYLOAD.json: {e}"

    age_hours = (time.time() - context_path.stat().st_mtime) / 3600
    if age_hours > 24:
        return False, (
            f"Codex context stale ({age_hours:.1f}h ago); run `kait-codex` or "
...
def print_status():
    ...
    print("\n" + "=" * 60)
    print("  KAIT INTELLIGENCE - INTEGRATION STATUS")
    print("=" * 60)
  ...
  1. Create ~/.claude/settings.json with:
     {
       "hooks": {
         "PreToolUse": [{"matcher":"","hooks":[{"type":"command",
           "command":"python /path/to/kait/hooks/observe.py"}]}],
         "PostToolUse": [...same...],
...
""")
  ...
  - Advisory packet store is not healthy. Recommended checks:
    ...
    d) Inspect ~/.kait/advice_packets/index.json for unexpected corruption
""")
  ...
  - Codex sync is disabled in this run context. Enable if you want this check:
    a) Launch with `kait-codex` wrapper, or
    b) Set KAIT_SYNC_TARGETS=codex
    c) Or set KAIT_CODEX_CMD / CODEX_CMD
""")
                    else:
                        print("""
  - Codex sync artifacts missing or stale. Recommended checks:
    a) Run `python -m kait.cli sync-context` in this directory
    b) Ensure KAIT_SYNC_TARGETS includes `codex`
    c) Verify `KAIT_CONTEXT_FOR_CODEX.md` and `KAIT_ADVISORY_PAYLOAD.json` exist
