...
from lib.bridge import update_kait_context
...
BRIDGE_HEARTBEAT_FILE = Path.home() / ".kait" / "bridge_worker_heartbeat.json"

# --- OpenClaw notification integration ---
KAIT_OPENCLAW_NOTIFY = os.environ.get("KAIT_OPENCLAW_NOTIFY", "1").strip().lower() not in {
    "0", "false", "no", "off"
}
_NOTIFY_COOLDOWN_S = 300  # 5 minutes
_last_notify_time: float = 0.0
BRIDGE_STEP_TIMEOUT_S = float(os.environ.get("KAIT_BRIDGE_STEP_TIMEOUT_S", "45"))
BRIDGE_DISABLE_TIMEOUTS = os.environ.get("KAIT_BRIDGE_DISABLE_TIMEOUTS", "0").strip().lower() in {
    "1", "true", "yes", "on"
}
...
# Default: collect every 3 cycles. Set KAIT_BRIDGE_GC_EVERY=1 to restore
# previous behavior.
try:
    _BRIDGE_GC_EVERY = int(os.environ.get("KAIT_BRIDGE_GC_EVERY", "3"))
except Exception:
    _BRIDGE_GC_EVERY = 3
...
def _chips_disabled() -> bool:
    return str(os.environ.get("KAIT_ADVISORY_DISABLE_CHIPS", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _chips_enabled() -> bool:
    ...
    if not _premium_tools_enabled():
        return False
    return str(os.environ.get("KAIT_CHIPS_ENABLED", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
CHIP_MERGE_MIN_CONFIDENCE = _env_float("KAIT_CHIP_MERGE_MIN_CONFIDENCE", 0.55)
CHIP_MERGE_MIN_QUALITY = _env_float("KAIT_CHIP_MERGE_MIN_QUALITY", 0.55)
...
_STEP_EXECUTOR_WORKERS = max(2, int(os.environ.get("KAIT_BRIDGE_STEP_EXECUTOR_WORKERS", "4")))
...
def run_bridge_cycle(
    *,
    ...
    try:
        # --- Context update ---
        ok, _result, error = _run_step("context", update_kait_context, query=query)
        if ok:
        ...
        if ok:
            ...
            # enabling KAIT_DEBUG (stderr logs aren't always available on Windows services).
        ...
        if chips_enabled:
            # TODO: Filter chips by project context (game-dev shouldn't fire during kait-checker work)
            # See: kait_reports/day1_fixes_plan.md for details
                ...
                if recent_insights or pattern_summaries:
                    ...
                    if advisory:
                        ...
                        if pruned_advisory.strip():
                            ...
                            # Write advisory to KAIT_CONTEXT for agent consumption
        ...
        try:
            ...
            _eidos_counter_file = Path.home() / ".kait" / "eidos_llm_counter.txt"
            counter = 0
...
def _write_llm_advisory(advisory: str) -> None:
    """Write LLM-generated advisory to KAIT_CONTEXT supplement."""
    try:
        advisory_file = Path.home() / ".kait" / "llm_advisory.md"
        from datetime import datetime
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
        content = f"# Kait Advisory ({timestamp})\n\n{advisory}\n"
        advisory_file.write_text(content, encoding="utf-8")

        for workspace in discover_openclaw_workspaces(include_nonexistent=True):
            ctx_file = workspace / "KAIT_ADVISORY.md"
    ...
    try:
        ...
        if structured is not None:
            ...
            entry = {
                ...
                "schema": structured.get("schema") or "kait.eidos.v1",
                "insights": kept[:3],
...
def _maybe_notify_openclaw(stats: Dict[str, Any]) -> None:
    ...
    try:
        ...
        summary = "Kait bridge cycle: " + ", ".join(findings)
        notify_agent(summary, priority="normal")
        wake_agent(
            f"🔮 Kait found something — read KAIT_CONTEXT.md and KAIT_NOTIFICATIONS.md. Summary: {summary}"
        )
