...
def _resolve_workspace() -> Path:
    explicit = os.environ.get("KAIT_CLAWDBOT_WORKSPACE") or os.environ.get("CLAWDBOT_WORKSPACE")
    if explicit:
...
def _parse_targets() -> List[str]:
    raw = os.environ.get("KAIT_CLAWDBOT_TARGETS") or os.environ.get("CLAWDBOT_TARGETS")
    if raw:
        parts = [p.strip() for p in raw.split(",") if p.strip()]
        return parts

    # Default: USER.md (auto-injected) + KAIT_CONTEXT.md (ready for hook injection).
    return ["USER.md", "KAIT_CONTEXT.md"]


def _resolve_paths() -> List[Path]:
    explicit = os.environ.get("KAIT_CLAWDBOT_CONTEXT_PATH") or os.environ.get("CLAWDBOT_CONTEXT_PATH")
    if explicit:
        return [Path(explicit).expanduser()]
...
def _payload_path(paths: List[Path]) -> Optional[Path]:
    if not paths:
        return None
    first = paths[0]
    return Path(first).parent / "KAIT_ADVISORY_PAYLOAD.json"
