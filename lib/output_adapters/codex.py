"""Codex output adapter (`KAIT_CONTEXT_FOR_CODEX.md` + payload).
...
def write(
    ...
    path = root / "KAIT_CONTEXT_FOR_CODEX.md"
    ok = write_marked_section(path, context, create_header="# KAIT CODEX CONTEXT")
    if advisory_payload is not None:
        write_json(root / "KAIT_ADVISORY_PAYLOAD.json", advisory_payload)
    return ok
