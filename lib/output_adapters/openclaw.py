"""OpenClaw output adapter: write Kait context to ~/.openclaw/workspace/KAIT_CONTEXT.md."""
...
FALLBACK_ADVISORY_FILE = Path.home() / ".kait" / "llm_advisory.md"


def _resolve_workspaces() -> list[Path]:
    explicit = os.environ.get("KAIT_OPENCLAW_WORKSPACE") or os.environ.get("OPENCLAW_WORKSPACE")
    if explicit:
...
Write a JSON file to `~/.openclaw/workspace/kait_reports/` and Kait picks it up automatically.
...
def write(context: str, config: Optional[dict] = None, advisory_payload: Optional[dict] = None) -> bool:
    """Write Kait context into OpenClaw workspace as KAIT_CONTEXT.md.
    ...
    for workspace in _resolve_workspaces():
        path = workspace / "KAIT_CONTEXT.md"
        result = write_marked_section(
            path,
            formatted,
            create_header="# Kait Intelligence Context",
            marker_start="<!-- KAIT:BEGIN -->",
            marker_end="<!-- KAIT:END -->",
        )
        ...
        if FALLBACK_ADVISORY_FILE.exists():
            advisory_path = workspace / "KAIT_ADVISORY.md"
        ...
        if advisory_payload is not None:
            write_json(workspace / "KAIT_ADVISORY_PAYLOAD.json", advisory_payload)
