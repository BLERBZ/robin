...
Goal
----
Turn high-signal conversational statements into durable Kait learnings *without*
platform coupling.

Design constraints
------------------
- Works from normalized KaitEventV1 payloads stored in the existing queue.
...
- input: Kait queue events (KaitEvent)
...
def importance_score(text: str) -> Tuple[float, Dict[str, float]]:
    ...
    tech_hits = len(re.findall(
        ...
        r"dbt|Airflow|Kait|Kafka|RabbitMQ|"
...
def commit_learning(
    ...
    try:
        ...
        import logging
        logging.getLogger("kait.memory_capture").warning("commit_learning failed: %s", e)
        return False
...
def process_recent_memory_events(limit: int = 50) -> Dict[str, Any]:
    ...
    Portable rule: we only use data in Kait queue.
    ...
    for e in events:
        ...
        # We only understand KaitEventV1 shaped payloads via kaitd ingest
