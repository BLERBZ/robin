...
The key insight: without feedback, Kait's distillation rate stays at ~0.4%.
...
REPORTS_DIR = Path.home() / ".openclaw" / "workspace" / "kait_reports"
FEEDBACK_STATE_FILE = Path.home() / ".kait" / "feedback_state.json"
FEEDBACK_LOG_FILE = Path.home() / ".kait" / "feedback_log.jsonl"
...
def _process_decision(data: Dict, state: Dict) -> None:
    ...
    source = data.get("source", "")  # e.g., "kait_advisory"

    # If this decision was prompted by a Kait advisory, track it
    if source in ("kait_advisory", "kait_context", "kait"):
...
def report_advisory_feedback(
    ...
    Call this after reading and acting (or not) on KAIT_ADVISORY.md.
    """
