"""
Kait Promoter: Auto-promote high-value insights to project files
...
PROJECT_START = "<!-- KAIT_PROJECT_START -->"
PROJECT_END = "<!-- KAIT_PROJECT_END -->"
PROMOTION_LOG_FILE = Path.home() / ".kait" / "promotion_log.jsonl"
...
_AUTO_PROMOTED_LINE = "*Auto-promoted insights from Kait*"
...
*Auto-promoted insights from Kait*
...
"""
    ...
    def demote_stale_promotions(self) -> Dict[str, int]:
        ...
        for key, insight in list(cognitive.insights.items()):
            ...
            if target_file:
                ...
                removed = self._remove_from_section(file_path, "## Kait Learnings", insight.insight)
            else:
    ...
    def promote_project_profile(self, profile: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            print("[KAIT] Updated PROJECT.md from project profile")
            return True
        except Exception as e:
            print(f"[KAIT] PROJECT.md update failed: {e}")
            return False
    ...
    def get_promotable_insights(self, include_operational: bool = False) -> List[Tuple[CognitiveInsight, str, PromotionTarget]]:
        ...
        if fast_tracked:
            print(f"[KAIT] {fast_tracked} insights qualify via confidence fast-track")
        ...
        if not include_operational:
            ...
            if operational:
                print(f"[KAIT] Filtered {len(operational)} operational insights (telemetry)")
            safe_only, unsafe = filter_unsafe_insights(cognitive_only)
            if unsafe:
                print(f"[KAIT] Filtered {len(unsafe)} unsafe insights (safety guardrail)")
            return safe_only
    ...
    def promote_insight(self, insight: CognitiveInsight, insight_key: str, 
        ...
        try:
            ...
            print(f"[KAIT] Promoted to {target.filename}: {insight.insight[:50]}...")
            return True
            
        except Exception as e:
            print(f"[KAIT] Promotion failed: {e}")
            return False
    ...
    def promote_all(self, dry_run: bool = False, include_project: bool = True, include_chip_merge: bool = True) -> Dict[str, int]:
        ...
        if include_chip_merge and not dry_run:
            try:
                ...
                if chip_merge_stats.get("merged", 0) > 0:
                    print(
                        f"[KAIT] Merged {chip_merge_stats.get('merged', 0)} high-value chip insights "
                        f"into cognitive pipeline before promotion"
                    )
            except Exception as e:
                print(f"[KAIT] Chip merge pre-promotion failed: {e}")
        ...
        if not dry_run:
            try:
                ...
                if demotion_stats.get("demoted", 0) > 0:
                    print(
                        f"[KAIT] Demoted {demotion_stats.get('demoted', 0)} stale promotions "
                        f"({demotion_stats.get('doc_removed', 0)} removed from docs)"
                    )
            except Exception as e:
                print(f"[KAIT] Demotion pass failed: {e}")
        ...
        if not promotable:
            print("[KAIT] No insights ready for promotion")
            return stats

        print(f"[KAIT] Found {len(promotable)} insights ready for promotion")
