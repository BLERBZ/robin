"""
Kait Advisor: The Missing Link Between Learning and Action

This module closes the critical gap in Kait's architecture:
  Storage → Analysis → [ADVISOR] → Decision Impact

The Problem:
  - Kait captures insights beautifully (cognitive_learner, aha_tracker)
  - Kait stores them persistently (Mind sync, JSON files)
...
# Import existing Kait components
...
# ============= Configuration =============
ADVISOR_DIR = Path.home() / ".kait" / "advisor"
...
CHIP_INSIGHTS_DIR = Path.home() / ".kait" / "chip_insights"
CHIP_ADVICE_FILE_TAIL = 40
CHIP_ADVICE_MAX_FILES = 6
CHIP_ADVICE_LIMIT = 4
CHIP_ADVICE_MIN_SCORE = 0.7
CHIP_TELEMETRY_BLOCKLIST = {"kait-core", "bench_core"}
...
_TRANSCRIPT_ARTIFACT_PATTERNS = (
    ...
    re.compile(r"^\s*#\s*kait\s", re.I),
)
RECENT_OUTCOMES_MAX = 5000
REPLAY_ADVISORY_ENABLED = os.environ.get("KAIT_ADVISORY_REPLAY_ENABLED", "1") != "0"
REPLAY_MIN_STRICT_SAMPLES = int(os.environ.get("KAIT_ADVISORY_REPLAY_MIN_STRICT", "4") or 4)
REPLAY_MIN_IMPROVEMENT_DELTA = float(
    os.environ.get("KAIT_ADVISORY_REPLAY_MIN_DELTA", "0.20") or 0.20
)
REPLAY_MAX_RECORDS = int(os.environ.get("KAIT_ADVISORY_REPLAY_MAX_RECORDS", "3500") or 3500)
REPLAY_MAX_AGE_S = int(os.environ.get("KAIT_ADVISORY_REPLAY_MAX_AGE_S", str(21 * 86400)) or (21 * 86400))
REPLAY_STRICT_WINDOW_S = int(os.environ.get("KAIT_ADVISORY_REPLAY_STRICT_WINDOW_S", "1200") or 1200)
REPLAY_MIN_CONTEXT_MATCH = float(os.environ.get("KAIT_ADVISORY_REPLAY_MIN_CONTEXT", "0.12") or 0.12)
...
# Defaults — overridden by ~/.kait/tuneables.json → "advisor" section at module load.
...
MIND_MAX_STALE_SECONDS = float(os.environ.get("KAIT_ADVISOR_MIND_MAX_STALE_S", "0"))
MIND_STALE_ALLOW_IF_EMPTY = os.environ.get("KAIT_ADVISOR_MIND_STALE_ALLOW_IF_EMPTY", "1") != "0"
MIND_MIN_SALIENCE = float(os.environ.get("KAIT_ADVISOR_MIND_MIN_SALIENCE", "0.5"))
...
# Live routing tuneables are loaded from ~/.kait/tuneables.json -> "retrieval".
...
def _chips_enabled() -> bool:
    if str(os.environ.get("KAIT_ADVISORY_DISABLE_CHIPS", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
        ...
        return False
    chips_raw = str(os.environ.get("KAIT_CHIPS_ENABLED", "")).strip().lower()
    premium_raw = str(os.environ.get("KAIT_PREMIUM_TOOLS", "")).strip().lower()
...
def _premium_tools_enabled() -> bool:
    return str(os.environ.get("KAIT_PREMIUM_TOOLS", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _load_advisor_config() -> None:
    """Load advisor tuneables from ~/.kait/tuneables.json → "advisor" section.
    ...
    try:
        # Tests should be deterministic and not depend on user-local ~/.kait state.
        ...
        if "pytest" in sys.modules and str(os.environ.get("KAIT_TEST_ALLOW_HOME_TUNEABLES", "")).strip().lower() not in {
            "1",
            "true",
            "yes",
            "on",
...
def _maybe_warn_deprecated_advisor_retrieval_policy(
    ...
    sys.stderr.write(
        "[KAIT][warn] 'advisor.retrieval_policy.*' in ~/.kait/tuneables.json is ignored by runtime. "
...
_advisor_log = logging.getLogger("kait.advisor")
...
class KaitAdvisor:
    ...
    def _load_retrieval_policy(self) -> Dict[str, Any]:
        ...
        level = str(os.getenv("KAIT_RETRIEVAL_LEVEL", "1") or "1").strip()
        ...
        env_mode = str(os.getenv("KAIT_RETRIEVAL_MODE", "") or "").strip().lower()
        if env_mode in {"auto", "embeddings_only", "hybrid_agentic"}:
            policy["mode"] = env_mode

        env_minimax_fast = str(os.getenv("KAIT_ADVISORY_MINIMAX_FAST_RERANK", "") or "").strip().lower()
        ...
        if env_minimax_high_volume_items is not None:
            try:
                ...
                pass
        env_minimax_require_agentic = str(os.getenv("KAIT_ADVISORY_MINIMAX_REQUIRE_AGENTIC") or "").strip().lower()
        ...
        env_minimax_model = str(os.getenv("KAIT_ADVISORY_MINIMAX_MODEL") or "").strip()
        if env_minimax_model:
            policy["minimax_fast_rerank_model"] = env_minimax_model
        env_minimax_timeout = os.getenv("KAIT_ADVISORY_MINIMAX_TIMEOUT_S")
        if env_minimax_timeout is not None:
            try:
                policy["minimax_fast_rerank_timeout_s"] = max(2.0, float(env_minimax_timeout))
            except Exception:
                pass
        env_minimax_cooldown = os.getenv("KAIT_ADVISORY_MINIMAX_COOLDOWN_S")
        ...
        policy["minimax_fast_rerank_model"] = str(
            policy.get("minimax_fast_rerank_model", os.getenv("KAIT_MINIMAX_MODEL", "MiniMax-M2.5")) or "MiniMax-M2.5"
    ...
    def _load_memory_emotion_cfg(self) -> Dict[str, Any]:
        ...
        env_enabled = os.getenv("KAIT_ADVISORY_MEMORY_EMOTION_ENABLED")
        if env_enabled is not None:
            cfg["enabled"] = _parse_bool(env_enabled, cfg["enabled"])
        env_weight = os.getenv("KAIT_ADVISORY_MEMORY_EMOTION_WEIGHT")
        if env_weight is not None:
            cfg["advisory_rerank_weight"] = _safe_float(env_weight, cfg["advisory_rerank_weight"])
        env_min_sim = os.getenv("KAIT_ADVISORY_MEMORY_EMOTION_MIN_SIM")
    ...
    def _current_emotion_state_for_rerank(self) -> Optional[Dict[str, Any]]:
        try:
            from .kait_emotions import KaitEmotions

            state = (KaitEmotions().status() or {}).get("state") or {}
    ...
    def _get_bank_advice(self, context: str) -> List[Advice]:
        ...
        for mem in memories:
            ...
            if self._is_metadata_pattern(text):
                continue
            if text.startswith("You are Kait Intelligence, observing a live coding session"):
    ...
    def _load_recent_eidos_priority_map(self) -> Dict[str, float]:
        ...
        try:
            eidos_file = Path.home() / ".kait" / "eidos_distillations.jsonl"
    ...
    def _is_inventory_style_text(self, text: str) -> bool:
        ...
        return any(
            ...
            for marker in (
                ...
                "you are kait intelligence, observing a live coding session",
...
# ============= Singleton =============
_advisor: Optional[KaitAdvisor] = None

def get_advisor() -> KaitAdvisor:
    """Get the global advisor instance."""
    global _advisor
    if _advisor is None:
        _advisor = KaitAdvisor()
    return _advisor
